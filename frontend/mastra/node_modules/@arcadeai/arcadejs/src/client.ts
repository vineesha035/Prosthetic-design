...
export class Arcade {
   ...
   */
  constructor({
    baseURL = readEnv('ARCADE_BASE_URL'),
    apiKey = readEnv('ARCADE_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
    ...
    this.timeout = options.timeout ?? Arcade.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('ARCADE_LOG'), "process.env['ARCADE_LOG']", this) ??
      defaultLogLevel;
