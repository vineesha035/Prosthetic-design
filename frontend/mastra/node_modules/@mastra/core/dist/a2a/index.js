import "../rolldown-runtime-DP3BCW9_.js";
import { a as RequestContext } from "../request-context-p_Tq-4EM.js";
import { n as MessageList, t as convertMessages } from "../message-list-mC29laJJ.js";
import { t as MastraA2AError } from "../client-D9NcUAi0.js";
...
function unwrapA2AResult(result) {
	if (!result || typeof result !== "object") throw MastraA2AError.invalidAgentResponse("Remote A2A agent returned an invalid response.");
...
async function requireResponseBody(response, operation) {
	if (!response.body) throw MastraA2AError.invalidAgentResponse(`Remote A2A agent returned an empty stream for ${operation}.`);
...
var A2AAgent = class {
	...
	async resumeGenerate(resumeData, options) {
		...
		if (!runId) throw MastraA2AError.invalidParams("A2AAgent.resumeGenerate requires a runId.");
		const state = this.#runState.get(runId);
		if (!state) throw MastraA2AError.invalidParams(`No resumable A2A run state found for runId "${runId}".`);
		const bootstrap = await this.#getBootstrap();
		const memoryInfo = resolveMemoryInfo(options);
		...
		if (!state.taskId) throw MastraA2AError.invalidParams(`A2AAgent resume state for "${runId}" is missing a taskId.`);
	...
	async resumeStream(resumeData, options) {
		const runId = options?.runId;
		if (!runId) throw MastraA2AError.invalidParams("A2AAgent.resumeStream requires a runId.");
		const state = this.#runState.get(runId);
		if (!state) throw MastraA2AError.invalidParams(`No resumable A2A run state found for runId "${runId}".`);
		const bootstrap = await this.#getBootstrap();
		const memoryInfo = resolveMemoryInfo(options);
	...
	async #getTask({ bootstrap, taskId, signal }) {
		...
		if (!isTask(result)) throw MastraA2AError.invalidAgentResponse("Remote A2A agent returned a non-task response for tasks/get.");
		return result;
	}
	...
	async #request(url, { method = "POST", headers = {}, body, stream = false, credentials, signal } = {}) {
		...
		while (attempts <= this.#retries) try {
			...
			if (!response.ok) throw MastraA2AError.invalidAgentResponse(`Remote A2A request failed with status ${response.status}.`, {
...
//#endregion
export { A2AAgent, MastraA2AError };
