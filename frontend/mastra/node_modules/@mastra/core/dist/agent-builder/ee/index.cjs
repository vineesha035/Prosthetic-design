...
function splitRuntimeModelString(input) {
	const providers = require_provider_registry.getRegisteredProviders().sort((a, b) => b.length - a.length);
	...
	const parsed = require_provider_registry.parseModelString(input);
...
function isModelAllowed(allowed, candidate) {
	...
	const activeEntries = allowed.filter((entry) => {
		...
		return require_provider_registry.isProviderRegistered(entry.provider);
	});
