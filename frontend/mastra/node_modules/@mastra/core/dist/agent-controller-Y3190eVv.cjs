...
const require_channels = require("./channels-DpBkVl9k.cjs");
const require_mastra = require("./mastra-CSCDBtDZ.cjs");
...
function getDisplayTransform(metadata, phase, fallback) {
	const transform = require_payload_transform.getTransformedToolPayload(metadata, "display", phase);
...
function describeServerSideFallback(providerMetadata) {
	const fallback = require_agent.getServerSideFallbackInfo(providerMetadata);
	if (!fallback) return;
	...
	*/
	...
	async processStreamChunk(state, chunk, requestContext) {
		...
		switch (chunk.type) {
			...
			case "tool-call-delta": {
				...
				const toolName = getString(payload.toolName);
				const transform = require_payload_transform.getTransformedToolPayload(chunk.metadata, "display", "input-delta");
				if (!transform?.suppress) this.#session.emit({
					type: "tool_input_delta",
					toolCallId,
					argsTextDelta: require_payload_transform.hasTransformedToolPayload(transform) ? transform.transformed : argsTextDelta,
					toolName
				});
				break;
			}
			...
			case "tool-call-approval": {
				...
				const approvalTransform = require_payload_transform.getTransformedToolPayload(chunk.metadata, "display", "approval");
				const toolArgs = require_payload_transform.hasTransformedToolPayload(approvalTransform) ? approvalTransform.transformed : getDisplayTransform(chunk.metadata, "input-available", getPayload(chunk).args);
			...
			case "error": {
				const streamError = require_error.getErrorFromUnknown(getPayload(chunk).error);
				this.#session.emit({
					type: "error",
					error: streamError
				});
	...
	async handleSubscribedStreamError(error) {
		if (error instanceof Error && error.name === "AbortError") this.#session.emit({
			...
			this.#session.emit({
				type: "error",
				error: require_error.getErrorFromUnknown(error)
			});
			this.#session.emit({
				type: "agent_end",
				reason: "error"
...
var SessionOMRole = class {
	...
	*/
	resolvedModel() {
		...
		return new require_llm.ModelRouterLanguageModel(modelId, this.#gateways);
	}
...
var SessionState = class {
	...
	constructor({ initialState, stateSchema }, bus) {
		this.#schema = stateSchema ? (0, _mastra_schema_compat_schema.toStandardSchema)(stateSchema) : void 0;
		this.#state = {
	...
	apply(event) {
		...
		switch (event.type) {
			...
			case "tool_update": {
				...
				if (tool) tool.partialResult = typeof event.partialResult === "string" ? event.partialResult : require_utils_safe_stringify.safeStringify(event.partialResult);
				break;
			}
			case "tool_end": {
...
var Session = class {
	...
	*/
	...
	constructor({ resourceId, state, id, ownerId, tags, workspace, browser }) {
		...
		if (!workspace || !(workspace instanceof require_workspace.Workspace)) throw new Error(`A session requires a valid workspace instance.`);
	...
	*/
	sendSignal(input, options) {
		...
		const signal = require_signals.createSignal("content" in input ? {
			type: "user",
			tagName: "user",
	...
	*/
	async respondToToolSuspension({ resumeData, toolCallId, requestContext }) {
		...
		try {
			...
			const err = require_error.getErrorFromUnknown(error);
			this.emit({
				type: "error",
				error: err
			});
...
- The task is self-contained and can be delegated`,
		inputSchema: zod_v4.z.object({
			agentType: zod_v4.z.enum(subagentIds).describe("Type of subagent to spawn"),
			task: zod_v4.z.string().describe("Clear, self-contained description of what the subagent should do. For non-forked subagents include all relevant context — the subagent cannot see the parent conversation."),
			modelId: zod_v4.z.string().optional().describe("Optional model ID override for this task. Ignored when `forked: true` (the parent agent's model is used)."),
			forked: zod_v4.z.boolean().optional().describe("If true, fork the parent conversation: clone the parent thread and run with the parent agent's instructions/tools so prompt cache is preserved. Requires memory to be configured on the AgentController. Defaults to the subagent definition's `forked` setting.")
		}),
		execute: async (input, context) => {
			...
			if (runAsForked) {
				...
				if (context?.requestContext) {
					subagentRequestContext = new require_request_context.RequestContext(context.requestContext.entries());
					if (controllerCtx) subagentRequestContext.set("controller", {
						...controllerCtx,
						threadId: forkedThread.id,
						resourceId: forkedThread.resourceId
					});
					subagentRequestContext.delete(require_agent.CHAT_CHANNEL_RENDER_CONTEXT_KEY);
				}
				...
				subagentToRun = new require_agent.Agent({
				...
				const allWorkspaceToolNames = workspace && allowedWs ? new Set(Object.keys(await require_workspace.createWorkspaceTools(workspace, {
...
function patchTaskToolForFork(tool, toolId) {
	const stubExecute = async () => {
		...
		if (toolId === "task_check") {
			const taskCheck = require_task_state_processor.summarizeTaskCheck([]);
			return {
				...baseResult,
...
var AgentController = class {
	...
	*/
	...
	constructor(config) {
		...
		if (config.channels) {
			this.#channels = new require_channels.AgentControllerChannels(config.channels);
			this.#channels.__setController(this);
		}
		this.#gatewayManager = new require_llm.GatewayManager([...config.gateways ?? [], ...require_llm.defaultGateways]);
	...
	*/
	...
	async #createSessionForResource(ownerId, id, effectiveResourceId, tags, overrides) {
		const requestContext = overrides?.requestContext ?? new require_request_context.RequestContext();
		...
		if (workspaceToConnect && workspaceToConnect instanceof require_workspace.Workspace) try {
			...
			const initError = require_error.getErrorFromUnknown(error);
			session.emit({
				type: "workspace_status_changed",
				status: "error",
				error: initError
	...
	*/
	...
	async runInit() {
		if (this.config.storage && !this.#externalMastra) {
			...
			this.#internalMastra = new require_mastra.Mastra({
				logger: false,
		...
		if (this.config.workspace && !this.workspaceInitialized && typeof this.workspace !== "function") try {
			if (!this.workspace) this.workspace = new require_workspace.Workspace(this.config.workspace);
	...
	*/
	...
	getAgentForMode(mode) {
		...
		if (!this.#legacyAgentMode[mode.id]) {
			...
			this.#legacyAgentMode[mode.id] = new require_agent.Agent({
				id: `${this.id}-agent`,
	...
	*/
	async buildToolsets(session, requestContext) {
		const builtInTools = {
			ask_user: require_tools.askUserTool,
			submit_plan: require_tools.submitPlanTool,
	...
	*/
	async buildRequestContext(session, requestContext) {
		requestContext ??= new require_request_context.RequestContext();
...
Object.defineProperty(exports, "defaultOMProgressState", {
	enumerable: true,
	get: function() {
		return defaultOMProgressState;
	}
});
Object.defineProperty(exports, "parseSubagentMeta", {
	enumerable: true,
	get: function() {
		return parseSubagentMeta;
	}
});

//# sourceMappingURL=agent-controller-Y3190eVv.cjs.map