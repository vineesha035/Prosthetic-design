import type { BackgroundTaskFailedPayload, BackgroundTaskResultPayload, BackgroundTaskStartedPayload, BackgroundTaskProgressPayload, BackgroundTaskSuspendedPayload, BackgroundTaskResumedPayload, AgentChunkType } from '../stream/types.js';
export type BackgroundTaskStatus = 'pending' | 'running' | 'suspended' | 'completed' | 'failed' | 'cancelled' | 'timed_out';
...
export type BackgroundTaskOutputChunk = Extract<AgentChunkType, {
    type: 'tool-output';
}>;
export interface BackgroundTaskEvent extends BackgroundTask {
    chunk?: BackgroundTaskOutputChunk;
}
export type UpdateBackgroundTask = Partial<Omit<BackgroundTask, 'id' | 'createdAt' | 'threadId' | 'resourceId' | 'runId' | 'agentId' | 'toolCallId' | 'toolName' | 'args'>>;
/**
...
export type AgentBackgroundToolConfig = boolean | {
    enabled: boolean;
    timeoutMs?: number;
};
