...
import { E as isProcessorWorkflow } from "../trip-wire-csEv7lz7.js";
import { d as BrowserCliHandler, f as browserCliHandler } from "../workspace-CvZ9jmZN.js";
import { randomUUID } from "crypto";
import { EventEmitter } from "events";
import { z } from "zod";
import { closeSync, existsSync, fstatSync, lstatSync, mkdirSync, openSync, readdirSync, unlinkSync, writeSync } from "fs";
...
*/
var MastraBrowser = class MastraBrowser extends MastraBase {
	/**
	...
	*/
	...
	constructor(config = {}) {
		super({
			name: "MastraBrowser",
			component: RegisteredLogger.BROWSER
		});
		this.config = config;
...
2. Remove profile/executablePath to connect to the existing browser via CDP`);
	...
	*/
	getInputProcessors(configuredProcessors = []) {
		if (configuredProcessors.some((p) => !isProcessorWorkflow(p) && "id" in p && p.id === "browser-context")) return [];
...
function resolveOutputPath(id, outputDir, requestedPath) {
	...
	if (rel === "" || isAbsolute(rel) || rel === ".." || rel.startsWith(`..${sep}`)) throw new Error(`Recording outputPath must be inside ${baseDir}`);
	return outputPath;
}
function clearState() {
...
const recordSchema = z.object({
	...
	outputPath: z.string().min(1).optional().describe("Only used with action=\"start\". Absolute path inside the configured recording output directory. Defaults to a generated file there.")
});
...
//#endregion
export { BrowserCliHandler, BrowserContextProcessor, DEFAULT_THREAD_ID, MastraBrowser, SCREENCAST_DEFAULTS, ScreencastStream as ScreencastStreamImpl, ThreadManager, browserCliHandler, createBrowserRecordingTools, createError };
