...
var MastraA2AError = class MastraA2AError extends Error {
	...
	*/
	toJSONRPCError() {
		...
		if (this.data !== void 0) errorObject.data = this.data;
		return errorObject;
	}
	...
	static taskNotFound(taskId) {
		return new MastraA2AError(ErrorCodeTaskNotFound, `Task not found: ${taskId}`, void 0, taskId);
	}
	static taskNotCancelable(taskId) {
		return new MastraA2AError(ErrorCodeTaskNotCancelable, `Task not cancelable: ${taskId}`, void 0, taskId);
	}
	...
	static contentTypeNotSupported(contentType) {
		return new MastraA2AError(ErrorCodeContentTypeNotSupported, `Content type not supported: ${contentType}`, { contentType });
	}
	static invalidAgentResponse(message, data) {
		return new MastraA2AError(ErrorCodeInvalidAgentResponse, message, data);
	}
	static extendedAgentCardNotConfigured() {
		return new MastraA2AError(ErrorCodeExtendedAgentCardNotConfigured, "Extended agent card is not configured");
	}
	static extensionSupportRequired(extension) {
		return new MastraA2AError(ErrorCodeExtensionSupportRequired, extension ? `Extension support required: ${extension}` : "Extension support required", extension ? { extension } : void 0);
	}
	static versionNotSupported(version) {
...
Object.defineProperty(exports, "MastraA2AError", {
	enumerable: true,
	get: function() {
		return MastraA2AError;
	}
});

//# sourceMappingURL=client-C9xbmacV.cjs.map