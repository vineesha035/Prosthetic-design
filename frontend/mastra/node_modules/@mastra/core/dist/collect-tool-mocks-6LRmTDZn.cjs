//#region src/evals/collect-tool-mocks.ts
/**
...
function extractToolName(label) {
	return label.match(/^(?:mcp_)?tool:\s*'(.*?)'/)?.[1] ?? label;
}
/**
...
function collectToolMocksInto(steps, acc) {
	...
	for (const step of steps) {
		...
		if (!isToolCall && step.children?.length) collectToolMocksInto(step.children, acc);
	}
	return acc;
}
//#endregion
Object.defineProperty(exports, "collectToolMocks", {
	enumerable: true,
	get: function() {
		return collectToolMocks;
	}
});

//# sourceMappingURL=collect-tool-mocks-6LRmTDZn.cjs.map