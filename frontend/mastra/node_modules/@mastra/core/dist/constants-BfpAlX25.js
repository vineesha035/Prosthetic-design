import { An as sortDirectionSchema, B as deltaLimitSchema, F as dbTimestamps, Gt as listModeSchema, In as traceIdField, Mn as spanContextFields, Nn as spanIdField, P as dateRangeSchema, Pn as tagsField, R as deltaCursorSchema, cn as normalizeObservabilityListArgs, dn as paginationInfoSchema, en as metadataField, hn as refineObservabilityListMode, un as paginationArgsSchema, z as deltaInfoSchema } from "./utils-DxsDNzD2.js";
import { n as SpanType } from "./tracing-Bm0k4FBA.js";
import { n as getZodInnerType, r as getZodTypeName } from "./zod-utils-DTkc-hhd.js";
import { z } from "zod/v4";
//#region src/evals/types.ts
const scoringSourceSchema = z.enum(["LIVE", "TEST"]);
const scoringEntityTypeSchema = z.enum([
	"AGENT",
	"WORKFLOW",
	"TRAJECTORY",
	"STEP",
...
const scoringPromptsSchema = z.object({
...
const scoringHookInputSchema = z.object({
	runId: z.string().optional(),
	scorer: recordSchema,
	input: z.unknown(),
	output: z.unknown(),
...
const scoringExtractStepResultSchema = optionalRecordSchema;
const scoringValueSchema = z.number();
const scoreResultSchema = z.object({
	result: optionalRecordSchema,
	score: scoringValueSchema,
	prompt: z.string().optional()
});
const scoringInputWithExtractStepResultSchema = scoringInputSchema.extend({
	runId: z.string(),
	extractStepResult: optionalRecordSchema,
	extractPrompt: z.string().optional()
});
const scoringInputWithExtractStepResultAndAnalyzeStepResultSchema = scoringInputWithExtractStepResultSchema.extend({
	score: z.number(),
	analyzeStepResult: optionalRecordSchema,
	analyzePrompt: z.string().optional()
});
const scoringInputWithExtractStepResultAndScoreAndReasonSchema = scoringInputWithExtractStepResultAndAnalyzeStepResultSchema.extend({
	reason: z.string().optional(),
	reasonPrompt: z.string().optional()
});
const scoreRowDataSchema = z.object({
	id: z.string(),
	scorerId: z.string(),
	entityId: z.string(),
	runId: z.string(),
...
const saveScorePayloadSchema = scoreRowDataSchema.omit({
	id: true,
	createdAt: true,
	updatedAt: true
});
...
function extractTrajectory(output) {
	...
	for (const message of output) {
		...
		for (const invocation of toolInvocations) if (invocation && invocation.toolName && (invocation.state === "result" || invocation.state === "call")) {
...
function extractWorkflowTrajectory(stepResults, stepExecutionPath) {
	...
	for (const stepId of stepIds) {
		...
		if (result.startedAt != null) {
			if (totalStartedAt == null || result.startedAt < totalStartedAt) totalStartedAt = result.startedAt;
		}
		const endedAt = "endedAt" in result ? result.endedAt : void 0;
		if (endedAt != null) {
			if (totalEndedAt == null || endedAt > totalEndedAt) totalEndedAt = endedAt;
		}
	...
	return {
		steps,
		totalDurationMs: totalStartedAt != null && totalEndedAt != null ? totalEndedAt - totalStartedAt : void 0,
		rawWorkflowResult: {
			stepResults,
			stepExecutionPath
		}
...
*/
const SKIPPED_SPAN_TYPES = /* @__PURE__ */ new Set([
	"scorer_run",
	"scorer_step",
	"generic",
	"model_step",
...
function spanToTrajectorySteps(node) {
	...
	if (SKIPPED_SPAN_TYPES.has(span.spanType)) return childNodes.flatMap(spanToTrajectorySteps);
	...
	switch (span.spanType) {
		...
		case "agent_run": return [{
			...base,
			stepType: "agent_run",
			agentId: span.entityId ?? void 0
		}];
		case "workflow_run": return [{
			...base,
			stepType: "workflow_run",
			workflowId: span.entityId ?? void 0
		}];
		case "workflow_step": {
		...
		case "workflow_conditional": return [{
			...base,
			stepType: "workflow_conditional"
		}];
		case "workflow_parallel": return [{
			...base,
			stepType: "workflow_parallel"
		}];
		case "workflow_loop": return [{
			...base,
			stepType: "workflow_loop"
		}];
		case "workflow_sleep": return [{
			...base,
			stepType: "workflow_sleep"
		}];
		case "workflow_wait_event": return [{
			...base,
			stepType: "workflow_wait_event"
		}];
		case "processor_run": return [{
			...base,
			stepType: "processor_run"
		}];
		default: return childSteps;
	}
}
/**
...
function extractTrajectoryFromTrace(spans, rootSpanId) {
	if (spans.length === 0) return { steps: [] };
	const nodeMap = /* @__PURE__ */ new Map();
	for (const span of spans) nodeMap.set(span.spanId, {
		span,
		children: []
	});
	const roots = [];
	for (const span of spans) {
		...
		if (span.parentSpanId && nodeMap.has(span.parentSpanId)) nodeMap.get(span.parentSpanId).children.push(node);
		else roots.push(node);
	}
	for (const node of nodeMap.values()) node.children.sort((a, b) => a.span.startedAt.getTime() - b.span.startedAt.getTime());
	let targetRoots;
	if (rootSpanId) {
		const rootNode = nodeMap.get(rootSpanId);
		targetRoots = rootNode ? [rootNode] : roots;
	} else targetRoots = roots;
	let stepsToConvert;
	if (targetRoots.length === 1) {
		const root = targetRoots[0];
		if ((/* @__PURE__ */ new Set(["workflow_run", "agent_run"])).has(root.span.spanType)) stepsToConvert = root.children;
		else stepsToConvert = targetRoots;
	} else stepsToConvert = targetRoots;
	const steps = stepsToConvert.flatMap(spanToTrajectorySteps);
	let totalDurationMs;
	if (targetRoots.length === 1) {
		const root = targetRoots[0].span;
		if (root.endedAt && root.startedAt) totalDurationMs = root.endedAt.getTime() - root.startedAt.getTime();
	}
	return {
		steps,
		totalDurationMs
...
const STORAGE_VISIBILITY_VALUES = ["private", "public"];
function unwrapSchema(schema) {
	...
	while (true) {
		...
		if (!typeName) break;
		if (typeName === "ZodNullable" || typeName === "ZodOptional") nullable = true;
...
function getZodChecks(schema) {
	if ("_zod" in schema) {
		...
		if (checks && Array.isArray(checks)) return checks.map((check) => {
			if (typeof check === "object" && check !== null && "def" in check && typeof check.def === "object" && check.def !== null) {
				const def = check.def;
				if (def.check === "number_format" && def.format === "safeint") return { kind: "int" };
				if (def.check === "string_format" && typeof def.format === "string") return { kind: def.format };
	...
	if ("_def" in schema) {
		...
		if (checks && Array.isArray(checks)) return checks;
	}
	return [];
}
function zodToStorageType(schema) {
	...
	if (typeName === "ZodString") {
		if (getZodChecks(schema).some((c) => c.kind === "uuid")) return "uuid";
		return "text";
	}
	if (typeName === "ZodNativeEnum" || typeName === "ZodEnum") return "text";
	if (typeName === "ZodNumber") return getZodChecks(schema).some((c) => c.kind === "int") ? "integer" : "float";
	if (typeName === "ZodBigInt" || typeName === "ZodBigint") return "bigint";
	if (typeName === "ZodDate") return "timestamp";
...
const STORAGE_FAVORITE_ENTITY_TYPES = ["agent", "skill"];
//#endregion
...
*/
const createOmitKeys = (shape) => Object.fromEntries(Object.keys(shape).map((k) => [k, true]));
...
let TraceStatus = /* @__PURE__ */ function(TraceStatus) {
...
const hasChildErrorField = z.preprocess((v) => {
...
const spanIdsSchema = z.object({ ...spanIds });
...
const traceSpanSchema = spanRecordSchema.extend({ status: traceStatusField }).describe("Trace span with computed status (root spans only)");
/**
...
*/
const createSpanRecordSchema = spanRecordSchema.omit(omitDbTimestamps);
/**
* Schema for createSpan operation arguments
*/
const createSpanArgsSchema = z.object({ span: createSpanRecordSchema }).describe("Arguments for creating a single span");
/**
* Schema for batchCreateSpans operation arguments
*/
const batchCreateSpansArgsSchema = z.object({ records: z.array(createSpanRecordSchema) }).describe("Arguments for batch creating spans");
/**
* Schema for getSpan operation arguments
*/
const getSpanArgsSchema = z.object({
...
*/
const getSpanResponseSchema = z.object({ span: spanRecordSchema });
/**
...
* entire trace.
*/
const getSpansArgsSchema = z.object({
	traceId: traceIdField.min(1),
...
const getSpansResponseSchema = z.object({
	traceId: traceIdField,
...
*/
const getRootSpanArgsSchema = z.object({ traceId: traceIdField.min(1) }).describe("Arguments for getting a root span");
/**
* Response schema for getRootSpan operation
*/
const getRootSpanResponseSchema = z.object({ span: spanRecordSchema });
/**
* Schema for getTrace operation arguments
*/
const getTraceArgsSchema = z.object({ traceId: traceIdField.min(1) }).describe("Arguments for getting a single trace");
/**
* Response schema for getTrace operation
*/
const getTraceResponseSchema = z.object({
	traceId: traceIdField,
	spans: z.array(spanRecordSchema)
});
/**
...
*/
const getBranchArgsSchema = z.object({
	traceId: traceIdField.min(1),
	spanId: spanIdField.min(1),
...
*/
const getBranchResponseSchema = z.object({
	traceId: traceIdField,
	spans: z.array(spanRecordSchema)
});
/**
...
*/
const lightSpanRecordSchema = z.object({
	...spanIds,
	name: spanNameField,
	spanType: spanTypeField,
	isEvent: isEventField,
	...
	...dbTimestamps
}).describe("Lightweight span record for timeline rendering (excludes input, output, attributes, metadata, tags, links)");
/**
* Response schema for getStructure operation.
* Returns a trace with lightweight spans (only fields needed for timeline).
*/
const getStructureResponseSchema = z.object({
	traceId: traceIdField,
	spans: z.array(lightSpanRecordSchema)
});
/** @deprecated Use {@link getStructureResponseSchema} instead. */
const getTraceLightResponseSchema = getStructureResponseSchema;
...
*/
const tracesOrderByFieldSchema = z.enum(["startedAt", "endedAt"]).describe("Field to order by: 'startedAt' | 'endedAt'");
/**
...
*/
const listTracesArgsSchema = z.object({
...
const listTracesLightResponseSchema = z.object({
	pagination: paginationInfoSchema,
	spans: z.array(lightSpanRecordSchema)
});
/**
...
const BRANCH_SPAN_TYPE_SET = new Set(BRANCH_SPAN_TYPES);
/** Schema for filtering branch anchor spans in list queries. */
const branchesFilterSchema = z.object({
	startedAt: dateRangeSchema.optional().describe("Filter by span start time range"),
	endedAt: dateRangeSchema.optional().describe("Filter by span end time range"),
	spanType: spanTypeField.optional(),
...
const branchesOrderByFieldSchema = z.enum(["startedAt", "endedAt"]).describe("Field to order by: 'startedAt' | 'endedAt'");
const branchesOrderBySchema = z.object({
...
*/
const updateSpanRecordSchema = createSpanRecordSchema.omit(omitSpanIds);
/**
* Schema for updateSpan operation arguments
*/
const updateSpanArgsSchema = z.object({
	spanId: spanIdField,
	traceId: traceIdField,
...
*/
const batchUpdateSpansArgsSchema = z.object({ records: z.array(z.object({
	traceId: traceIdField,
	spanId: spanIdField,
	updates: updateSpanRecordSchema.partial()
...
*/
const batchDeleteTracesArgsSchema = z.object({ traceIds: z.array(traceIdField) }).describe("Arguments for batch deleting traces");
/** Schema for listScoresBySpan operation response */
const listScoresBySpanResponseSchema = z.object({
	pagination: paginationInfoSchema,
	scores: z.array(scoreRowDataSchema)
});
/** Schema for scoreTraces operation request */
const scoreTracesRequestSchema = z.object({
...
const scoreTracesResponseSchema = z.object({
	status: z.string(),
	message: z.string(),
	traceCount: z.number()
});
...
const TABLE_SPANS = "mastra_ai_spans";
const TABLE_AGENTS = "mastra_agents";
const TABLE_AGENT_VERSIONS = "mastra_agent_versions";
const TABLE_OBSERVATIONAL_MEMORY = "mastra_observational_memory";
const TABLE_PROMPT_BLOCKS = "mastra_prompt_blocks";
const TABLE_PROMPT_BLOCK_VERSIONS = "mastra_prompt_block_versions";
const TABLE_SCORER_DEFINITIONS = "mastra_scorer_definitions";
const TABLE_SCORER_DEFINITION_VERSIONS = "mastra_scorer_definition_versions";
const TABLE_MCP_CLIENTS = "mastra_mcp_clients";
const TABLE_MCP_CLIENT_VERSIONS = "mastra_mcp_client_versions";
const TABLE_MCP_SERVERS = "mastra_mcp_servers";
const TABLE_MCP_SERVER_VERSIONS = "mastra_mcp_server_versions";
const TABLE_WORKSPACES = "mastra_workspaces";
const TABLE_WORKSPACE_VERSIONS = "mastra_workspace_versions";
const TABLE_SKILLS = "mastra_skills";
const TABLE_SKILL_VERSIONS = "mastra_skill_versions";
const TABLE_SKILL_BLOBS = "mastra_skill_blobs";
const TABLE_FAVORITES = "mastra_favorites";
const TABLE_DATASETS = "mastra_datasets";
const TABLE_DATASET_ITEMS = "mastra_dataset_items";
const TABLE_DATASET_VERSIONS = "mastra_dataset_versions";
const TABLE_EXPERIMENTS = "mastra_experiments";
const TABLE_EXPERIMENT_RESULTS = "mastra_experiment_results";
const TABLE_BACKGROUND_TASKS = "mastra_background_tasks";
const TABLE_SCHEDULES = "mastra_schedules";
const TABLE_SCHEDULE_TRIGGERS = "mastra_schedule_triggers";
const TABLE_WORKFLOW_DEFINITIONS = "mastra_workflow_definitions";
const TABLE_CHANNEL_INSTALLATIONS = "mastra_channel_installations";
const TABLE_CHANNEL_CONFIG = "mastra_channel_config";
const TABLE_TOOL_PROVIDER_CONNECTIONS = "mastra_tool_provider_connections";
const TABLE_NOTIFICATIONS = "mastra_notifications";
const TABLE_HARNESS_SESSIONS = "mastra_harness_sessions";
const TABLE_THREAD_STATE = "mastra_thread_state";
const SCORERS_SCHEMA = {
	id: {
		type: "text",
		nullable: false,
		primaryKey: true
...
const PROMPT_BLOCK_VERSIONS_SCHEMA = {
	id: {
		type: "text",
		nullable: false,
		primaryKey: true
...
const SCORER_DEFINITIONS_SCHEMA = {
	id: {
		type: "text",
		nullable: false,
		primaryKey: true
...
const SCORER_DEFINITION_VERSIONS_SCHEMA = {
	id: {
		type: "text",
		nullable: false,
		primaryKey: true
...
const MCP_CLIENT_VERSIONS_SCHEMA = {
	id: {
		type: "text",
		nullable: false,
		primaryKey: true
...
const MCP_SERVER_VERSIONS_SCHEMA = {
	id: {
		type: "text",
		nullable: false,
		primaryKey: true
...
const WORKFLOW_DEFINITIONS_SCHEMA = {
	id: {
		type: "text",
		nullable: false,
		primaryKey: true
...
const WORKSPACE_VERSIONS_SCHEMA = {
	id: {
		type: "text",
		nullable: false,
		primaryKey: true
...
* context).
*/
const TOOL_PROVIDER_CONNECTIONS_SCHEMA = {
	authorId: {
		type: "text",
		nullable: false
	},
...
const OBSERVATIONAL_MEMORY_SCHEMA = {
	id: {
		type: "text",
		nullable: false,
		primaryKey: true
...
const EXPERIMENT_RESULTS_SCHEMA = {
	id: {
		type: "text",
		nullable: false,
		primaryKey: true
...
const TABLE_CONFIGS = {
	[TABLE_DATASET_ITEMS]: {
		columns: DATASET_ITEMS_SCHEMA,
		compositePrimaryKey: ["id", "datasetVersion"]
	},
	[TABLE_FAVORITES]: {
		columns: FAVORITES_SCHEMA,
		compositePrimaryKey: [
			"userId",
			"entityType",
			"entityId"
	...
	[TABLE_NOTIFICATIONS]: {
		columns: NOTIFICATIONS_SCHEMA,
		compositePrimaryKey: ["threadId", "id"]
	},
	[TABLE_THREAD_STATE]: {
		columns: THREAD_STATE_SCHEMA,
...
*/
const OBSERVATIONAL_MEMORY_TABLE_SCHEMA = { [TABLE_OBSERVATIONAL_MEMORY]: OBSERVATIONAL_MEMORY_SCHEMA };
//#endregion
export { TABLE_SCORERS as $, spanIds as $t, TABLE_CHANNEL_CONFIG as A, createSpanRecordSchema as At, TABLE_MCP_CLIENTS as B, getStructureResponseSchema as Bt, SKILLS_SCHEMA as C, scoringInputSchema as Cn, batchDeleteTracesArgsSchema as Ct, TABLE_AGENTS as D, scoringPromptsSchema as Dn, branchesOrderBySchema as Dt, SPAN_SCHEMA as E, scoringInputWithExtractStepResultSchema as En, branchesOrderByFieldSchema as Et, TABLE_DATASET_VERSIONS as F, getRootSpanResponseSchema as Ft, TABLE_NOTIFICATIONS as G, listBranchesArgsSchema as Gt, TABLE_MCP_SERVERS as H, getTraceLightResponseSchema as Ht, TABLE_EXPERIMENTS as I, getSpanArgsSchema as It, TABLE_PROMPT_BLOCK_VERSIONS as J, listTracesArgsSchema as Jt, TABLE_OBSERVATIONAL_MEMORY as K, listBranchesResponseSchema as Kt, TABLE_EXPERIMENT_RESULTS as L, getSpanResponseSchema as Lt, TABLE_CONFIGS as M, getBranchArgsSchema as Mt, TABLE_DATASETS as N, getBranchResponseSchema as Nt, TABLE_AGENT_VERSIONS as O, scoringSourceSchema as On, computeTraceStatus as Ot, TABLE_DATASET_ITEMS as P, getRootSpanArgsSchema as Pt, TABLE_SCHEMAS as Q, scoreTracesResponseSchema as Qt, TABLE_FAVORITES as R, getSpansArgsSchema as Rt, SCORER_DEFINITION_VERSIONS_SCHEMA as S, scoringHookInputSchema as Sn, batchCreateSpansArgsSchema as St, SKILL_VERSIONS_SCHEMA as T, scoringInputWithExtractStepResultAndScoreAndReasonSchema as Tn, branchesFilterSchema as Tt, TABLE_MCP_SERVER_VERSIONS as U, getTraceResponseSchema as Ut, TABLE_MCP_CLIENT_VERSIONS as V, getTraceArgsSchema as Vt, TABLE_MESSAGES as W, lightSpanRecordSchema as Wt, TABLE_SCHEDULES as X, listTracesResponseSchema as Xt, TABLE_RESOURCES as Y, listTracesLightResponseSchema as Yt, TABLE_SCHEDULE_TRIGGERS as Z, scoreTracesRequestSchema as Zt, OLD_SPAN_SCHEMA as _, saveScorePayloadSchema as _n, WORKSPACES_SCHEMA as _t, DATASET_VERSIONS_SCHEMA as a, tracesFilterSchema as an, TABLE_SPANS as at, SCORERS_SCHEMA as b, scoringEntityTypeSchema as bn, BRANCH_SPAN_TYPE_SET as bt, FAVORITES_SCHEMA as c, updateSpanArgsSchema as cn, TABLE_TOOL_PROVIDER_CONNECTIONS as ct, MCP_CLIENT_VERSIONS_SCHEMA as d, STORAGE_VISIBILITY_VALUES as dn, TABLE_WORKFLOW_SNAPSHOT as dt, spanIdsSchema as en, TABLE_SCORER_DEFINITIONS as et, MCP_SERVERS_SCHEMA as f, buildStorageSchema as fn, TABLE_WORKSPACES as ft, OBSERVATIONAL_MEMORY_TABLE_SCHEMA as g, listScoresResponseSchema as gn, WORKFLOW_DEFINITIONS_SCHEMA as gt, OBSERVATIONAL_MEMORY_SCHEMA as h, extractWorkflowTrajectory as hn, TOOL_PROVIDER_CONNECTIONS_SCHEMA as ht, DATASET_ITEMS_SCHEMA as i, traceSpanSchema as in, TABLE_SKILL_VERSIONS as it, TABLE_CHANNEL_INSTALLATIONS as j, extractBranchSpans as jt, TABLE_BACKGROUND_TASKS as k, scoringValueSchema as kn, createSpanArgsSchema as kt, HARNESS_SESSIONS_SCHEMA as l, updateSpanRecordSchema as ln, TABLE_TRACES as lt, NOTIFICATIONS_SCHEMA as m, extractTrajectoryFromTrace as mn, THREAD_STATE_SCHEMA as mt, AGENT_VERSIONS_SCHEMA as n, toTraceSpan as nn, TABLE_SKILLS as nt, EXPERIMENTS_SCHEMA as o, tracesOrderByFieldSchema as on, TABLE_THREADS as ot, MCP_SERVER_VERSIONS_SCHEMA as p, extractTrajectory as pn, TABLE_WORKSPACE_VERSIONS as pt, TABLE_PROMPT_BLOCKS as q, listScoresBySpanResponseSchema as qt, DATASETS_SCHEMA as r, toTraceSpans as rn, TABLE_SKILL_BLOBS as rt, EXPERIMENT_RESULTS_SCHEMA as s, tracesOrderBySchema as sn, TABLE_THREAD_STATE as st, AGENTS_SCHEMA as t, spanRecordSchema as tn, TABLE_SCORER_DEFINITION_VERSIONS as tt, MCP_CLIENTS_SCHEMA as u, STORAGE_FAVORITE_ENTITY_TYPES as un, TABLE_WORKFLOW_DEFINITIONS as ut, PROMPT_BLOCKS_SCHEMA as v, scoreResultSchema as vn, WORKSPACE_VERSIONS_SCHEMA as vt, SKILL_BLOBS_SCHEMA as w, scoringInputWithExtractStepResultAndAnalyzeStepResultSchema as wn, batchUpdateSpansArgsSchema as wt, SCORER_DEFINITIONS_SCHEMA as x, scoringExtractStepResultSchema as xn, TraceStatus as xt, PROMPT_BLOCK_VERSIONS_SCHEMA as y, scoreRowDataSchema as yn, BRANCH_SPAN_TYPES as yt, TABLE_HARNESS_SESSIONS as z, getSpansResponseSchema as zt };

//# sourceMappingURL=constants-BfpAlX25.js.map