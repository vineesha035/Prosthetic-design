...
function resolveDurableToolCallConcurrency({ options, toolsMetadata, toolCalls }) {
	...
	return typeof configured === "number" && configured > 0 ? configured : require_agent.DurableAgentDefaults.TOOL_CALL_CONCURRENCY;
}
//#endregion
//#region src/agent/durable/workflows/steps/background-task-check.ts
const BG_CHECK_STEP_ID = `${require_agent.DurableStepIds.AGENTIC_EXECUTION}-bg-task-check`;
/**
...
*/
const bgCheckInputSchema = zod.z.any();
const bgCheckOutputSchema = zod.z.any();
/**
...
async function applyToolPayloadTransformToChunk(chunk, opts) {
	...
	let transform;
	if (chunk.type === "tool-call") transform = await require_payload_transform.transformToolPayloadForTargets({
		...
		transformedChunk = require_payload_transform.withToolPayloadTransformMetadata(transformedChunk, await require_payload_transform.transformToolPayloadForTargets({
			phase: "input-available",
			toolName,
			toolCallId,
			input: payload.args,
		...
		transform = await require_payload_transform.transformToolPayloadForTargets({
	...
	return require_payload_transform.withToolPayloadTransformMetadata(transformedChunk, transform);
}
//#endregion
...
function makeSaveQueueManager(memory, mastra) {
	...
	return new require_agent.SaveQueueManager({
...
function restoreRequestContext(entries) {
	return entries ? new require_request_context.RequestContext(Object.entries(entries)) : new require_request_context.RequestContext();
}
/**
...
async function resolveRuntimeDependencies(options) {
	...
	const messageList = existingEntry?.messageList ? existingEntry.messageList.deserialize(input.messageListState) : new require_message_list.MessageList({
...
async function toolRequiresApproval(tool, globalRequireApproval, args, approvalContext) {
	...
	const needsApprovalFn = require_toolchecks.getNeedsApprovalFn(tool);
...
async function resolveModelFromConfig(config, mastra) {
	const requestContext = new require_request_context.RequestContext();
	const modelConfigString = config.originalConfig ?? `${config.provider}/${config.modelId}`;
	if (typeof modelConfigString === "string") return await require_llm.resolveModelConfig(modelConfigString, requestContext, mastra);
	return await require_llm.resolveModelConfig(modelConfigString, requestContext, mastra);
}
/**
...
*/
const durableLLMInputSchema = zod.z.object({
	runId: zod.z.string(),
	agentId: zod.z.string(),
	agentName: zod.z.string().optional(),
	messageListState: zod.z.any(),
	...
	modelList: zod.z.array(zod.z.object({
		id: zod.z.string(),
		config: zod.z.object({
			provider: zod.z.string(),
			modelId: zod.z.string(),
...
*/
const durableLLMOutputSchema = zod.z.object({
	messageListState: zod.z.any(),
	text: zod.z.string().optional(),
	toolCalls: zod.z.array(zod.z.object({
...
function createDurableLLMExecutionStep(_options) {
	return require_agent.createStep$1({
		id: require_agent.DurableStepIds.LLM_EXECUTION,
		...
		execute: async (params) => {
			...
			for (let modelIndex = 0; modelIndex < modelList.length; modelIndex++) {
				...
				for (let attempt = 0; attempt <= maxRetries; attempt++) try {
					...
					if (!require_trip_wire.isSupportedLanguageModel(model)) {
					...
					let currentProviderOptions = require_agent.mergeProviderOptions(execOptions.providerOptions, modelEntry.config.providerOptions);
					...
					const stepInputProcessors = registryEntry?.prepareStep ? [...baseInputProcessors, new require_agent.PrepareStepProcessor({ prepareStep: registryEntry.prepareStep })] : baseInputProcessors;
					if (stepInputProcessors.length) {
						const inputStepWriter = pubsub ? { custom: async (data) => {
							await emitChunkEvent(pubsub, runId, data);
						} } : void 0;
						const runner = new require_trip_wire.ProcessorRunner({
						...
						try {
							...
							const merged = require_agent.composeStepInput({
								messageId: currentMessageId,
								model: currentModel,
								tools: currentTools,
								toolChoice: currentToolChoice,
							...
							if (processInputStepResult.tools) {
								const boundLogger = logger || new require_logger.ConsoleLogger({ level: "error" });
								const convertedTools = {};
								for (const [name, tool] of Object.entries(currentTools)) if (require_toolchecks.isMastraTool(tool)) convertedTools[name] = require_utils$1.makeCoreTool(tool, {
									name,
									runId,
									threadId: typedInput.state?.threadId,
									resourceId: typedInput.state?.resourceId,
									logger: boundLogger,
									mastra: mastra ? require_utils$1.createMastraProxy({
										mastra,
										logger: boundLogger
									}) : void 0,
									memory: registryEntry?.memory,
					...
					const messageListPromptArgs = await require_agent.buildLlmPromptArgs({ model: currentModel });
					...
					inputMessages = require_agent.applyAutoResumeSystemMessage({
						autoResume: execOptions.autoResumeSuspendedTools,
						inputMessages,
						messages: messageList.get.all.db()
					});
					inputMessages = require_agent.injectBackgroundTaskPrompt({
						inputMessages,
					...
					const requestStepRunner = allInputProcessors.length > 0 ? new require_trip_wire.ProcessorRunner({
					...
					const resolveToolDef = (toolName) => {
						...
						const providerTool = require_agent.findProviderToolByName(currentTools, toolName);
						if (providerTool) return providerTool;
						return require_agent.findProviderToolByName(registryEntry?.tools, toolName);
					};
					...
					const injectClientToolObservability = ({ toolCallId, toolName, args, providerExecuted, payload }) => {
						...
						if (!(!require_agent.inferProviderExecuted(providerExecuted, toolDef) && !toolDef?.execute) || !mastra || !tracingContext?.currentSpan) return { toolDef };
					...
					const recordProviderToolCall = ({ toolCallId, toolName, args, providerExecuted }) => {
						...
						const toolDef = resolveToolDef(toolName);
						if (!require_agent.inferProviderExecuted(providerExecuted, toolDef)) return;
					...
					const cleanupToolObservabilitySpans = (flushPendingProviderToolCalls) => {
						...
						if (flushPendingProviderToolCalls) for (const [toolCallId, pending] of pendingProviderToolCallsByToolCallId.entries()) require_agent.endPendingProviderToolSpan({
							toolCallId,
							pending,
					...
					modelSpanTracker?.setInferenceContext?.({
						...
						providerOptions: currentProviderOptions,
						availableTools: require_utils.getStepAvailableToolNames(currentTools, currentActiveTools),
						toolChoice: currentToolChoice,
					...
					if (cachedResponse) {
					...
					} else modelResult = require_agent.execute({
						runId,
						model: currentModel,
						providerOptions: currentProviderOptions,
						inputMessages,
						...
						headers: require_agent.mergeLlmCallHeaders({
							memoryHeaders: require_agent.buildMemoryHeaders({
								threadId: typedInput.state?.threadId,
								resourceId: typedInput.state?.resourceId
							}),
					...
					const outputStream = new require_trip_wire.MastraModelOutput({
						model: {
					...
					try {
						...
						for await (const rawChunk of trackedStream) {
							...
							if (!stepStartEmitted && pubsub) {
								...
								await emitStepStartEvent(pubsub, runId, {
									stepId: require_agent.DurableStepIds.LLM_EXECUTION,
									request,
									warnings
								});
							}
							...
							switch (rawChunk.type) {
								...
								case "tool-result": {
									...
									if (pending) {
										require_agent.endPendingProviderToolSpan({
											toolCallId: payload.toolCallId,
											pending,
										...
										if (!require_agent.inferProviderExecuted(payload.providerExecuted, resultToolDef2)) break;
						...
						if (registryEntryInner?.errorProcessors?.length && canRetryErrorInner) try {
							...
							const currentMessageList = new require_message_list.MessageList();
					...
					const builtMessages = require_agent.buildMessagesFromChunks({
						chunks: collectedChunks,
						messageId: currentMessageId,
						tools: currentTools,
					...
					if (effectiveOutputProcessors.length > 0) {
						const outputStepRunner = new require_trip_wire.ProcessorRunner({
							inputProcessors: [],
							outputProcessors: effectiveOutputProcessors,
							logger,
							agentName: typedInput.agentName ?? typedInput.agentId,
					...
					if (error instanceof require_trip_wire.TripWire) throw error;
...
*/
const durableToolCallInputSchema = zod.z.object({
	toolCallId: zod.z.string(),
	toolName: zod.z.string(),
	args: zod.z.record(zod.z.string(), zod.z.any()),
	providerMetadata: zod.z.record(zod.z.string(), zod.z.any()).optional(),
...
async function processChunkThroughOutputProcessors(chunk, registryEntry, pubsub, runId, agentName, logger, messageList) {
	...
	try {
		const { part: processed, blocked, reason, tripwireOptions, processorId } = await new require_trip_wire.ProcessorRunner({
			inputProcessors: [],
			outputProcessors: registryEntry.outputProcessors,
			logger,
			agentName,
...
function createDurableToolCallStep() {
	return require_agent.createStep$1({
		id: require_agent.DurableStepIds.TOOL_CALL,
		...
		execute: async (params) => {
			...
			if (!tool) tool = require_agent.findProviderToolByName(registryEntry?.tools, toolName);
			...
			if (!tool && mastra) {
				...
				if (mastraTools) {
					tool = require_agent.findProviderToolByName(mastraTools, toolName);
			...
			if ((!tool || needsSaveQueueForFlush) && mastra) {
				...
				if (rebuilt) {
					...
					if (!tool) tool = require_agent.findProviderToolByName(rebuiltTools, toolName);
			...
			if (requiresApproval && !resumeData) {
				...
				await require_agent.stopGoalActivity({
					agentId: initData.agentId,
					runId
				});
				if (pubsub) await emitChunkEvent(pubsub, runId, {
			...
			if (bgManager && !bgConfig?.disabled && typeof cleanedArgs === "object" && cleanedArgs !== null) {
				const bgResolved = require_background_tasks.resolveBackgroundConfig({
					llmBgOverrides,
					toolName,
				...
				if (bgResolved.runInBackground) try {
					const bgTask = require_background_tasks.createBackgroundTask(bgManager, {
						toolName,
						toolCallId,
						args: cleanedArgs,
						agentId: initData.agentId,
...
*/
const durableLLMMappingInputSchema = zod.z.object({
	llmOutput: zod.z.any(),
...
*/
const durableLLMMappingOutputSchema = zod.z.object({
	messageListState: zod.z.any(),
	messageId: zod.z.string(),
	stepResult: zod.z.any(),
	toolResults: zod.z.array(zod.z.any()),
...
function createDurableLLMMappingStep() {
	return require_agent.createStep$1({
		id: require_agent.DurableStepIds.LLM_MAPPING,
...
*/
function createDurableIsTaskCompleteStep(defaultMaxSteps = require_agent.DurableAgentDefaults.MAX_STEPS) {
	return require_agent.createStep$1({
		id: require_agent.DurableStepIds.IS_TASK_COMPLETE,
		...
		execute: async (params) => {
			...
			let result;
			try {
				result = await require_agent.runStreamCompletionScorers(isTaskComplete.scorers, ctx, {
			...
			if (!result.complete) {
				const feedback = require_agent.formatStreamCompletionFeedback(result, maxIterationReached);
				messageList.add({
...
function createDurableGoalStep() {
	return require_agent.createStep$1({
		...
		execute: async (params) => {
			...
			const store = await require_task_state_processor.resolveGoalStore(mastra);
			const record = await require_task_state_processor.readObjective(store, threadId);
			if (!record || record.status !== "active" || !store || !threadId) return state;
			const effective = require_task_state_processor.resolveEffectiveGoalSettings(record, {
			...
			try {
				...
				if (!scorer) {
					const judgeModel = typeof judgeModelConfig === "string" ? await require_llm.resolveModelConfig(judgeModelConfig, requestContext, mastra) : judgeModelConfig;
					const goalTools = typeof goalConfig.tools === "function" ? await goalConfig.tools({
						requestContext,
						mastra
					}) : goalConfig.tools;
					scorer = require_agent.createGoalScorer({
						mastra,
						judgeModel,
				...
				result = await require_agent.runStreamCompletionScorers([scorer], goalContext, { strategy: "all" });
			} catch (error) {
				result = {
					complete: false,
					...
					scorers: [{
						...
						scorerId: require_task_state_processor.GOAL_SCORER_ID,
						scorerName: "Goal (LLM)",
						duration: 0,
						errored: true
					}],
			...
			await require_task_state_processor.writeObjective(store, threadId, {
				...record,
				runsUsed,
				status,
			...
			const sendSignal = require_trip_wire.createProcessorSendSignal({
				messageList,
...
const SIGNAL_DRAIN_STEP_ID = `${require_agent.DurableStepIds.AGENTIC_EXECUTION}-signal-drain`;
/**
	...
	return require_agent.createStep$1({
		...
		execute: async (params) => {
			...
			try {
				...
				const drainList = new require_message_list.MessageList();
...
*/
const durableAgenticInputSchema = zod.z.object({
...
* Extends base schema with model list for fallback support.
*/
const iterationStateSchema = baseIterationStateSchema.extend({ modelList: zod.z.array(zod.z.any()).optional() });
/**
...
function createDurableAgenticWorkflow(options) {
	const maxSteps = options?.maxSteps ?? require_agent.DurableAgentDefaults.MAX_STEPS;
	...
	const singleIterationWorkflow = require_agent.createWorkflow({
		id: require_agent.DurableStepIds.AGENTIC_EXECUTION,
		...
		options: {
			...
			pruneSnapshot: require_agent.pruneAgentLoopSnapshot,
	...
	return require_agent.createWorkflow({
		id: require_agent.DurableStepIds.AGENTIC_LOOP,
		...
		if (onIterationComplete && !state.backgroundTaskPending) {
			const lastStep = state.accumulatedSteps[state.accumulatedSteps.length - 1];
			try {
				const callbackMessageList = new require_message_list.MessageList();
				try {
		...
		if (!isFinal) {
			state.messageId = mastra?.generateId?.() ?? globalThis.crypto?.randomUUID?.() ?? `msg_${Date.now()}`;
			try {
				const boundaryList = new require_message_list.MessageList();
		...
		if (registryEntry?.outputProcessors?.length) try {
			...
			const outputMessageList = new require_message_list.MessageList();
			outputMessageList.deserialize(state.messageListState);
			await runner.runOutputProcessors(outputMessageList, require_observability.createObservabilityContext(tracingContext), requestContext ?? new require_request_context.RequestContext(), 0);
		} catch (error) {
		...
		if (registryEntry?.saveQueueManager && registryEntry.memory && durableState?.threadId && durableState?.resourceId && !durableState.observationalMemory && !durableState.memoryConfig?.readOnly) try {
			const memoryMessageList = new require_message_list.MessageList();
		...
		const logger = mastra?.getLogger?.();
		const inputMessageList = new require_message_list.MessageList();
		...
		const resolveContext = requestContext ?? new require_request_context.RequestContext();
		for (const [scorerKey, scorerEntry] of Object.entries(scorers)) {
			...
			try {
				...
				require_agent.runScorer({
					...
					threadId: initData.state?.threadId,
					resourceId: initData.state?.resourceId,
					...require_observability.createObservabilityContext(tracingContext)
				});
			} catch (error) {
...
var DurableAgent = class extends require_agent.Agent {
	/** The wrapped agent */
	#wrappedAgent;
	/** Registry for per-run non-serializable state */
	#runRegistry;
	...
	*/
	constructor(config) {
		...
		this.#innerPubsub = pubsub ?? new require_event_emitter.EventEmitterPubSub();
	...
	*/
		...
		} else if (this.#innerPubsub instanceof require_caching_pubsub.CachingPubSub) {
			this.#cachingPubsub = this.#innerPubsub;
			this.#resolvedCache = this.#cacheConfig ?? this.#mastra?.serverCache ?? null;
		} else {
			const resolvedCache = this.#cacheConfig ?? this.#mastra?.serverCache ?? new require_inmemory.InMemoryServerCache();
			this.#resolvedCache = resolvedCache;
			this.#cachingPubsub = new require_caching_pubsub.CachingPubSub(this.#innerPubsub, resolvedCache);
		}
	}
	/**
	...
	*/
	...
	async #resolveExecutionOptions(options) {
		...
		const resolvedOptions = require_utils$1.deepMerge(await this.getDefaultOptions({ requestContext: options?.requestContext }) ?? {}, options ?? {});
	...
	*/
	async executeWorkflow(runId, workflowInput) {
		...
		const result = await (await workflow.createRun({
			...
			...require_observability.createObservabilityContext({ currentSpan: entry?.agentSpan })
		});
	...
	async deleteRunSnapshots(runId) {
		try {
			...
			await (await this.#mastra?.getStorage()?.getStore("workflows"))?.deleteWorkflowRunById({
				runId,
				workflowName: require_agent.DurableStepIds.AGENTIC_EXECUTION
			});
		} catch (error) {
	...
	*/
	async stream(messages, options) {
		...
		const workflowExecution = ready.then(async () => {
			...
			if (this.__getGoalConfig()) await require_agent.beginGoalActivity({
				mastra: this.#mastra,
		...
		await require_storage.agentThreadStreamRuntime.registerRun(this, output, options, this.getPubSub());
		const cleanup = () => {
			if (autoCleanupTimer) {
	...
	async resume(runId, resumeData, options) {
		...
		if (!entry) {
			const persisted = await (await this.#mastra?.getStorage()?.getStore("workflows"))?.getWorkflowRunById({
				runId,
				workflowName: require_agent.DurableStepIds.AGENTIC_LOOP
			});
			...
			if (!workflowInput || workflowInput.__workflowKind !== "durable-agent") throw new require_error.MastraError({
			...
			if (workflowInput.agentId !== this.id) throw new require_error.MastraError({
			...
			const snapshotRequestContext = workflowInput.requestContextEntries ? new require_request_context.RequestContext(Object.entries(workflowInput.requestContextEntries)) : void 0;
		...
		if (origTraceId && this.#mastra?.observability) try {
			...
			const resumeAgentSpan = require_utils.getOrCreateSpan({
				type: "agent_run",
		...
		const workflowExecution = ready.then(async () => {
			...
			try {
				result = await run.resume({
					...
					requestContext,
					actor: resolvedOptions.actor,
					...require_observability.createObservabilityContext({ currentSpan: entry.resumeAgentSpan ?? entry.agentSpan })
				});
			} finally {
				await require_agent.stopGoalActivity({
					agentId: this.id,
		...
		await require_storage.agentThreadStreamRuntime.registerRun(this, output, resumeStreamOptions, this.getPubSub());
		const cleanup = () => {
			if (autoCleanupTimer) {
				clearTimeout(autoCleanupTimer);
				autoCleanupTimer = null;
	...
	*/
	async recover(runId, options) {
		if (!this.#mastra) throw new require_error.MastraError({
		...
		if (!workflowsStore) throw new require_error.MastraError({
		...
		if (!persisted) throw new require_error.MastraError({
		...
		const requestContext = workflowInput.requestContextEntries ? new require_request_context.RequestContext(Object.entries(workflowInput.requestContextEntries)) : new require_request_context.RequestContext();
		const messageListMemoryInfo = workflowInput.messageListState?.memoryInfo;
		const threadId = workflowInput.state?.threadId ?? messageListMemoryInfo?.threadId;
		const resourceId = workflowInput.state?.resourceId ?? messageListMemoryInfo?.resourceId;
		const messageList = new require_message_list.MessageList({
		...
		if (this.#mastra?.observability) try {
			...
			recoverAgentSpan = require_utils.getOrCreateSpan({
				type: "agent_run",
		...
		const workflowExecution = ready.then(async () => {
			try {
				const result = await (await workflow.createRun({
					...
					requestContext,
					...require_observability.createObservabilityContext({ currentSpan: recoverAgentSpan })
				});
				if (result?.status && result.status !== "suspended") await this.deleteRunSnapshots(runId);
				if (result?.status === "failed") {
	...
	*/
	async listActiveRuns(options = {}) {
		...
		if (perPage !== void 0 && (!Number.isInteger(perPage) || perPage <= 0)) throw new require_error.MastraError({
		...
		if (page !== void 0 && (!Number.isInteger(page) || page < 0)) throw new require_error.MastraError({
	...
	*/
	#clearPubsubTopic(runId) {
		this.pubsub.clearTopic(require_agent.AGENT_STREAM_TOPIC(runId));
	}
	/**
	...
	*/
	async #getPubsubOffset(runId) {
		...
		try {
			const history = await pubsub.getHistory(require_agent.AGENT_STREAM_TOPIC(runId));
...
var create_durable_agent_exports = /* @__PURE__ */ require_rolldown_runtime.__exportAll({
...
//#endregion
Object.defineProperty(exports, "DurableAgent", {
	enumerable: true,
	get: function() {
		return DurableAgent;
	}
});
Object.defineProperty(exports, "ExtendedRunRegistry", {
	enumerable: true,
	get: function() {
		return ExtendedRunRegistry;
	}
});
Object.defineProperty(exports, "RunRegistry", {
	enumerable: true,
	get: function() {
		return RunRegistry;
	}
});
Object.defineProperty(exports, "accumulatedUsageSchema", {
	enumerable: true,
	get: function() {
		return accumulatedUsageSchema;
	}
});
Object.defineProperty(exports, "baseDurableAgenticInputSchema", {
	enumerable: true,
	get: function() {
		return baseDurableAgenticInputSchema;
	}
});
Object.defineProperty(exports, "baseIterationStateSchema", {
	enumerable: true,
	get: function() {
		return baseIterationStateSchema;
	}
});
Object.defineProperty(exports, "buildStepRecord", {
	enumerable: true,
	get: function() {
		return buildStepRecord;
	}
});
Object.defineProperty(exports, "calculateAccumulatedUsage", {
	enumerable: true,
	get: function() {
		return calculateAccumulatedUsage;
	}
});
Object.defineProperty(exports, "createBaseIterationStateUpdate", {
	enumerable: true,
	get: function() {
		return createBaseIterationStateUpdate;
	}
});
Object.defineProperty(exports, "createDurableAgent", {
	enumerable: true,
	get: function() {
		return createDurableAgent;
	}
});
Object.defineProperty(exports, "createDurableAgentStream", {
	enumerable: true,
	get: function() {
		return createDurableAgentStream;
	}
});
Object.defineProperty(exports, "createDurableAgenticWorkflow", {
	enumerable: true,
	get: function() {
		return createDurableAgenticWorkflow;
	}
});
Object.defineProperty(exports, "createDurableBackgroundTaskCheckStep", {
	enumerable: true,
	get: function() {
		return createDurableBackgroundTaskCheckStep;
	}
});
Object.defineProperty(exports, "createDurableLLMExecutionStep", {
	enumerable: true,
	get: function() {
		return createDurableLLMExecutionStep;
	}
});
Object.defineProperty(exports, "createDurableLLMMappingStep", {
	enumerable: true,
	get: function() {
		return createDurableLLMMappingStep;
	}
});
Object.defineProperty(exports, "createDurableToolCallStep", {
	enumerable: true,
	get: function() {
		return createDurableToolCallStep;
	}
});
Object.defineProperty(exports, "createWorkflowInput", {
	enumerable: true,
	get: function() {
		return createWorkflowInput;
	}
});
Object.defineProperty(exports, "create_durable_agent_exports", {
	enumerable: true,
	get: function() {
		return create_durable_agent_exports;
	}
});
Object.defineProperty(exports, "durableAgenticOutputSchema", {
	enumerable: true,
	get: function() {
		return durableAgenticOutputSchema;
	}
});
Object.defineProperty(exports, "emitChunkEvent", {
	enumerable: true,
	get: function() {
		return emitChunkEvent;
	}
});
Object.defineProperty(exports, "emitErrorEvent", {
	enumerable: true,
	get: function() {
		return emitErrorEvent;
	}
});
Object.defineProperty(exports, "emitFinishEvent", {
	enumerable: true,
	get: function() {
		return emitFinishEvent;
	}
});
Object.defineProperty(exports, "emitStepFinishEvent", {
	enumerable: true,
	get: function() {
		return emitStepFinishEvent;
	}
});
Object.defineProperty(exports, "emitStepStartEvent", {
	enumerable: true,
	get: function() {
		return emitStepStartEvent;
	}
});
Object.defineProperty(exports, "emitSuspendedEvent", {
	enumerable: true,
	get: function() {
		return emitSuspendedEvent;
	}
});
Object.defineProperty(exports, "globalRunRegistry", {
	enumerable: true,
	get: function() {
		return globalRunRegistry;
	}
});
Object.defineProperty(exports, "isDurableAgent", {
	enumerable: true,
	get: function() {
		return isDurableAgent;
	}
});
Object.defineProperty(exports, "isLocalDurableAgent", {
	enumerable: true,
	get: function() {
		return isLocalDurableAgent;
	}
});
...
Object.defineProperty(exports, "modelListEntrySchema", {
	enumerable: true,
	get: function() {
		return modelListEntrySchema;
	}
});
Object.defineProperty(exports, "prepareForDurableExecution", {
	enumerable: true,
	get: function() {
		return prepareForDurableExecution;
	}
});
Object.defineProperty(exports, "rebuildRunToolsFromMastra", {
	enumerable: true,
	get: function() {
		return rebuildRunToolsFromMastra;
	}
});
Object.defineProperty(exports, "resolveDurableToolCallConcurrency", {
	enumerable: true,
	get: function() {
		return resolveDurableToolCallConcurrency;
	}
});
Object.defineProperty(exports, "resolveInternalState", {
	enumerable: true,
	get: function() {
		return resolveInternalState;
	}
});
...
Object.defineProperty(exports, "resolveRuntimeDependencies", {
	enumerable: true,
	get: function() {
		return resolveRuntimeDependencies;
	}
});
Object.defineProperty(exports, "resolveTool", {
	enumerable: true,
	get: function() {
		return resolveTool;
	}
});
Object.defineProperty(exports, "runDurableStreamUntilIdle", {
	enumerable: true,
	get: function() {
		return runDurableStreamUntilIdle;
	}
});
Object.defineProperty(exports, "runResumeDurableStreamUntilIdle", {
	enumerable: true,
	get: function() {
		return runResumeDurableStreamUntilIdle;
	}
});
Object.defineProperty(exports, "serializeDurableOptions", {
	enumerable: true,
	get: function() {
		return serializeDurableOptions;
	}
});
Object.defineProperty(exports, "serializeDurableState", {
	enumerable: true,
	get: function() {
		return serializeDurableState;
	}
});
Object.defineProperty(exports, "serializeModelConfig", {
	enumerable: true,
	get: function() {
		return serializeModelConfig;
	}
});
Object.defineProperty(exports, "serializeToolsMetadata", {
	enumerable: true,
	get: function() {
		return serializeToolsMetadata;
	}
});
Object.defineProperty(exports, "toolRequiresApproval", {
	enumerable: true,
	get: function() {
...
//# sourceMappingURL=create-durable-agent-CB4JGA_3.cjs.map