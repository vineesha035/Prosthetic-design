...
//#region src/workflows/scheduler/cron.ts
/**
...
function validateCron(cron, timezone) {
	if (typeof cron !== "string" || cron.trim() === "") throw new Error(`Invalid cron expression: expected a non-empty cron string (e.g. "0 * * * *"), but received ${cron === void 0 ? "undefined" : JSON.stringify(cron)}.`);
	let job;
	try {
		job = new Cron(cron);
	} catch (error) {
...
function computeNextFireAt(cron, options) {
	...
	if (!next) throw new Error(`Cron expression "${cron}" has no future occurrence after ${reference.toISOString()}`);
	return next.getTime();
}
//#endregion
export { validateCron as n, computeNextFireAt as t };

//# sourceMappingURL=cron-B2j814dd.js.map