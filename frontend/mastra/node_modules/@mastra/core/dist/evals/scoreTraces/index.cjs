...
const getTraceStep = require_agent.createStep({
	id: "__process-trace-scoring",
	inputSchema: zod_v4.z.object({
		targets: zod_v4.z.array(zod_v4.z.object({
			traceId: zod_v4.z.string(),
			spanId: zod_v4.z.string().optional()
		})),
		scorerId: zod_v4.z.string()
	}),
	...
	execute: async ({ inputData, mastra }) => {
		...
		try {
			...
			const mastraError = new require_error.MastraError({
				...
				domain: require_error.ErrorDomain.SCORER,
				category: require_error.ErrorCategory.SYSTEM,
		...
		await (0, p_map.default)(inputData.targets, async (target) => {
			try {
				await scoreTrace({
					storage,
					scorer,
...
async function resolveTraceAndSpan({ storage, target }) {
	...
	if (!observabilityStore) throw new require_error.MastraError({
...
async function runScorerForTrace({ scorer, trace, span }) {
	...
	return scorer.run({
		...
		scoreSource: "trace",
		targetScope: "span",
		targetEntityType: require_utils.getEntityTypeForSpan(span),
...
async function scoreTraceBatch({ storage, scorer, targets, batchId, datasetId, concurrency = 3 }) {
	const results = await (0, p_map.default)(targets, async (target, index) => {
		try {
...
async function validateAndSaveScore({ storage, scorerResult }) {
	...
	if (!scoresStore) throw new require_error.MastraError({
		id: "MASTRA_SCORES_STORAGE_NOT_AVAILABLE",
		domain: require_error.ErrorDomain.STORAGE,
		category: require_error.ErrorCategory.SYSTEM,
		text: "Scores storage domain is not available"
	});
	const payloadToSave = require_constants.saveScorePayloadSchema.parse(scorerResult);
...
const scoreTracesWorkflow = require_agent.createWorkflow$1({
...
//#endregion
exports.scoreTrace = scoreTrace;
exports.scoreTraceBatch = scoreTraceBatch;
