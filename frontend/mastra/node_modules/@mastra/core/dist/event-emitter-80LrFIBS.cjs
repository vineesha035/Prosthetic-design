...
//#endregion
//#region src/events/event-emitter/batch-policy.ts
const defaultDeps = {
	now: () => Date.now(),
...
var BatchPolicy = class {
	...
	constructor(opts, deps = defaultDeps) {
		this.opts = opts;
		this.deps = deps;
		this.maxBufferSize = opts.maxBufferSize ?? 256;
		this.overflow = opts.overflow ?? "coalesce-or-drop-oldest";
	}
	...
	*/
	onEnqueue(event) {
		this.size += 1;
		if (this.firstQueuedAt === null) this.firstQueuedAt = this.deps.now();
		const now = this.deps.now();
		const intervalFloor = this.lastDeliveredAt + (this.opts.minIntervalMs ?? 0);
		if (this.opts.isImmediate?.(event)) {
			if (now >= intervalFloor) return "flush-now";
			this.scheduleAt(intervalFloor);
			return "wait";
		}
		if (this.size >= this.maxBufferSize) return "flush-now";
	...
	*/
	prepareBatch(events) {
		...
		if (this.opts.coalesce) {
			...
			working = coalesced.every((e) => inputRefs.has(e)) ? coalesced : [];
		}
		const keptRefs = new Set(working);
		const computeDropped = () => events.filter((e) => !keptRefs.has(e));
		if (working.length <= this.maxBufferSize) return {
			delivered: working,
			dropped: computeDropped()
		};
		...
		switch (this.overflow) {
			case "drop-newest": {
				const splitFromEnd = this.takeWithoutDropping(working, overBy, isImmediate, true);
				kept = splitFromEnd.kept;
				droppedFromOverflow = splitFromEnd.dropped;
				break;
			}
			default: {
				const splitFromStart = this.takeWithoutDropping(working, overBy, isImmediate, false);
		...
		return {
			delivered: kept,
			dropped: [...computeDropped(), ...droppedFromOverflow]
		};
	}
	...
	scheduleDeadline() {
		if (this.opts.maxWaitMs === void 0 && this.opts.minIntervalMs === void 0) return;
	...
	*/
	takeWithoutDropping(items, count, isImmediate, fromEnd) {
		...
		return {
			kept: result.filter((e) => e !== void 0),
			dropped
		};
	}
};
//#endregion
//#region src/events/event-emitter/ack-handle-buffer.ts
/**
...
var AckHandleBuffer = class {
	...
	constructor(cb, opts, deps, onError) {
		...
		this.policy.bindFlushHandler(() => this.flush().catch((err) => {
	...
	*/
	async push(event, ack, nack) {
		...
		if (this.policy.onEnqueue(event) === "flush-now") await this.flush();
	}
	/**
...
//#endregion
//#region src/events/event-emitter/index.ts
const NOOP_ACK = async () => {};
var EventEmitterPubSub = class extends PubSub {
	...
	*/
	...
	constructor(existingEmitter, options = {}) {
		super();
		this.emitter = existingEmitter ?? new events.default();
	...
	*/
	...
	async subscribe(topic, cb, options) {
		if (options?.batch) {
			...
			byCb.set(cb, buffer);
			if (options.group) this.subscribeWithGroup(topic, cb, options.group);
			else {
				const wrapper = (event) => {
					buffer.push(event, NOOP_ACK, NOOP_ACK).catch((err) => {
	...
	async unsubscribe(topic, cb) {
		...
		for (const [group, topicMap] of this.groups) {
			...
			if (members) {
				...
				if (idx !== -1) {
					...
					if (topicMap.size === 0) this.groups.delete(group);
					return;
				}
			}
		}
		...
		} else this.emitter.off(topic, cb);
	}
	async flush() {
		while (true) {
			const drains = [];
			for (const [topic, byCb] of this.batchBuffers.entries()) for (const buffer of byCb.values()) drains.push({
				topic,
				promise: buffer.flush()
			});
			if (drains.length > 0) {
				const results = await Promise.allSettled(drains.map((d) => d.promise));
				for (let i = 0; i < results.length; i++) {
					const result = results[i];
					if (result.status === "rejected") this.logBufferError(drains[i].topic, result.reason, { phase: "cb" });
				}
			}
			if (this.pendingNacks.size === 0) return;
			await new Promise((resolve) => {
				const check = () => {
					if (this.pendingNacks.size === 0) resolve();
	...
	*/
	async close() {
		for (const handle of this.pendingNacks) clearTimeout(handle);
		this.pendingNacks.clear();
		this.deliveryAttempts.clear();
		for (const byCb of this.batchBuffers.values()) for (const buffer of byCb.values()) buffer.dispose();
	...
	deliverToGroup(topic, group, listenerKey, event) {
		...
		if (buffer) buffer.push(eventWithAttempt, ack, nack).catch((err) => {
			this.logBufferError(topic, err, { phase: "cb" });
		});
		else member(eventWithAttempt, ack, nack);
	}
	leases = /* @__PURE__ */ new Map();
	acquireLease(key, owner, ttlMs) {
		const now = Date.now();
		const existing = this.leases.get(key);
		if (existing && existing.expiresAt > now && existing.owner !== owner) return Promise.resolve({
			acquired: false,
	...
	releaseLease(key, owner) {
		const existing = this.leases.get(key);
		if (existing && existing.owner === owner) this.leases.delete(key);
		return Promise.resolve();
	}
	renewLease(key, owner, ttlMs) {
		const existing = this.leases.get(key);
		if (!existing || existing.owner !== owner || existing.expiresAt <= Date.now()) return Promise.resolve(false);
		existing.expiresAt = Date.now() + ttlMs;
		return Promise.resolve(true);
	}
	transferLease(key, fromOwner, toOwner, ttlMs) {
		const existing = this.leases.get(key);
		if (!existing || existing.owner !== fromOwner || existing.expiresAt <= Date.now()) return Promise.resolve(false);
		this.leases.set(key, {
			owner: toOwner,
...
Object.defineProperty(exports, "isLeaseProvider", {
	enumerable: true,
	get: function() {
		return isLeaseProvider;
	}
});

//# sourceMappingURL=event-emitter-80LrFIBS.cjs.map