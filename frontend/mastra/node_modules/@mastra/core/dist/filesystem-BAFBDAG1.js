...
//#region src/storage/domains/mcp-servers/base.ts
...
//#endregion
//#region src/storage/domains/mcp-servers/inmemory.ts
var InMemoryMCPServersStorage = class extends MCPServersStorage {
	...
	async create(input) {
		...
		if (this.db.mcpServers.has(mcpServer.id)) throw new Error(`MCP server with id ${mcpServer.id} already exists`);
	...
	async update(input) {
		...
		if (!existingConfig) throw new Error(`MCP server with id ${id} not found`);
	...
	async createVersion(input) {
		if (this.db.mcpServerVersions.has(input.id)) throw new Error(`Version with id ${input.id} already exists`);
		for (const version of this.db.mcpServerVersions.values()) if (version.mcpServerId === input.mcpServerId && version.versionNumber === input.versionNumber) throw new Error(`Version number ${input.versionNumber} already exists for MCP server ${input.mcpServerId}`);
		const version = {
			...input,
	...
	async getVersionByNumber(mcpServerId, versionNumber) {
		for (const version of this.db.mcpServerVersions.values()) if (version.mcpServerId === mcpServerId && version.versionNumber === versionNumber) return this.deepCopyVersion(version);
		return null;
	}
	async getLatestVersion(mcpServerId) {
		let latest = null;
		for (const version of this.db.mcpServerVersions.values()) if (version.mcpServerId === mcpServerId) {
	...
	async deleteVersionsByParentId(entityId) {
		const idsToDelete = [];
		for (const [id, version] of this.db.mcpServerVersions.entries()) if (version.mcpServerId === entityId) idsToDelete.push(id);
		for (const id of idsToDelete) this.db.mcpServerVersions.delete(id);
	}
	async countVersions(mcpServerId) {
		let count = 0;
		for (const version of this.db.mcpServerVersions.values()) if (version.mcpServerId === mcpServerId) count++;
		return count;
	}
...
//#endregion
//#region src/storage/domains/mcp-servers/filesystem.ts
var FilesystemMCPServersStorage = class extends MCPServersStorage {
	...
	async listVersions(input) {
		return await this.helpers.listVersions(input, "mcpServerId");
	}
	async deleteVersion(id) {
...
//#endregion
export { InMemoryMCPServersStorage as n, MCPServersStorage as r, FilesystemMCPServersStorage as t };

//# sourceMappingURL=filesystem-BAFBDAG1.js.map