...
//#region src/storage/domains/prompt-blocks/base.ts
var PromptBlocksStorage = class extends require_filesystem_versioned.VersionedStorageDomain {
...
//#endregion
//#region src/storage/domains/prompt-blocks/inmemory.ts
var InMemoryPromptBlocksStorage = class extends PromptBlocksStorage {
	...
	async create(input) {
		...
		if (this.db.promptBlocks.has(promptBlock.id)) throw new Error(`Prompt block with id ${promptBlock.id} already exists`);
	...
	async update(input) {
		...
		if (!existingBlock) throw new Error(`Prompt block with id ${id} not found`);
	...
	async list(args) {
		...
		if (status) blocks = blocks.filter((block) => block.status === status);
		if (authorId !== void 0) blocks = blocks.filter((block) => block.authorId === authorId);
		if (metadata && Object.keys(metadata).length > 0) blocks = blocks.filter((block) => {
			if (!block.metadata) return false;
			return Object.entries(metadata).every(([key, value]) => require_deep_equal.deepEqual(block.metadata[key], value));
		});
		const clonedBlocks = this.sortBlocks(blocks, field, direction).map((block) => this.deepCopyBlock(block));
	...
	async createVersion(input) {
		if (this.db.promptBlockVersions.has(input.id)) throw new Error(`Version with id ${input.id} already exists`);
		for (const version of this.db.promptBlockVersions.values()) if (version.blockId === input.blockId && version.versionNumber === input.versionNumber) throw new Error(`Version number ${input.versionNumber} already exists for prompt block ${input.blockId}`);
		const version = {
			...input,
	...
	async getVersionByNumber(blockId, versionNumber) {
		for (const version of this.db.promptBlockVersions.values()) if (version.blockId === blockId && version.versionNumber === versionNumber) return this.deepCopyVersion(version);
		return null;
	}
	async getLatestVersion(blockId) {
		let latest = null;
		for (const version of this.db.promptBlockVersions.values()) if (version.blockId === blockId) {
	...
	async deleteVersionsByParentId(entityId) {
		const idsToDelete = [];
		for (const [id, version] of this.db.promptBlockVersions.entries()) if (version.blockId === entityId) idsToDelete.push(id);
		for (const id of idsToDelete) this.db.promptBlockVersions.delete(id);
	}
	async countVersions(blockId) {
		let count = 0;
		for (const version of this.db.promptBlockVersions.values()) if (version.blockId === blockId) count++;
		return count;
	}
	deepCopyBlock(block) {
		return {
...
//#endregion
//#region src/storage/domains/prompt-blocks/filesystem.ts
var FilesystemPromptBlocksStorage = class extends PromptBlocksStorage {
	...
	async listVersions(input) {
		return await this.helpers.listVersions(input, "blockId");
	}
	async deleteVersion(id) {
...
Object.defineProperty(exports, "PromptBlocksStorage", {
	enumerable: true,
	get: function() {
		return PromptBlocksStorage;
	}
});

//# sourceMappingURL=filesystem-CceZwSX7.cjs.map