...
//#region src/storage/domains/scorer-definitions/base.ts
var ScorerDefinitionsStorage = class extends require_filesystem_versioned.VersionedStorageDomain {
...
//#endregion
//#region src/storage/domains/scorer-definitions/inmemory.ts
var InMemoryScorerDefinitionsStorage = class extends ScorerDefinitionsStorage {
	...
	async create(input) {
		...
		if (this.db.scorerDefinitions.has(scorerDefinition.id)) throw new Error(`Scorer definition with id ${scorerDefinition.id} already exists`);
		...
		const { id: _id, authorId: _authorId, organizationId: _organizationId, projectId: _projectId, metadata: _metadata, ...snapshotConfig } = scorerDefinition;
	...
	async update(input) {
		...
		if (!existingScorer) throw new Error(`Scorer definition with id ${id} not found`);
	...
	async list(args) {
		const { page = 0, perPage: perPageInput, orderBy, authorId, organizationId, projectId, metadata, status } = args || {};
		...
		if (status) scorers = scorers.filter((scorer) => scorer.status === status);
		if (authorId !== void 0) scorers = scorers.filter((scorer) => scorer.authorId === authorId);
		if (organizationId !== void 0) scorers = scorers.filter((scorer) => scorer.organizationId === organizationId);
		if (projectId !== void 0) scorers = scorers.filter((scorer) => scorer.projectId === projectId);
		if (metadata && Object.keys(metadata).length > 0) scorers = scorers.filter((scorer) => {
			if (!scorer.metadata) return false;
			return Object.entries(metadata).every(([key, value]) => require_deep_equal.deepEqual(scorer.metadata[key], value));
		});
		const clonedScorers = this.sortScorers(scorers, field, direction).map((scorer) => this.deepCopyScorer(scorer));
	...
	async createVersion(input) {
		if (this.db.scorerDefinitionVersions.has(input.id)) throw new Error(`Version with id ${input.id} already exists`);
		for (const version of this.db.scorerDefinitionVersions.values()) if (version.scorerDefinitionId === input.scorerDefinitionId && version.versionNumber === input.versionNumber) throw new Error(`Version number ${input.versionNumber} already exists for scorer definition ${input.scorerDefinitionId}`);
		const version = {
			...input,
	...
	async getVersionByNumber(scorerDefinitionId, versionNumber) {
		for (const version of this.db.scorerDefinitionVersions.values()) if (version.scorerDefinitionId === scorerDefinitionId && version.versionNumber === versionNumber) return this.deepCopyVersion(version);
		return null;
	}
	async getLatestVersion(scorerDefinitionId) {
		let latest = null;
		for (const version of this.db.scorerDefinitionVersions.values()) if (version.scorerDefinitionId === scorerDefinitionId) {
	...
	async listVersions(input) {
		...
		const maxOffset = Number.MAX_SAFE_INTEGER / 2;
		if (page * perPage > maxOffset) throw new Error("page value too large");
		let versions = Array.from(this.db.scorerDefinitionVersions.values()).filter((v) => v.scorerDefinitionId === scorerDefinitionId);
	...
	async deleteVersionsByParentId(entityId) {
		const idsToDelete = [];
		for (const [id, version] of this.db.scorerDefinitionVersions.entries()) if (version.scorerDefinitionId === entityId) idsToDelete.push(id);
		for (const id of idsToDelete) this.db.scorerDefinitionVersions.delete(id);
	}
	async countVersions(scorerDefinitionId) {
		let count = 0;
		for (const version of this.db.scorerDefinitionVersions.values()) if (version.scorerDefinitionId === scorerDefinitionId) count++;
		return count;
	}
...
//#endregion
//#region src/storage/domains/scorer-definitions/filesystem.ts
var FilesystemScorerDefinitionsStorage = class extends ScorerDefinitionsStorage {
	...
	async listVersions(input) {
		return await this.helpers.listVersions(input, "scorerDefinitionId");
	}
	async deleteVersion(id) {
...
Object.defineProperty(exports, "ScorerDefinitionsStorage", {
	enumerable: true,
	get: function() {
		return ScorerDefinitionsStorage;
	}
});

//# sourceMappingURL=filesystem-CpAldLfc.cjs.map