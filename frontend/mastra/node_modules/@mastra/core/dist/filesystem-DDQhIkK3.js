import { d as normalizePerPage, o as VersionedStorageDomain, t as FilesystemVersionedHelpers, u as calculatePagination } from "./filesystem-versioned-M7zmYTpZ.js";
import { randomUUID } from "crypto";
//#region src/storage/domains/skills/base.ts
...
//#endregion
//#region src/storage/domains/skills/skill-snapshot-field-equal.ts
function skillSnapshotFieldValuesEqual(a, b) {
	...
	const objectA = a;
	const objectB = b;
	return Array.from(/* @__PURE__ */ new Set([...Object.keys(objectA), ...Object.keys(objectB)])).filter((key) => !(objectA[key] == null && objectB[key] == null)).sort().every((key) => skillSnapshotFieldValuesEqual(objectA[key], objectB[key]));
}
//#endregion
//#region src/storage/domains/skills/inmemory.ts
var InMemorySkillsStorage = class extends SkillsStorage {
	...
	async create(input) {
		const { skill } = input;
		if (this.db.skills.has(skill.id)) throw new Error(`Skill with id ${skill.id} already exists`);
	...
	async update(input) {
		...
		if (!existingConfig) throw new Error(`Skill with id ${id} not found`);
		...
		if (hasConfigUpdate) {
			...
			if (!latestVersion) throw new Error(`No versions found for skill ${id}`);
			...
			const changedFields = configFieldNames.filter((field) => field in configFields && !skillSnapshotFieldValuesEqual(configFields[field], latestConfig[field]));
	...
	async list(args) {
		const { page = 0, perPage: perPageInput, orderBy, authorId, status, visibility, metadata, entityIds, pinFavoritedFor, favoritedOnly } = args || {};
		...
		if (status !== void 0) configs = configs.filter((config) => config.status === status);
		if (visibility !== void 0) configs = configs.filter((config) => config.visibility === visibility);
		if (metadata && Object.keys(metadata).length > 0) configs = configs.filter((_config) => {
			return false;
		});
		const favoritedIds = pinFavoritedFor ? this.collectFavoritedIdsFor(pinFavoritedFor) : void 0;
		if (favoritedOnly) if (favoritedIds) configs = configs.filter((config) => favoritedIds.has(config.id));
		else configs = [];
		const clonedConfigs = this.sortConfigs(configs, field, direction, favoritedIds).map((config) => this.deepCopyConfig(config));
	...
	async createVersion(input) {
		if (this.db.skillVersions.has(input.id)) throw new Error(`Version with id ${input.id} already exists`);
		for (const version of this.db.skillVersions.values()) if (version.skillId === input.skillId && version.versionNumber === input.versionNumber) throw new Error(`Version number ${input.versionNumber} already exists for skill ${input.skillId}`);
		const version = {
			...input,
	...
	async getVersionByNumber(skillId, versionNumber) {
		for (const version of this.db.skillVersions.values()) if (version.skillId === skillId && version.versionNumber === versionNumber) return this.deepCopyVersion(version);
		return null;
	}
	async getLatestVersion(skillId) {
		let latest = null;
		for (const version of this.db.skillVersions.values()) if (version.skillId === skillId) {
	...
	async deleteVersionsByParentId(entityId) {
		const idsToDelete = [];
		for (const [id, version] of this.db.skillVersions.entries()) if (version.skillId === entityId) idsToDelete.push(id);
		for (const id of idsToDelete) this.db.skillVersions.delete(id);
	}
	async countVersions(skillId) {
		let count = 0;
		for (const version of this.db.skillVersions.values()) if (version.skillId === skillId) count++;
		return count;
	}
	...
	*/
	collectFavoritedIdsFor(userId) {
		...
		for (const row of this.db.favorites.values()) if (row.userId === userId && row.entityType === "skill") favorited.add(row.entityId);
		return favorited;
	}
...
//#endregion
//#region src/storage/domains/skills/filesystem.ts
var FilesystemSkillsStorage = class extends SkillsStorage {
	...
	async update(input) {
		...
		if (!existing) throw new Error(`FilesystemSkillsStorage: skill with id ${id} not found`);
		const { authorId, visibility, activeVersionId, status, ...rawConfigFields } = updates;
		const configFields = {};
		for (const [key, value] of Object.entries(rawConfigFields)) if (value !== void 0) configFields[key] = value;
		const configFieldNames = [
		...
		if (activeVersionId !== void 0 && status === void 0) updatedEntity.status = "published";
		if (hasConfigUpdate) {
			const latestVersion = await this.getLatestVersion(id);
			if (!latestVersion) throw new Error(`No versions found for skill ${id}`);
			const { id: _versionId, skillId: _skillId, versionNumber: _versionNumber, changedFields: _changedFields, changeMessage: _changeMessage, createdAt: _createdAt, ...latestConfig } = latestVersion;
		...
		if (activeVersionId !== void 0 && status === void 0) entityUpdates.status = "published";
	...
	async listVersions(input) {
		return await this.helpers.listVersions(input, "skillId");
	}
	async deleteVersion(id) {
...
//#endregion
export { SkillsStorage as i, InMemorySkillsStorage as n, skillSnapshotFieldValuesEqual as r, FilesystemSkillsStorage as t };

//# sourceMappingURL=filesystem-DDQhIkK3.js.map