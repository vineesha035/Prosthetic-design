...
//#region src/storage/domains/thread-state/base.ts
/**
...
//#endregion
//#region src/storage/domains/thread-state/inmemory.ts
...
function normalizePerPage(perPageInput, defaultValue) {
	...
	else if (typeof perPageInput === "number" && perPageInput > 0) return perPageInput;
	else if (typeof perPageInput === "number" && perPageInput < 0) throw new Error("perPage must be >= 0");
	return defaultValue;
}
/**
...
function isPruneCapable(value) {
	return typeof value === "object" && value !== null && typeof value.prune === "function";
}
	...
	*/
	...
	constructor(config) {
		...
		if (config.default || config.editor || config.domains) {
			...
			const hasOverrideDomains = Object.values(domainOverrides).some((v) => v !== void 0 && v !== false);
			if (!hasDefaultDomains && !hasEditorDomains && !hasOverrideDomains) throw new Error("MastraCompositeStore requires at least one storage source. Provide a default storage, an editor storage, or domain overrides.");
			...
			this.stores = {
				...
				threadState: domainOverrides.threadState === false ? void 0 : resolve("threadState") ?? new InMemoryThreadStateStorage()
			};
		}
	}
	/**
	...
	*/
	__registerMastra(mastra, seen = /* @__PURE__ */ new Set()) {
		...
		const cascade = (target) => {
			...
			if (typeof fn === "function") fn.call(target, mastra, seen);
			else seen.add(target);
		};
		if (this.parentDefault) cascade(this.parentDefault);
		if (this.parentEditor) cascade(this.parentEditor);
		if (this.stores) for (const domain of Object.values(this.stores)) cascade(domain);
	}
	/**
	...
	*/
	async init() {
		...
		const initPromise = this.#runInit().catch((error) => {
			if (this.hasInitialized === initPromise) this.hasInitialized = null;
			throw error;
		});
	...
	async #runInit() {
		...
		const addParentDomains = (parent) => {
			...
			for (const domain of Object.values(parent.stores)) if (domain) alreadyInitialized.add(domain);
		};
...
//#endregion
//#region src/storage/domains/versioned.ts
const ENTITY_ORDER_BY_SET = {
	createdAt: true,
	updatedAt: true
};
const SORT_DIRECTION_SET = {
	ASC: true,
	DESC: true
};
const VERSION_ORDER_BY_SET = {
	versionNumber: true,
	createdAt: true
};
/**
...
var VersionedStorageDomain = class extends StorageDomain {
	...
	*/
	extractSnapshotConfig(version) {
		...
		for (const [key, value] of Object.entries(version)) if (!metadataSet.has(key)) result[key] = value;
		return result;
	}
	/**
	...
	*/
	async listResolved(args) {
		...
		const resolved = await Promise.all(entities.map((entity) => this.resolveEntity(entity, { status })));
		return {
			...result,
	...
	*/
	async resolveEntity(entity, options) {
		...
		let version = null;
		if (options?.versionId) version = await this.getVersion(options.versionId);
		else if (status === "draft") version = await this.getLatestVersion(entity.id);
		else {
			if (entity.activeVersionId) {
				version = await this.getVersion(entity.activeVersionId);
				if (!version) this.logger?.warn?.(`Entity ${entity.id} has activeVersionId ${entity.activeVersionId} but version not found. Falling back to latest version.`);
			}
			if (!version) version = await this.getLatestVersion(entity.id);
		}
		if (version) {
...
//#endregion
//#region src/storage/git-history.ts
/**
...
var GitHistory = class {
	...
	*/
	async getFileHistory(dir, filename, limit = 50) {
		...
		if (this.commitCache.has(cacheKey)) return this.commitCache.get(cacheKey);
	...
	*/
	async getFileAtCommit(dir, commitHash, filename) {
		...
		if (this.snapshotCache.has(cacheKey)) return this.snapshotCache.get(cacheKey);
	...
	*/
	relativeToRepo(dir, filename) {
		...
		const relDir = relative(realpathSync(root), realpathSync(dir));
...
//#endregion
//#region src/storage/source-control.ts
const SOURCE_CONTROL_AGENTS_DIR = "agents";
...
//#endregion
//#region src/storage/filesystem-versioned.ts
/**
...
function stableSortKeys(value) {
	...
	if (value && typeof value === "object" && !(value instanceof Date)) return Object.fromEntries(Object.entries(value).filter(([, entry]) => entry !== void 0).sort(([left], [right]) => left.localeCompare(right)).map(([key, entry]) => [key, stableSortKeys(entry)]));
	return value;
}
/**
...
* ```
*/
var FilesystemVersionedHelpers = class FilesystemVersionedHelpers {
	db;
	entitiesFile;
	parentIdField;
	name;
	...
	*/
	...
	perEntityFilename(entityId) {
		if (!this.perEntityFilesDir) throw new Error(`${this.name}: per-entity files directory is not configured`);
	...
	*/
	hydrate() {
		...
		if (this.perEntityFilesDir) for (const filename of this.db.listDomainFiles(this.perEntityFilesDir)) {
	...
	*/
	async ensureGitHistory() {
		this.hydrate();
		if (this.gitHistoryPromise) await this.gitHistoryPromise;
	}
	/**
	...
	*/
	async loadGitHistory() {
		...
		if (!await git.isGitRepo(dir)) return;
		const orderedCommits = [...await git.getFileHistory(dir, this.entitiesFile, this.gitHistoryLimit)].reverse();
		...
		for (let i = 0; i < orderedCommits.length; i++) {
			...
			const fileContent = await git.getFileAtCommit(dir, commit.hash, this.entitiesFile);
		...
		if (this.perEntityFilesDir) {
			...
			for (const filename of this.db.listDomainFiles(this.perEntityFilesDir)) perEntityIds.add(this.entityIdFromPerEntityFilename(filename));
			for (const entityId of this.entities.keys()) perEntityIds.add(entityId);
		...
		for (const [entityId, gitCount] of entityVersionCount) {
			...
			if (version) version.versionNumber = gitCount + 1;
		}
	}
	/**
	...
	*/
	persistToDisk() {
		...
		for (const [entityId, entity] of this.entities) {
			...
			} else diskData[entityId] = stableSortKeys(snapshotConfig);
		}
		const hasSharedEntries = Object.keys(diskData).length > 0;
		const sharedFileExists = this.db.domainFileExists(this.entitiesFile);
		if (hasSharedEntries || !this.perEntityFilesDir || sharedFileExists) this.db.writeDomain(this.entitiesFile, diskData);
		if (this.perEntityFilesDir) {
			for (const filename of this.db.listDomainFiles(this.perEntityFilesDir)) {
				const entityId = this.entityIdFromPerEntityFilename(filename);
				if (!perEntityData.has(entityId)) this.db.removeDomainFile(filename);
			}
			for (const [entityId, snapshotConfig] of perEntityData) this.db.writeDomain(this.perEntityFilename(entityId), snapshotConfig);
		}
	}
	/**
	...
	*/
	...
	async createEntity(id, entity) {
		this.hydrate();
		if (this.entities.has(id)) throw new Error(`${this.name}: entity with id ${id} already exists`);
	...
	async updateEntity(id, updates) {
		...
		if (!existing) throw new Error(`${this.name}: entity with id ${id} not found`);
		const updated = { ...existing };
		for (const [key, value] of Object.entries(updates)) {
			if (key === "id") continue;
			if (value === void 0) continue;
			if (key === "metadata" && typeof value === "object" && value !== null) updated["metadata"] = {
				...updated["metadata"] ?? {},
				...value
			};
		...
		if (updatedEntity.status === "published" && updatedEntity.activeVersionId || wasPublished && updates["status"] !== void 0) this.persistToDisk();
	...
	async listEntities(args) {
		...
		if (filters) for (const [key, value] of Object.entries(filters)) {
			if (value === void 0) continue;
			if (key === "metadata" && typeof value === "object" && value !== null) entities = entities.filter((e) => {
				const meta = e["metadata"];
				if (!meta) return false;
				return Object.entries(value).every(([k, v]) => JSON.stringify(meta[k]) === JSON.stringify(v));
			});
			else entities = entities.filter((e) => e[key] === value);
		}
	...
	async createVersion(input) {
		...
		if (this.versions.has(input.id)) throw new Error(`${this.name}: version with id ${input.id} already exists`);
		const parentId = input[this.parentIdField];
		for (const v of this.versions.values()) if (v[this.parentIdField] === parentId && v.versionNumber === input.versionNumber) throw new Error(`${this.name}: version number ${input.versionNumber} already exists for entity ${parentId}`);
		const version = {
			...input,
	...
	async getVersionByNumber(entityId, versionNumber) {
		await this.ensureGitHistory();
		for (const v of this.versions.values()) if (v[this.parentIdField] === entityId && v.versionNumber === versionNumber) return structuredClone(v);
		return null;
	}
	async getLatestVersion(entityId) {
		await this.ensureGitHistory();
		let latest = null;
		for (const v of this.versions.values()) if (v[this.parentIdField] === entityId) {
	...
	async listVersions(input, parentIdField) {
		...
		const versions = Array.from(this.versions.values()).filter((v) => v[this.parentIdField] === entityId);
	...
	async deleteVersionsByParentId(entityId) {
		await this.ensureGitHistory();
		for (const [versionId, version] of this.versions) if (version[this.parentIdField] === entityId) {
	...
	async countVersions(entityId) {
		await this.ensureGitHistory();
		let count = 0;
		for (const v of this.versions.values()) if (v[this.parentIdField] === entityId) count++;
		return count;
	}
	/**
	...
	*/
	async ensurePerEntityGitHistory(entityId) {
		...
		if (Array.from(this.versions.values()).some((v) => v[this.parentIdField] === entityId)) return;
		...
		if (newCount > startCount) this.gitVersionCounts.set(entityId, newCount);
	}
	...
	_getNextVersionNumber(entityId) {
		let maxVersion = this.gitVersionCounts.get(entityId) ?? 0;
		for (const v of this.versions.values()) if (v[this.parentIdField] === entityId) maxVersion = Math.max(maxVersion, v.versionNumber);
		return maxVersion + 1;
	}
	async dangerouslyClearAll() {
		this.entities.clear();
		...
		if (this.perEntityFilesDir) for (const filename of this.db.listDomainFiles(this.perEntityFilesDir)) this.db.removeDomainFile(filename);
	}
};
//#endregion
export { GitHistory as a, MastraCompositeStore as c, normalizePerPage as d, InMemoryThreadStateStorage as f, getSourceControlEntityFilePath as i, MastraStorage as l, SOURCE_CONTROL_AGENTS_DIR as n, VersionedStorageDomain as o, ThreadStateStorage as p, getSourceAgentFilePath as r, EDITOR_DOMAINS as s, FilesystemVersionedHelpers as t, calculatePagination as u };

//# sourceMappingURL=filesystem-versioned-M7zmYTpZ.js.map