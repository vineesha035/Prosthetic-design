...
var MastraServerCache = class extends require_base.MastraBase {
	constructor({ name }) {
		super({
...
var InMemoryServerCache = class extends MastraServerCache {
	...
	constructor(options = {}) {
		...
		this.cache = new _isaacs_ttlcache.TTLCache({
	...
	async listPush(key, value) {
		...
		if (Array.isArray(existing)) {
			existing.push(value);
			if (this.ttlMs > 0) this.cache.set(key, existing, { ttl: this.ttlMs });
		} else if (existing !== void 0) throw new Error(`${key} exists but is not an array`);
	...
	async increment(key) {
		const value = this.cache.get(key);
		let counter;
		if (value === void 0) counter = 1;
		else if (typeof value === "number") counter = value + 1;
		else throw new Error(`${key} exists but is not a number`);
		this.cache.set(key, counter);
		return counter;
	}
};
//#endregion
Object.defineProperty(exports, "InMemoryServerCache", {
	enumerable: true,
	get: function() {
		return InMemoryServerCache;
	}
});
Object.defineProperty(exports, "MastraServerCache", {
	enumerable: true,
	get: function() {
		return MastraServerCache;
	}
});

//# sourceMappingURL=inmemory-CVHnncRp.cjs.map