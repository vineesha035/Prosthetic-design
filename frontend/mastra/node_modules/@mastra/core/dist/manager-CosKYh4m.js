	...
	try {
		...
		await pMap(items.map((item, idx) => ({
			...
			try {
				...
				} else itemScorers = datasetScorers;
				let itemSignal = executionSignal;
				if (itemTimeout) {
				...
				let retryCount = 0;
				let execResult = scorerConfigError ? {
					output: null,
					error: scorerConfigError,
					traceId: null
				...
				while (execResult.error && !scorerConfigError && retryCount < maxRetries) {
					...
					if (execResult.error.code === "TOOL_MOCK_MISMATCH" || execResult.error.code === "TOOL_MOCK_EXHAUSTED" || execResult.error.code === "TOOL_MOCK_NOT_DECLARED") break;
					retryCount++;
					const delay = Math.min(1e3 * Math.pow(2, retryCount - 1), 3e4);
					const jitter = delay * .2 * Math.random();
				...
				if (!scorerConfigError) {
					...
					const flatScores = await runScorersForItem(itemScorers, item, execResult.output, storage ?? null, experimentId, targetType ?? "agent", targetId ?? "inline", item.id, execResult.scorerInput, execResult.scorerOutput, execResult.traceId ?? void 0, workflowData, persistScores);
					const stepScores = await runStepScorersForItem(itemStepScorers, item, workflowData, storage ?? null, experimentId, targetType ?? "agent", targetId ?? "inline", item.id, execResult.traceId ?? void 0, persistScores);
					itemScores = [...flatScores, ...stepScores];
				}
				if (experimentsStore) try {
					...
					persistenceFailures++;
					itemResult.persistenceError = { message: persistError instanceof Error ? persistError.message : String(persistError) };
					mastra.getLogger()?.error(`Failed to persist experiment result for item ${item.id} in experiment ${experimentId}: ${itemResult.persistenceError.message}`, { error: persistError });
				}
				results[idx] = {
					...itemResult,
					scores: itemScores
				};
				if (execResult.error) failedCount++;
				else succeededCount++;
				if (experimentsStore) {
					const now = Date.now();
				...
				if (eventDispatcher) await eventDispatcher.emit(createItemCompletedEvent({
					experimentId,
					target: eventTarget
				}, idx, results[idx], execResult.traceId ?? null));
			} catch (error) {
				if (eventDispatcher?.failure) return;
				throw error;
			}
		...
		if (experimentsStore) await experimentsStore.updateExperiment({
			id: experimentId,
			status: "failed",
			succeededCount,
			failedCount,
...
async function resolveTarget(mastra, targetType, targetId, agentVersion) {
	...
	switch (targetType) {
		case "agent":
			try {
				if (agentVersion) resolved = await mastra.getAgentById(targetId, { versionId: agentVersion });
				else resolved = mastra.getAgentById(targetId);
			} catch {
				try {
					if (agentVersion) resolved = await mastra.getAgent(targetId, { versionId: agentVersion });
					else resolved = mastra.getAgent(targetId);
				} catch {}
			}
			break;
		case "workflow":
...
//#endregion
//#region src/datasets/dataset.ts
/**
...
var Dataset = class {
	...
	*/
	...
	async #getDatasetsStore() {
		...
		if (!storage) throw new MastraError({
		...
		if (!store) throw new MastraError({
	...
	*/
	async #assertScope() {
		...
		if (!await (await this.#getDatasetsStore()).getDatasetById({
			id: this.id,
			filters: this.#scope
	...
	*/
	async getDetails() {
		const record = await (await this.#getDatasetsStore()).getDatasetById({
			id: this.id,
			filters: this.#scope
		});
	...
	*/
	async update(input) {
		...
		if (inputSchema !== void 0 && inputSchema !== null && isZodType(inputSchema)) inputSchema = zodToJsonSchema(inputSchema);
		if (groundTruthSchema !== void 0 && groundTruthSchema !== null && isZodType(groundTruthSchema)) groundTruthSchema = zodToJsonSchema(groundTruthSchema);
	...
	*/
	async addItem(input) {
		return (await this.#getDatasetsStore()).addItem({
			datasetId: this.id,
			...input,
			filters: this.#scope
		});
	...
	*/
	async addItems(input) {
		return (await this.#getDatasetsStore()).batchInsertItems({
			datasetId: this.id,
			items: input.items,
			filters: this.#scope
		});
	...
	*/
	async getItem(args) {
		...
		const item = await (await this.#getDatasetsStore()).getItemById({
			id: args.itemId,
	...
	*/
	async listItems(args) {
		const store = await this.#getDatasetsStore();
		if (args?.version !== void 0 && args.search === void 0 && args.page === void 0 && args.perPage === void 0) {
			await this.#assertScope();
			return store.getItemsByVersion({
				datasetId: this.id,
	...
	*/
	async deleteItem(args) {
		return (await this.#getDatasetsStore()).deleteItem({
			id: args.itemId,
			datasetId: this.id,
			filters: this.#scope
		});
	...
	*/
	async deleteItems(args) {
		return (await this.#getDatasetsStore()).batchDeleteItems({
			datasetId: this.id,
	...
	*/
	async listVersions(args) {
		await this.#assertScope();
		return (await this.#getDatasetsStore()).listDatasetVersions({
			datasetId: this.id,
			pagination: {
				page: args?.page ?? 0,
				perPage: args?.perPage ?? 20
	...
	*/
	async getItemHistory(args) {
		await this.#assertScope();
		return (await (await this.#getDatasetsStore()).getItemHistory(args.itemId)).filter((row) => row.datasetId === this.id);
	}
	/**
	...
	*/
	async startExperimentAsync(config) {
		const experimentsStore = config.persistence?.experiments !== "none" ? await this.#getExperimentsStore() : void 0;
		const datasetsStore = await this.#getDatasetsStore();
		const dataset = await datasetsStore.getDatasetById({
			id: this.id,
			filters: this.#scope
		});
		...
		if (items.length === 0) throw new MastraError({
			id: "EXPERIMENT_NO_ITEMS",
		...
		if (experimentsStore) await experimentsStore.createExperiment({
			id: experimentId,
			datasetId: this.id,
	...
	*/
	async listExperiments(args) {
		await this.#assertScope();
		return (await this.#getExperimentsStore()).listExperiments({
			datasetId: this.id,
			...args?.targetType !== void 0 ? { targetType: args.targetType } : {},
			...args?.targetId !== void 0 ? { targetId: args.targetId } : {},
			...args?.agentVersion !== void 0 ? { agentVersion: args.agentVersion } : {},
	...
	* rejected.
	*/
	async #assertExperimentOwnership(experimentId) {
		await this.#assertScope();
		const experiment = await (await this.#getExperimentsStore()).getExperimentById({
			id: experimentId,
			filters: this.#scope
		});
		if (!experiment || experiment.datasetId !== this.id) throw new MastraError({
	...
	*/
	async listExperimentResults(args) {
		...
		return (await this.#getExperimentsStore()).listExperimentResults({
	...
	*/
	async updateExperimentResult(input) {
		if (!input.experimentId) throw new MastraError({
		...
		return (await this.#getExperimentsStore()).updateExperimentResult(input);
	}
	/**
	...
	*/
	async deleteExperiment(args) {
		await this.#assertExperimentOwnership(args.experimentId);
		return (await this.#getExperimentsStore()).deleteExperiment({
			id: args.experimentId,
			filters: this.#scope
		});
	}
...
//#endregion
//#region src/datasets/manager.ts
/**
...
function scopeFromArgs(args) {
	...
	return scope.organizationId === void 0 && scope.projectId === void 0 ? void 0 : scope;
}
/**
...
var DatasetsManager = class {
	...
	*/
	async create(input) {
		const store = await this.#getDatasetsStore();
		let { inputSchema, groundTruthSchema, ...rest } = input;
		if (inputSchema !== void 0 && isZodType(inputSchema)) inputSchema = zodToJsonSchema(inputSchema);
		if (groundTruthSchema !== void 0 && isZodType(groundTruthSchema)) groundTruthSchema = zodToJsonSchema(groundTruthSchema);
	...
	*/
	async list(args) {
		return (await this.#getDatasetsStore()).listDatasets({
			pagination: {
				page: args?.page ?? 0,
				perPage: args?.perPage ?? 20
			},
	...
	*/
	async delete(args) {
		return (await this.#getDatasetsStore()).deleteDataset({
			id: args.id,
	...
	*/
	async getExperiment(args) {
		return (await this.#getExperimentsStore()).getExperimentById({
			id: args.experimentId,
			filters: scopeFromArgs(args)
		});
	}
	...
	*/
	async compareExperiments(args) {
		...
		if (experimentIds.length < 2) throw new MastraError({
		...
		return {
			...
			items: internal.items.map((item) => {
				...
				return {
					...
					results: {
						[resolvedBaseline]: resultA ? {
							output: resultA.output,
							scores: item.scoresA
						} : null,
						[otherExperimentId]: resultB ? {
...
//#endregion
export { computeMean as a, EXPERIMENT_ITEM_SCORER_NOT_FOUND as c, executeTarget as d, TOOL_MOCK_EXHAUSTED as f, ToolMockMatcher as h, compareExperiments as i, resolveScorers as l, TOOL_MOCK_NOT_DECLARED as m, Dataset as n, computeScorerStats as o, TOOL_MOCK_MISMATCH as p, runExperiment as r, isRegression as s, DatasetsManager as t, runScorersForItem as u };

//# sourceMappingURL=manager-CosKYh4m.js.map