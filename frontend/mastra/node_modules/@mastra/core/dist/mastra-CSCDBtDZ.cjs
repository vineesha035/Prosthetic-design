...
const require_utils = require("./utils-Bw7FoAI3.cjs");
...
const require_schedules = require("./schedules-D_aayWxz.cjs");
...
const require_create_durable_agent = require("./create-durable-agent-CB4JGA_3.cjs");
const require_manager = require("./manager-BQFc3uOZ.cjs");
const require_license_index = require("./license/index.cjs");
const require_observability_context_storage = require("./observability/context-storage.cjs");
const require_tool_loop_agent = require("./tool-loop-agent--q5ddGzV.cjs");
const require_worker = require("./worker-H14yjTvH.cjs");
const require_workflows_builder_index = require("./workflows/builder/index.cjs");
...
function buildNotificationDispatchSchedule({ cron = NOTIFICATION_DISPATCH_DEFAULT_CRON, batchSize = 100 } = {}) {
	...
	return {
		...
		cron,
		status: "active",
		nextFireAt: require_cron.computeNextFireAt(cron, { after: now }),
		createdAt: now,
		updatedAt: now,
		metadata: {
			internal: true,
...
function createNotificationDispatchWorkflow({ batchSize = 100 } = {}) {
	const dispatchStep = require_agent.createStep({
		id: "dispatch-due-notifications",
		inputSchema: zod_v4.z.object({
			now: zod_v4.z.string().optional(),
			limit: zod_v4.z.number().optional()
		}),
		outputSchema: zod_v4.z.object({
			delivered: zod_v4.z.number(),
			failed: zod_v4.z.number()
		}),
		execute: async ({ inputData, mastra }) => {
			...
			const result = await require_storage$1.dispatchDueNotifications({
				mastra,
				storage,
	...
	return require_agent.createWorkflow$1({
...
function createUndefinedPrimitiveError(type, value, key) {
	...
	return new require_error.MastraError({
		id: `MASTRA_ADD_${type.toUpperCase().replace("-", "_")}_UNDEFINED`,
		domain: require_error.ErrorDomain.MASTRA,
...
var Mastra = class Mastra {
	...
	*/
	...
	static INTERNAL_WORKFLOW_TTL_MS = require_utils.readPositiveIntEnv("MASTRA_SUSPENDED_RUN_TTL_MS", 1800 * 1e3);
	...
	get agentThreadStreamRuntime() {
		return require_storage$1.agentThreadStreamRuntime;
	}
	get workers() {
		return this.#workers;
	}
	...
	*/
	...
	get datasets() {
		if (!this.#datasets) this.#datasets = new require_manager.DatasetsManager(this);
	...
	*/
	get schedules() {
		this.#schedules ??= new require_schedules.Schedules(this);
	...
	*/
	generateId(context) {
		if (this.#idGenerator) {
			...
			if (!id) {
				const error = new require_error.MastraError({
	...
	*/
	registerExporter(exporter, instance, entrypoint) {
		if (this.#observability instanceof require_observability.NoOpObservability) {
	...
	*/
	constructor(config) {
		require_observability_context_storage.initContextStorage();
		this.#serverCache = config?.cache ?? new require_inmemory.InMemoryServerCache();
		...
		this.#toolPayloadTransform = require_payload_transform.normalizeToolPayloadTransformPolicy(config?.transform ?? config?.toolPayloadProjection);
		if (config?.pubsub) this.#pubsub = config.pubsub;
		else this.#pubsub = new require_event_emitter.EventEmitterPubSub();
		this.#events = {};
		...
		if (workersOption === false) this.#workersDisabled = true;
			...
			if (pubsubModes.includes("pull")) defaultWorkers.push(new require_worker.OrchestrationWorker());
			if (config?.backgroundTasks?.enabled) defaultWorkers.push(new require_worker.BackgroundTaskWorker(config.backgroundTasks));
		...
		if (config?.logger === false) {
			logger = require_logger_index.noopLogger;
			this.#loggerExplicit = true;
		} else if (config?.logger) {
			logger = config.logger;
			this.#loggerExplicit = true;
		} else logger = new require_logger.ConsoleLogger({
			name: "Mastra",
			level: process.env.NODE_ENV === "production" && process.env.MASTRA_DEV !== "true" ? require_logger.LogLevel.WARN : require_logger.LogLevel.INFO
		});
		this.#logger = logger;
		...
		if (config?.storage) {
			...
			storage = new require_storage.InMemoryStore();
		...
		storage = require_agent.augmentWithInit(storage);
		if (storage.stores) {
			if (!storage.stores.workflows || !storage.stores.backgroundTasks) {
				const fallbackDb = new require_source.InMemoryDB();
				if (!storage.stores.workflows) storage.stores.workflows = new require_storage.WorkflowsInMemory({ db: fallbackDb });
				if (!storage.stores.backgroundTasks) storage.stores.backgroundTasks = new require_storage.BackgroundTasksInMemory({ db: fallbackDb });
			}
		}
		if (config?.observability) {
			...
			if (typeof config.observability.getDefaultInstance === "function") {
				...
				this.#observability = new require_observability.NoOpObservability();
			}
		} else this.#observability = new require_observability.NoOpObservability();
		const dualLogger = new require_logger_index.DualLogger(this.#logger, () => this.loggerVNext);
		...
		if (process.env.MASTRA_LICENSE_KEY || process.env.MASTRA_EE_LICENSE) require_license_index.LicenseClient.getInstance(this.#logger).validate().catch(() => {});
		...
		for (const gateway of require_llm.defaultGateways) {
			const key = require_gateway_helpers.getGatewayId(gateway);
			if (!Object.values(this.#gateways).some((existingGateway) => existingGateway != null && require_gateway_helpers.getGatewayId(existingGateway) === key)) this.#gateways[key] = gateway;
		}
		...
		if (!config?.__ephemeral) {
			this.#onScorerHook = require_agent.createOnScorerHook(this);
			require_hooks.registerHook("onScorerRun", this.#onScorerHook);
		}
		this.#observability.setMastraContext({ mastra: this });
		...
		const bgManager = new require_background_tasks.BackgroundTaskManager(effectiveMode ? {
		...
		for (const { scheduleId, workflowId, cfg } of declared) try {
			...
			if (!existing) {
				await schedulesStore.createSchedule({
					...
					status: "active",
					nextFireAt: require_cron.computeNextFireAt(cfg.cron, {
						timezone: cfg.timezone,
						after: now
					}),
					createdAt: now,
			...
			if (cronChanged || timezoneChanged) patch.nextFireAt = require_cron.computeNextFireAt(cfg.cron, {
				timezone: cfg.timezone,
				after: now
			});
	...
	*/
	getChannels() {
		...
		for (const [agentKey, agent] of Object.entries(this.#agents ?? {})) {
			...
			if (agentChannels instanceof require_agent.AgentChannels && !controllerOwned.has(agentChannels)) result[agentKey] = agentChannels;
		}
	...
	*/
	addAgent(agent, key, options) {
		...
		if (!(require_agent.isDurableAgentLike(agent) && agent.agent !== agent) && agent.durable) {
			const durableOption = agent.durable;
			const opts = durableOption === true ? {} : { ...durableOption };
			agent = require_create_durable_agent.createDurableAgent({
				agent,
				...opts
			});
		}
		if (require_agent.isDurableAgentLike(agent)) {
		...
		let mastraAgent;
		if (require_tool_loop_agent.isToolLoopAgentLike(agent)) mastraAgent = require_tool_loop_agent.toolLoopAgentToMastraAgent(agent, { fallbackName: key });
	...
	*/
	__registerFsObservability(fsObservability) {
		...
		const rawLogger = this.#logger instanceof require_logger_index.DualLogger ? this.#logger.baseLogger : this.#logger;
	...
	*/
	addWorkspace(workspace, key, metadata) {
		...
		if (source === "agent" && (!metadata?.agentId || !metadata?.agentName)) throw new require_error.MastraError({
	...
	__registerInternalWorkflow(workflow, runId) {
		...
		if (runId) {
			...
			if (isNewRegistration) {
				...
				if (!this.#runScopes.has(runId)) this.#runScopes.set(runId, require_agent.createRunScope());
			}
	...
	*/
	__createRunScope(runId) {
		...
		if (!scope) {
			scope = require_agent.createRunScope();
	...
	*/
	...
	__getInternalWorkflow(id, runId) {
		...
		if (!workflow) throw new require_error.MastraError({
	...
	*/
	async recoverAllDurableAgents() {
		...
		const durableAgents = [];
		for (const agent of Object.values(this.#agents ?? {})) if (agent && require_agent.isDurableAgentLike(agent)) durableAgents.push(agent);
	...
	*/
	getPromptBlock(key) {
		...
		if (!block) throw new require_error.MastraError({
	...
	*/
	getToolById(id) {
		...
		if (!allTools) throw new require_error.MastraError({
	...
	*/
	getProcessorById(id) {
		...
		if (!allProcessors) throw new require_error.MastraError({
	...
	*/
	#buildWorkflowRegistryIndex() {
		...
		for (const [key, tool] of Object.entries(this.listTools() ?? {})) {
			const schemas = {
				inputSchema: require_validate.toJsonSchemaOrUndefined(tool.inputSchema),
				outputSchema: require_validate.toJsonSchemaOrUndefined(tool.outputSchema)
			};
			tools[key] = schemas;
		...
		for (const [key, workflow] of Object.entries(this.#workflows)) {
			const schemas = {
				inputSchema: require_validate.toJsonSchemaOrUndefined(workflow.inputSchema),
				outputSchema: require_validate.toJsonSchemaOrUndefined(workflow.outputSchema)
			};
	...
	*/
	async addStoredWorkflows(defs) {
		...
		const members = defs.map((def) => ({
			def,
			normalized: require_workflows_builder_index.normalizeWorkflowBuilderDefinition({
				id: def.id,
		...
		for (const { normalized } of members) require_validate.assertValidStoredWorkflow(normalized, index);
		const ordered = [];
		...
		while (remaining.size > 0 && progress) {
			...
			for (const [id, member] of Array.from(remaining)) {
				if (Array.from(require_validate.collectNestedWorkflowIds(member.def.graph)).filter((dependency) => dependency !== id && bundleIds.has(dependency) && !hydrated.has(dependency)).length > 0) continue;
				remaining.delete(id);
				hydrated.add(id);
				ordered.push(member);
				progress = true;
		...
		try {
			for (const { def } of ordered) {
				const { workflow } = await require_validate.rehydrateWorkflow(def, this);
	...
	*/
	async #loadStoredWorkflows() {
		...
		for (const def of pending) {
			const all = require_validate.collectNestedWorkflowIds(def.graph);
		...
		while (remaining.size > 0 && progress) {
			...
			for (const [id, def] of Array.from(remaining)) {
				...
				progress = true;
				try {
					const { workflow } = await require_validate.rehydrateWorkflow({
						id: def.id,
						description: def.description,
						metadata: def.metadata,
						inputSchema: def.inputSchema,
	...
	*/
	...
	async #startSchedulingWorkers() {
		...
		if (!this.#findSchedulerWorker()) {
			const sw = new require_worker.SchedulerWorker(this.#schedulerConfig);
		...
		if (!this.#findAgentScheduleWorker()) {
			const { AgentScheduleWorker } = await Promise.resolve().then(() => require("./worker-D6tELOAF.cjs"));
	...
	*/
	setStorage(storage) {
		this.#storageFallbackWarningPending = false;
		this.#storage = require_agent.augmentWithInit(storage);
		this.#storage?.__registerMastra?.(this);
		this.#ensureBackgroundTaskManager();
	}
	setLogger({ logger }) {
		const dualLogger = new require_logger_index.DualLogger(logger, () => this.loggerVNext);
		this.#logger = dualLogger;
	...
	*/
	get loggerVNext() {
		return this.#observability.getDefaultInstance()?.getLoggerContext?.() ?? require_observability.noOpLoggerContext;
	}
	/**
	...
	*/
	get metrics() {
		return this.#observability.getDefaultInstance()?.getMetricsContext?.() ?? require_observability.noOpMetricsContext;
	}
	getServerMiddleware() {
	...
	*/
	...
	async listLogs(transportId, params) {
		...
		if (!this.#logger) throw new require_error.MastraError({
	...
	*/
	async handleWorkflowEvent(event) {
		if (!this.#workflowEventProcessor) this.#workflowEventProcessor = new require_workflow_event_processor.WorkflowEventProcessor({ mastra: this });
	...
	*/
	__unregisterHooks() {
		if (this.#onScorerHook) {
			require_hooks.deregisterHook("onScorerRun", this.#onScorerHook);
	...
	*/
	getGatewayById(id) {
		...
		for (const gateway of Object.values(gateways)) if (require_gateway_helpers.getGatewayId(gateway) === id) return gateway;
		const error = new require_error.MastraError({
			...
			details: {
				status: 404,
				gatewayId: id,
				availableIds: Object.values(gateways).map((g) => require_gateway_helpers.getGatewayId(g)).join(", ")
			}
		});
		this.#logger?.trackException(error);
		throw error;
	...
	*/
	addGateway(gateway, key) {
		...
		const gatewayKey = key || require_gateway_helpers.getGatewayId(gateway);
		const gateways = this.#gateways;
	...
	*/
	#syncGatewayRegistry() {
		try {
			...
			Promise.resolve().then(() => require("./provider-registry-Bv8eMxuW.cjs")).then((n) => n.provider_registry_exports).then(async ({ GatewayRegistry }) => {
...
require_agent.__registerMastraCtor(Mastra);
//#endregion
...
//# sourceMappingURL=mastra-CSCDBtDZ.cjs.map