...
var MCPServerBase = class extends MastraBase {
	...
	*/
	__registerMastra(mastra) {
		...
		if (this.originalTools && typeof this.originalTools === "object") Object.entries(this.originalTools).forEach(([key, tool]) => {
			try {
				...
				if (!(error instanceof MastraError) || error.id !== "MASTRA_ADD_TOOL_DUPLICATE_KEY") throw error;
			}
		});
		if (this.agents && typeof this.agents === "object") Object.entries(this.agents).forEach(([key, agent]) => {
			try {
				mastra.addAgent(agent, key);
			} catch (error) {
				if (!(error instanceof MastraError) || error.id !== "MASTRA_ADD_AGENT_DUPLICATE_KEY") throw error;
			}
		});
		if (this.workflows && typeof this.workflows === "object") Object.entries(this.workflows).forEach(([key, workflow]) => {
			try {
				mastra.addWorkflow(workflow, key);
			} catch (error) {
				if (!(error instanceof MastraError) || error.id !== "MASTRA_ADD_WORKFLOW_DUPLICATE_KEY") throw error;
			}
		});
	}
	/**
	...
	*/
	constructor(config) {
		super({
			component: RegisteredLogger.MCP_SERVER,
			name: config.name
		});
		...
		if (config.id) {
			...
			this.idWasSet = true;
		} else this._id = this.mastra?.generateId() || randomUUID();
