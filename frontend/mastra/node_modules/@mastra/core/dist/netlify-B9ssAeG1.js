...
import { c as MastraModelGateway, i as MASTRA_USER_AGENT, l as createOpenAI, o as createAnthropic, p as createOpenAICompatible, t as createGoogleGenerativeAI } from "./dist-KaRIc0QX.js";
//#region src/llm/model/gateways/netlify.ts
var netlify_exports = /* @__PURE__ */ __exportAll({ NetlifyGateway: () => NetlifyGateway });
var NetlifyGateway = class extends MastraModelGateway {
	...
	async fetchProviders() {
		...
		if (!response.ok) throw new Error(`Failed to fetch from Netlify: ${response.statusText}`);
		...
		for (const [providerId, provider] of Object.entries(data.providers)) for (const model of provider.models) config.models.push(`${providerId}/${model}`);
	...
	*/
	async getOrFetchToken(siteId, netlifyToken) {
		...
		if (cached && cached.expiresAt > Date.now() / 1e3 + 60) return {
			token: cached.token,
			url: cached.url
		};
	...
	*/
	...
	async resolveLanguageModel({ modelId, providerId, apiKey, headers }) {
		...
		switch (providerId) {
			...
			default: return createOpenAICompatible({
				name: providerId,
				apiKey,
				baseURL,
				headers: mastraHeaders,
...
//#endregion
export { netlify_exports as n, NetlifyGateway as t };

//# sourceMappingURL=netlify-B9ssAeG1.js.map