//#region src/tools/payload-transform.ts
function normalizeToolPayloadTransformPolicy(policy) {
	if (!policy) return;
	if ("transformToolPayload" in policy) return policy;
	if ("projectToolPayload" in policy && policy.projectToolPayload) return {
		transformToolPayload: policy.projectToolPayload,
		targets: policy.targets
	};
}
const PHASE_TO_TOOL_TRANSFORMER = {
...
function safePlaceholder(context) {
	return { message: `Tool ${context.phase} payload unavailable` };
}
async function transformOneTarget(context, source, logger) {
	if (!isTransformConfigured(source, context.target)) return;
	const transformers = [isPolicyConfiguredForTarget(source?.policy, context.target) ? source?.policy?.transformToolPayload : void 0, getToolTransformer(source, context.target, context.phase)].filter(Boolean);
	if (transformers.length === 0) return context.phase === "input-delta" ? { suppress: true } : { transformed: safePlaceholder(context) };
	for (const transformer of transformers) try {
		const transformed = await transformer(context);
		if (transformed !== void 0) return { transformed };
	} catch (error) {
		...
		return context.phase === "input-delta" ? {
			...
			transformed: safePlaceholder(context),
			failed: true
		};
	}
...
async function transformToolPayloadForTargets(context, source, logger) {
	const display = await transformOneTarget({
		...context,
		target: "display"
	}, source, logger);
	const transcript = await transformOneTarget({
		...context,
		target: "transcript"
	}, source, logger);
	if (!display && !transcript) return;
	return {
...
function withToolPayloadTransformProviderMetadata(providerMetadata, transformMetadata) {
	...
	if (!transform) return providerMetadata;
	return {
		...providerMetadata ?? {},
		mastra: {
...
//#endregion
Object.defineProperty(exports, "getTransformedToolPayload", {
	enumerable: true,
	get: function() {
		return getTransformedToolPayload;
	}
});
Object.defineProperty(exports, "hasTransformedToolPayload", {
	enumerable: true,
	get: function() {
		return hasTransformedToolPayload;
	}
});
Object.defineProperty(exports, "normalizeToolPayloadTransformPolicy", {
	enumerable: true,
	get: function() {
		return normalizeToolPayloadTransformPolicy;
	}
});
Object.defineProperty(exports, "transformToolPayloadForTargets", {
	enumerable: true,
	get: function() {
		return transformToolPayloadForTargets;
	}
});
Object.defineProperty(exports, "withToolPayloadTransformMetadata", {
	enumerable: true,
	get: function() {
		return withToolPayloadTransformMetadata;
	}
});
Object.defineProperty(exports, "withToolPayloadTransformProviderMetadata", {
	enumerable: true,
	get: function() {
		return withToolPayloadTransformProviderMetadata;
	}
});

//# sourceMappingURL=payload-transform-DJzDdfpE.cjs.map