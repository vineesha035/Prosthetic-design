...
import { c as MastraModelGateway, i as MASTRA_USER_AGENT, o as createAnthropic, r as GATEWAY_AUTH_HEADER } from "./dist-KaRIc0QX.js";
import { i as getCapabilityFileName, n as getGatewayId, r as shouldEnableGateway } from "./gateway-helpers-DusR3xFY.js";
import { i as createOpenRouter, t as ModelsDevGateway } from "./models-dev-DTTDoZNB.js";
...
var mastra_exports = /* @__PURE__ */ __exportAll({ MastraGateway: () => MastraGateway });
var MastraGateway = class extends MastraModelGateway {
	...
	async getApiKey() {
		...
		if (!apiKey) throw new MastraError({
	...
	resolveLanguageModel({ modelId, providerId, apiKey, headers }) {
		...
		if (this.config?.customFetch && providerId === "anthropic") return createAnthropic({
		...
		if (this.config?.customFetch) return createOpenRouter({
			apiKey: "oauth-gateway-placeholder",
			baseURL,
			headers: {
				"User-Agent": MASTRA_USER_AGENT,
...
*/
var provider_registry_exports = /* @__PURE__ */ __exportAll({
...
function getEnabledGatewayIds(gateways) {
	...
	for (const gateway of gateways) if (shouldEnableGateway(gateway)) enabledGatewayIds.add(getGatewayId(gateway));
...
function getPackageRoot() {
	try {
		const packageJsonPath = createRequire(import.meta.url || "file://").resolve("@mastra/core/package.json");
...
var GatewayRegistry = class GatewayRegistry {
	...
	*/
	async syncGateways(forceRefresh = false, writeToSrc = false) {
		...
		try {
			const { ModelsDevGateway } = await import("./models-dev-DTTDoZNB.js").then((n) => n.n);
			const { NetlifyGateway } = await import("./netlify-B9ssAeG1.js").then((n) => n.n);
			const { MastraGateway } = await Promise.resolve().then(() => mastra_exports);
			const { fetchProvidersFromGateways, writeRegistryFiles } = await import("./registry-generator-BTyp-Kqp.js");
			...
			if (writeToSrc) {
				...
				if (fs.existsSync(distCapDir)) {
					...
					for (const file of capFiles) await fs.promises.copyFile(path.join(distCapDir, file), path.join(srcCapDir, file));
				}
			}
...
//#endregion
export { isProviderRegistered as a, modelSupportsTemperature as c, MastraGateway as d, getRegisteredProviders as i, parseModelString as l, PROVIDER_REGISTRY as n, modelSupportsAttachments as o, getProviderConfig as r, modelSupportsStructuredOutput as s, GatewayRegistry as t, provider_registry_exports as u };

//# sourceMappingURL=provider-registry-DOB2blrv.js.map