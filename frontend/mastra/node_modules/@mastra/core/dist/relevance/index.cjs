...
//#region src/relevance/relevance-score-provider.ts
...
//#endregion
//#region src/relevance/mastra-agent/index.ts
var MastraAgentRelevanceScorer = class {
	agent;
	constructor(name, model) {
		this.agent = new require_agent.Agent({
...
Always return just the number, no explanation.`,
	...
	async getRelevanceScore(query, text) {
		...
		let response;
		if (require_trip_wire.isSupportedLanguageModel(model)) response = (await this.agent.generate(prompt)).text;
		else response = (await this.agent.generateLegacy(prompt)).text;
