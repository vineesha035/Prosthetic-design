...
const ScheduleAttributesSchema = z.record(z.string(), z.union([
	z.string(),
	z.number(),
	z.boolean(),
	z.null()
...
const ScheduleStreamOptionsSchema = z.object({ requestContext: z.record(z.string(), z.unknown()).optional() });
...
//#endregion
//#region src/schedules/schedules.ts
...
function canonicalizeScheduleId(rawId, prefix) {
	...
	const slug = slugify(trimmed.startsWith(prefix) ? trimmed.slice(prefix.length) : trimmed);
	if (!slug) return "";
	return `${prefix}${slug}`;
}
/**
...
function normalizeScheduleId(rawId, prefix) {
	...
	if (!canonical) throw new MastraError({
...
var Schedules = class {
	...
	async #getStore() {
		const store = await this.#mastra.getStorage()?.getStore("schedules");
	...
	*/
	...
	async #createAgentSchedule(input) {
		...
		if (!input.threadId) {
			...
			if (offenders.length > 0) throw new MastraError({
		...
		const now = Date.now();
		const nextFireAt = computeNextFireAt(input.cron, {
		...
		return toAgentSchedule(await store.createSchedule(schedule));
	}
	async #createWorkflowSchedule(input) {
		...
		return toWorkflowSchedule(await store.createSchedule(schedule));
	}
	async #assertIdAvailable(store, id, callerProvided) {
		if (!callerProvided) return;
		if (await store.getSchedule(id)) throw new MastraError({
	...
	async list(filter) {
		const views = (await (await this.#getStore()).listSchedules({
			...filter?.agentId ? {
				ownerType: "agent",
				ownerId: filter.agentId
			} : {},
			...filter?.workflowId ? { workflowId: filter.workflowId } : {},
			...filter?.status ? { status: filter.status } : {}
		})).map(toScheduleView).filter((s) => s !== null).filter((s) => filter?.agentId ? s.agentId !== void 0 : true);
		if (!(filter?.threadId !== void 0 || filter?.resourceId !== void 0 || filter?.name !== void 0)) return views;
	...
	async update(id, patch) {
		...
		if (patch.cron !== void 0 || patch.timezone !== void 0) validateCron(nextCron, nextTimezone);
		...
		const nextFireAt = patch.cron !== void 0 || patch.timezone !== void 0 || resuming ? computeNextFireAt(nextCron, {
			timezone: nextTimezone,
			after: Date.now()
		}) : void 0;
		return toScheduleView(await store.updateSchedule(existing.id, {
	...
	async pause(id) {
		...
		return toScheduleView(await store.updateSchedule(existing.id, { status: "paused" }));
	}
	async resume(id) {
		const store = await this.#getStore();
		const existing = await this.#load(id);
...
//#endregion
export { AGENT_SCHEDULE_PREFIX as a, WORKFLOW_SCHEDULE_PREFIX as c, toWorkflowSchedule as i, toAgentSchedule as n, ScheduleInputSchema as o, toScheduleView as r, ScheduleOutputSchema as s, Schedules as t };

//# sourceMappingURL=schedules-vpzpiSqm.js.map