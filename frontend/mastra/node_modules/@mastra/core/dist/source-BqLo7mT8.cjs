...
//#region src/storage/domains/agents/base.ts
var AgentsStorage = class extends require_filesystem_versioned.VersionedStorageDomain {
	listKey = "agents";
...
//#endregion
//#region src/storage/domains/agents/inmemory.ts
var InMemoryAgentsStorage = class extends AgentsStorage {
	...
	async create(input) {
		const { agent } = input;
		if (this.db.agents.has(agent.id)) throw new Error(`Agent with id ${agent.id} already exists`);
	...
	async list(args) {
		const { page = 0, perPage: perPageInput, orderBy, authorId, visibility, metadata, status, entityIds, pinFavoritedFor, favoritedOnly } = args || {};
		...
		if (status) agents = agents.filter((agent) => agent.status === status);
		if (authorId !== void 0) agents = agents.filter((agent) => agent.authorId === authorId);
		if (visibility !== void 0) agents = agents.filter((agent) => agent.visibility === visibility);
		if (metadata && Object.keys(metadata).length > 0) agents = agents.filter((agent) => {
			if (!agent.metadata) return false;
			return Object.entries(metadata).every(([key, value]) => require_deep_equal.deepEqual(agent.metadata[key], value));
		});
		const favoritedIds = pinFavoritedFor ? this.collectFavoritedIdsFor(pinFavoritedFor) : void 0;
		if (favoritedOnly) if (favoritedIds) agents = agents.filter((agent) => favoritedIds.has(agent.id));
		else agents = [];
		const clonedAgents = this.sortAgents(agents, field, direction, favoritedIds).map((agent) => this.deepCopyAgent(agent));
	...
	async createVersion(input) {
		if (this.db.agentVersions.has(input.id)) throw new Error(`Version with id ${input.id} already exists`);
		for (const version of this.db.agentVersions.values()) if (version.agentId === input.agentId && version.versionNumber === input.versionNumber) throw new Error(`Version number ${input.versionNumber} already exists for agent ${input.agentId}`);
		const version = {
			...input,
	...
	async getVersionByNumber(agentId, versionNumber) {
		for (const version of this.db.agentVersions.values()) if (version.agentId === agentId && version.versionNumber === versionNumber) return this.deepCopyVersion(version);
		return null;
	}
	async getLatestVersion(agentId) {
		let latest = null;
		for (const version of this.db.agentVersions.values()) if (version.agentId === agentId) {
	...
	async deleteVersionsByParentId(entityId) {
		const idsToDelete = [];
		for (const [id, version] of this.db.agentVersions.entries()) if (version.agentId === entityId) idsToDelete.push(id);
		for (const id of idsToDelete) this.db.agentVersions.delete(id);
	}
	async countVersions(agentId) {
		let count = 0;
		for (const version of this.db.agentVersions.values()) if (version.agentId === agentId) count++;
		return count;
	}
	/**
	...
	*/
	collectFavoritedIdsFor(userId) {
		...
		for (const row of this.db.favorites.values()) if (row.userId === userId && row.entityType === "agent") favorited.add(row.entityId);
		return favorited;
	}
...
//#endregion
//#region src/storage/domains/inmemory-db.ts
/**
...
//#endregion
//#region src/storage/domains/agents/filesystem.ts
/**
...
*/
const PERSISTED_SNAPSHOT_FIELDS = /* @__PURE__ */ new Set([
	"name",
	"instructions",
	"model",
	"tools",
...
*/
const CODE_MODE_EXCLUDED_FIELDS = /* @__PURE__ */ new Set(["model", "name"]);
/**
...
function ownershipFromEditorConfig$1(editorConfig) {
	...
	return {
		ownsInstructions,
		ownsTools: toolsCfg === true || typeof toolsCfg === "object" && toolsCfg !== null && toolsCfg.description === true
	};
}
function stripUnusedFields(obj) {
	const result = {};
	for (const [key, value] of Object.entries(obj)) if (PERSISTED_SNAPSHOT_FIELDS.has(key)) result[key] = value;
	return result;
}
...
var FilesystemAgentsStorage = class extends AgentsStorage {
	...
	constructor({ db }) {
		...
		const getCodeAgent = (entityId) => {
			try {
				...
				if (isAgentNotFoundError(error, entityId)) return;
				throw error;
			}
		};
		...
		this.helpers = new require_filesystem_versioned.FilesystemVersionedHelpers({
			...
			perEntitySnapshotFilter: (snapshot, entity) => {
				const { ownsInstructions, ownsTools } = ownershipFromEditorConfig$1(editorConfigFor(entity.id));
				const excludedByOwnership = /* @__PURE__ */ new Set();
				if (!ownsInstructions) for (const field of OWNED_FIELDS_BY_GROUP$1.instructions) excludedByOwnership.add(field);
				if (!ownsTools) for (const field of OWNED_FIELDS_BY_GROUP$1.tools) excludedByOwnership.add(field);
				const result = {};
	...
	async update(input) {
		const { id, ...updates } = input;
		const entityUpdates = {};
		const entityFields = /* @__PURE__ */ new Set([
			"authorId",
			"visibility",
			"metadata",
			"activeVersionId",
		...
		for (const [key, value] of Object.entries(updates)) if (entityFields.has(key)) entityUpdates[key] = value;
	...
	async listVersions(input) {
		return await this.helpers.listVersions(input, "agentId");
	}
	async deleteVersion(id) {
...
//#endregion
//#region src/storage/domains/agents/source.ts
const SOURCE_VERSION_PREFIX = "source:";
const COMMON_EXCLUDED_FIELDS = /* @__PURE__ */ new Set([
	"id",
	"model",
	"scorers",
	"skills",
...
const CODE_SOURCE_EXCLUDED_FIELDS = /* @__PURE__ */ new Set(["name"]);
...
function filterSourceSnapshot(snapshot, editorConfig, isCodeDefinedAgent) {
	...
	if (isCodeDefinedAgent) {
		...
		if (!ownsInstructions) for (const field of OWNED_FIELDS_BY_GROUP.instructions) excludedByOwnership.add(field);
		if (!ownsTools) for (const field of OWNED_FIELDS_BY_GROUP.tools) excludedByOwnership.add(field);
	}
	const result = {};
	for (const [key, value] of Object.entries(snapshot)) {
...
function stableStringify(value) {
	...
	if (value && typeof value === "object") return `{${Object.entries(value).filter(([, entryValue]) => entryValue !== void 0).sort(([a], [b]) => a.localeCompare(b)).map(([key, entryValue]) => `${JSON.stringify(key)}:${stableStringify(entryValue)}`).join(",")}}`;
	return JSON.stringify(value);
}
function agentIdFromSourcePath(path) {
	const prefix = `${require_filesystem_versioned.SOURCE_CONTROL_AGENTS_DIR}/`;
	if (!path.startsWith(prefix) || !path.endsWith(".json")) return void 0;
	const filename = path.slice(prefix.length, -5);
...
var SourceAgentsSourceControl = class extends AgentsStorage {
	...
	async init() {
		...
		if (!capabilities.canRead) throw new Error(capabilities.reason ?? `Source provider ${this.provider.displayName} cannot read files`);
	...
	async useProviderRef(agentId, ref) {
		...
		for (const [versionId, version] of this.providerVersions.entries()) if (version.agentId === agentId) this.providerVersions.delete(versionId);
	...
	async create(input) {
		...
		if (await this.memory.getById(input.agent.id)) throw new Error(`Agent with id ${input.agent.id} already exists`);
	...
	async delete(id) {
		...
		for (const versionId of this.providerVersions.keys()) if (this.providerVersions.get(versionId)?.agentId === id) this.providerVersions.delete(versionId);
	...
	async createVersion(input) {
		...
		if (await this.memory.getVersion(input.id)) throw new Error(`Version with id ${input.id} already exists`);
		if (await this.memory.getVersionByNumber(input.agentId, input.versionNumber)) throw new Error(`Version number ${input.versionNumber} already exists for agent ${input.agentId}`);
		const snapshot = snapshotFromVersion({
			...input,
			createdAt: /* @__PURE__ */ new Date()
		});
	...
	async getVersion(id) {
		...
		if (providerVersion) return structuredClone(providerVersion);
		return this.memory.getVersion(id);
	}
	async getVersionByNumber(agentId, versionNumber) {
		await this.loadHistory(agentId);
		const providerVersion = [...this.providerVersions.values()].find((version) => version.agentId === agentId && version.versionNumber === versionNumber);
		if (providerVersion) return structuredClone(providerVersion);
	...
	async getLatestVersion(agentId) {
		...
		if (providerLatest) return structuredClone(providerLatest);
	...
	async listVersions(input) {
		...
		if (providerVersions.length === 0) return this.memory.listVersions(input);
		const { page = 0, perPage: perPageInput, orderBy } = input;
		const { field, direction } = this.parseVersionOrderBy(orderBy);
		const perPage = require_filesystem_versioned.normalizePerPage(perPageInput, 20);
		const sortedVersions = this.sortVersions(providerVersions, field, direction).map((version) => structuredClone(version));
	...
	async deleteVersionsByParentId(entityId) {
		for (const [versionId, version] of this.providerVersions.entries()) if (version.agentId === entityId) this.providerVersions.delete(versionId);
		await this.memory.deleteVersionsByParentId(entityId);
	}
	async countVersions(entityId) {
		await this.loadHistory(entityId);
		return [...this.providerVersions.values()].filter((version) => version.agentId === entityId).length || this.memory.countVersions(entityId);
	}
	refreshKnownAgentIds() {
		const agents = this.storageMastra?.listAgents?.();
		if (!agents) return;
		for (const agent of Object.values(agents)) if (agent.source === "code") this.knownAgentIds.add(agent.id);
	}
	async discoverProviderAgentIds() {
		if (this.providerAgentIdsDiscovered || !this.provider.listFiles) return;
		const files = await this.provider.listFiles({ path: require_filesystem_versioned.SOURCE_CONTROL_AGENTS_DIR });
		for (const file of files) {
			const agentId = agentIdFromSourcePath(file.path);
			if (agentId) this.knownAgentIds.add(agentId);
		}
	...
	async hydrateAgent(agentId) {
		...
		const file = await this.provider.readFile({
			path: require_filesystem_versioned.getSourceAgentFilePath(agentId),
			ref
		});
		if (!file) {
	...
	async persistSnapshot(agentId, snapshot, message) {
		const capabilities = await this.provider.getCapabilities();
		if (!capabilities.canWrite) throw new Error(capabilities.reason ?? `Source provider ${this.provider.displayName} cannot write files`);
	...
	async loadHistory(agentId) {
		...
		if (!(await this.provider.getCapabilities()).canListHistory) {
			this.loadedHistory.add(agentId);
			return;
		}
		const activeRef = this.activeRefs.get(agentId);
		const orderedEntries = [...await this.provider.listFileHistory({
			path: require_filesystem_versioned.getSourceAgentFilePath(agentId),
			ref: activeRef
		})].reverse();
		...
		for (const [versionId, version] of versions) this.providerVersions.set(versionId, version);
		this.loadedHistory.add(agentId);
	}
	...
	nextProviderVersionNumber(agentId) {
		return ([...this.providerVersions.values()].filter((version) => version.agentId === agentId).sort((a, b) => b.versionNumber - a.versionNumber)[0]?.versionNumber ?? 0) + 1;
	}
	sortVersions(versions, field, direction) {
		return versions.sort((a, b) => {
...
Object.defineProperty(exports, "SourceAgentsSourceControl", {
	enumerable: true,
	get: function() {
		return SourceAgentsSourceControl;
	}
});

//# sourceMappingURL=source-BqLo7mT8.cjs.map