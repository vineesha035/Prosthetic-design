...
async function sendNotificationRecord({ mastra, storage, record, now, batchThreadState }) {
	...
	if (current.priority === "high" && current.summarySignalId) {
		if ((batchThreadState ?? agentThreadStreamRuntime.getThreadState({
			resourceId: current.resourceId,
			threadId: current.threadId
		}, agent.getPubSub?.())) === "active") return null;
	}
...
async function sendNotificationSummary({ mastra, storage, records, now }) {
	...
	const target = records.every((record) => record.priority === "low") ? {
		resourceId: first.resourceId,
		threadId: first.threadId,
