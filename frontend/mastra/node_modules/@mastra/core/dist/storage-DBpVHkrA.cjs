...
const require_base$1 = require("./base-CfojwyC3.cjs");
const require_features_index = require("./features/index.cjs");
const require_source = require("./source-BqLo7mT8.cjs");
const require_inmemory = require("./inmemory-D6UzQ68e.cjs");
const require_filesystem = require("./filesystem-BdBNosM7.cjs");
const require_filesystem$1 = require("./filesystem-DHvxIGx3.cjs");
const require_storage = require("./storage-C4FD5U8Z.cjs");
const require_filesystem$2 = require("./filesystem-CceZwSX7.cjs");
const require_filesystem$3 = require("./filesystem-CpAldLfc.cjs");
const require_filesystem$4 = require("./filesystem-DJpgi1r3.cjs");
const require_filesystem$5 = require("./filesystem-Buug7_j8.cjs");
...
*/
var FactoryStorageDomain = class extends require_base$1.StorageDomain {
	name;
	#storage;
	constructor(name) {
...
*/
var ObservabilityStorage = class ObservabilityStorage extends require_base$1.StorageDomain {
	constructor() {
		super({
			component: "STORAGE",
			name: "OBSERVABILITY"
	...
	*/
	async createSpan(_args) {
		throw new require_error.MastraError({
			...
			domain: require_error.ErrorDomain.MASTRA_OBSERVABILITY,
			category: require_error.ErrorCategory.SYSTEM,
	...
	*/
	async getStructure(args) {
		if (this.getTraceLight === ObservabilityStorage.prototype.getTraceLight) throw new require_error.MastraError({
	...
	*/
	async getTraceLight(args) {
		if (this.getStructure === ObservabilityStorage.prototype.getStructure) throw new require_error.MastraError({
	...
	*/
	async getBranch(args) {
		const parsed = require_constants.getBranchArgsSchema.parse(args);
		try {
			const skeleton = await this.getStructure({ traceId: parsed.traceId });
			if (!skeleton) return null;
			const branchSpanIds = require_constants.extractBranchSpans(skeleton.spans, parsed.spanId, parsed.depth).map((s) => s.spanId);
			...
			if (!(error instanceof require_error.MastraError && (error.id === "OBSERVABILITY_STORAGE_GET_STRUCTURE_NOT_IMPLEMENTED" || error.id === "OBSERVABILITY_STORAGE_GET_TRACE_LIGHT_NOT_IMPLEMENTED" || error.id === "OBSERVABILITY_STORAGE_GET_SPANS_NOT_IMPLEMENTED"))) throw error;
		}
		const trace = await this.getTrace({ traceId: parsed.traceId });
		if (!trace) return null;
		const spans = require_constants.extractBranchSpans(trace.spans, parsed.spanId, parsed.depth);
		if (spans.length === 0) return null;
		return {
			traceId: parsed.traceId,
			spans
...
function transformRow(row, tableName, options = {}) {
	...
	const tableSchema = require_constants.TABLE_SCHEMAS[tableName];
	const result = {};
...
function transformScoreRow(row, options = {}) {
	return transformRow(row, require_constants.TABLE_SCORERS, options);
}
/**
...
var ObservabilityInMemory = class extends ObservabilityStorage {
	...
	deltaPollingFeatureEnabled() {
		return require_features_index.coreFeatures.has(OBSERVABILITY_DELTA_POLLING_FEATURE);
	}
	...
	*/
	upsertByIdField(records, cursorIds, record, idField) {
		...
		if (id == null) throw new require_error.MastraError({
			id: "OBSERVABILITY_MISSING_RECORD_ID",
			domain: require_error.ErrorDomain.STORAGE,
	...
	decodeDeltaCursor(cursor) {
		if (!/^\d+$/.test(cursor)) throw new require_error.MastraError({
		...
		if (!Number.isInteger(cursorId) || cursorId < 0) throw new require_error.MastraError({
			id: "OBSERVABILITY_INVALID_DELTA_CURSOR",
			domain: require_error.ErrorDomain.MASTRA_OBSERVABILITY,
			category: require_error.ErrorCategory.USER,
			text: "Invalid observability delta cursor"
	...
	maybeRegisterBranchCursor(span) {
		if (!require_constants.BRANCH_SPAN_TYPE_SET.has(span.spanType)) return;
	...
	validateCreateSpan(record) {
		if (!record.spanId) throw new require_error.MastraError({
			id: "OBSERVABILITY_SPAN_ID_REQUIRED",
			domain: require_error.ErrorDomain.MASTRA_OBSERVABILITY,
			category: require_error.ErrorCategory.SYSTEM,
			text: "Span ID is required for creating a span"
		});
		if (!record.traceId) throw new require_error.MastraError({
	...
	*/
	...
	getMatchingRootSpans(args) {
		const { filters, pagination, orderBy } = require_constants.listTracesArgsSchema.parse(args);
	...
	async listTraces(args) {
		const { mode, filters, after, limit } = require_constants.listTracesArgsSchema.parse(args);
		if (mode === "delta") {
			...
			return {
				spans: require_constants.toTraceSpans(deltaResponse.rows),
		...
		return {
			spans: require_constants.toTraceSpans(paged),
			pagination: {
				total,
				page,
				perPage,
	...
	*/
	...
	async listBranches(args) {
		const { mode, filters, pagination, orderBy, after, limit } = require_constants.listBranchesArgsSchema.parse(args);
		if (mode === "delta") {
			this.assertDeltaPollingEnabled();
			...
			return {
				branches: deltaResponse.rows.map(require_constants.toTraceSpan),
				delta: deltaResponse.delta,
				deltaCursor: deltaResponse.deltaCursor
			};
		}
		const allowedSpanTypes = filters?.spanType ? require_constants.BRANCH_SPAN_TYPE_SET.has(filters.spanType) ? /* @__PURE__ */ new Set([filters.spanType]) : /* @__PURE__ */ new Set() : require_constants.BRANCH_SPAN_TYPE_SET;
		const matches = [];
		...
		return {
			...
			branches: paged.map(require_constants.toTraceSpan),
	...
	*/
	spanMatchesBranchFilters(span, filters) {
		...
		if (filters.status !== void 0) {
			if (require_constants.toTraceSpan(span).status !== filters.status) return false;
		}
		return true;
	}
	async updateSpan(args) {
		...
		const traceEntry = this.db.traces.get(traceId);
		if (!traceEntry) throw new require_error.MastraError({
		...
		const span = traceEntry.spans[spanId];
		if (!span) throw new require_error.MastraError({
			id: "OBSERVABILITY_UPDATE_SPAN_NOT_FOUND",
			domain: require_error.ErrorDomain.MASTRA_OBSERVABILITY,
			category: require_error.ErrorCategory.SYSTEM,
	...
	async listMetrics(args) {
		const { mode, filters, pagination, orderBy, after, limit } = require_utils.listMetricsArgsSchema.parse(args);
		if (mode === "delta") {
			this.assertDeltaPollingEnabled();
	...
	*iterateObservabilityContextRecords() {
	...
	async getEntityTypes(_args) {
		const validTypes = new Set(Object.values(require_utils.EntityType));
	...
	async listLogs(args) {
		const { mode, filters, pagination, orderBy, after, limit } = require_utils.listLogsArgsSchema.parse(args);
		if (mode === "delta") {
			this.assertDeltaPollingEnabled();
	...
	async listScores(args) {
		const { mode, filters, pagination, orderBy, after, limit } = require_utils.listScoresArgsSchema.parse(args);
		if (mode === "delta") {
			this.assertDeltaPollingEnabled();
	...
	async listFeedback(args) {
		const { mode, filters, pagination, orderBy, after, limit } = require_utils.listFeedbackArgsSchema.parse(args);
		if (mode === "delta") {
			this.assertDeltaPollingEnabled();
...
const entityTypeValues = new Set(Object.values(require_utils.EntityType));
...
*/
var BackgroundTasksStorage = class extends require_base$1.StorageDomain {
	constructor() {
		super({
			component: "STORAGE",
...
*/
var BlobStore = class extends require_base.MastraBase {
	constructor() {
		super({
			component: "STORAGE",
			name: "BLOBS"
...
*/
var ChannelsStorage = class extends require_base$1.StorageDomain {
	constructor() {
		super({
			component: "STORAGE",
			name: "CHANNELS"
...
function resolveZodSchema(zodString) {
	return Function("z", `"use strict";return (${zodString});`)(zod_v4.z);
}
...
var SchemaValidator = class {
	...
	getValidator(schema, cacheKey) {
		...
		if (!zodSchema) {
			zodSchema = resolveZodSchema((0, _mastra_schema_compat_json_to_zod.jsonSchemaToZod)(schema));
...
var require_fast_deep_equal = /* @__PURE__ */ require_rolldown_runtime.__commonJSMin(((exports, module) => {
...
var import_fast_deep_equal = /* @__PURE__ */ require_rolldown_runtime.__toESM(require_fast_deep_equal(), 1);
const payloadFields = [
	"input",
	"groundTruth",
	"expectedTrajectory",
...
var DatasetItemIdentityConflictError = class extends require_error.MastraError {
	conflicts;
...
function validateDatasetItemExternalId(externalId) {
	if (externalId === "") throw new require_error.MastraError({
...
function validateDatasetItemPayloadSerialization(payload, path) {
	...
	for (const key of Object.keys(payload)) {
		...
		if (issue) throw new require_error.MastraError({
...
*/
var DatasetsStorage = class extends require_base$1.StorageDomain {
	constructor() {
		super({
			component: "STORAGE",
			name: "DATASETS"
	...
	validateCallerDefinedDatasetId(id) {
		if (id.length === 0) throw new require_error.MastraError({
	...
	*/
	resolveExistingDataset(existing, input) {
		if (DATASET_IMMUTABLE_FIELDS.some((field) => (existing[field] ?? null) !== (input[field] ?? null))) throw new require_error.MastraError({
...
var DatasetsInMemory = class extends DatasetsStorage {
	...
	async listDatasets(args) {
		...
		const { offset: start, perPage: perPageForResponse } = require_filesystem_versioned.calculatePagination(page, perPageInput, perPage);
...
*/
var ExperimentsStorage = class extends require_base$1.StorageDomain {
	constructor() {
		super({
			component: "STORAGE",
			name: "EXPERIMENTS"
...
var ExperimentsInMemory = class extends ExperimentsStorage {
	...
	async updateExperimentResult(input) {
		...
		const updated = {
			...
			comment: input.comment !== void 0 ? input.comment : existing.comment
		};
...
var HarnessStorage = class extends require_base$1.StorageDomain {
	constructor() {
		super({
			component: "STORAGE",
			name: "HARNESS"
...
var MemoryStorage = class extends require_base$1.StorageDomain {
	/**
...
var InMemoryMemory = class extends MemoryStorage {
	...
	async listMessages({ threadId, resourceId: optionalResourceId, include, filter, perPage: perPageInput, page = 0, orderBy }) {
		...
		const perPage = require_filesystem_versioned.normalizePerPage(perPageInput, 40);
		...
		if (include && include.length > 0) for (const includeItem of include) {
	...
	async listMessagesByResourceId({ resourceId, filter, perPage: perPageInput, page = 0, orderBy }) {
		const metadataFilter = validateStorageMetadataFilter(filter?.metadata);
		const { field, direction } = this.parseOrderBy(orderBy, "ASC");
		const perPage = require_filesystem_versioned.normalizePerPage(perPageInput, 40);
		if (page < 0) throw new Error("page must be >= 0");
		...
		const list = new require_message_list.MessageList().add(paginatedMessages.map((m) => this.parseStoredMessage(m)), "memory");
	...
	async listMessagesById({ messageIds }) {
		...
		return { messages: new require_message_list.MessageList().add(rawMessages.map((m) => this.parseStoredMessage(m)), "memory").get.all.db() };
	}
	async saveMessages(args) {
		...
		return { messages: new require_message_list.MessageList().add(messages, "memory").get.all.db() };
	}
	...
	async updateBufferedObservations(input) {
		...
		record.bufferedObservationChunks = [...Array.isArray(record.bufferedObservationChunks) ? record.bufferedObservationChunks : [], newChunk];
...
*/
var SchedulesStorage = class extends require_base$1.StorageDomain {
	constructor() {
		super({
			component: "STORAGE",
			name: "SCHEDULES"
...
var InMemorySchedulesStorage = class extends SchedulesStorage {
	...
	async recordTrigger(trigger) {
		const stored = {
			...trigger,
			id: trigger.id ?? (0, crypto$1.randomUUID)(),
...
var ScoresStorage = class extends require_base$1.StorageDomain {
	constructor() {
		super({
			component: "STORAGE",
			name: "SCORES"
...
var ScoresInMemory = class extends ScoresStorage {
	...
	async listScoresByScorerId({ scorerId, pagination, entityId, entityType, source, filters }) {
		...
		const perPage = require_filesystem_versioned.normalizePerPage(perPageInput, Number.MAX_SAFE_INTEGER);
		const { offset: start, perPage: perPageForResponse } = require_filesystem_versioned.calculatePagination(page, perPageInput, perPage);
...
*/
var ToolProviderConnectionsStorage = class extends require_base.MastraBase {
	constructor() {
		super({
			component: "STORAGE",
...
*/
var WorkflowDefinitionsStorage = class extends require_base$1.StorageDomain {
	constructor() {
		super({
			component: "STORAGE",
...
var WorkflowsStorage = class extends require_base$1.StorageDomain {
	constructor() {
		super({
			component: "STORAGE",
			name: "WORKFLOWS"
...
var WorkflowsInMemory = class extends WorkflowsStorage {
	...
	async listWorkflowRuns({ workflowName, fromDate, toDate, perPage, page, resourceId, status } = {}) {
		...
		if (perPage !== void 0 && page !== void 0) {
			const normalizedPerPage = require_filesystem_versioned.normalizePerPage(perPage, Number.MAX_SAFE_INTEGER);
...
* ```
*/
var InMemoryStore = class extends require_filesystem_versioned.MastraCompositeStore {
	stores;
	/**
	...
	*/
	...
	constructor({ id = "in-memory" } = {}) {
		...
		this.#db = new require_source.InMemoryDB();
		this.stores = {
			...
			agents: new require_source.InMemoryAgentsStorage({ db: this.#db }),
			channels: new InMemoryChannelsStorage(),
			notifications: new require_storage.InMemoryNotificationsStorage(),
			datasets: new DatasetsInMemory({ db: this.#db }),
			experiments: new ExperimentsInMemory({ db: this.#db }),
			promptBlocks: new require_filesystem$2.InMemoryPromptBlocksStorage({ db: this.#db }),
			scorerDefinitions: new require_filesystem$3.InMemoryScorerDefinitionsStorage({ db: this.#db }),
			mcpClients: new require_filesystem.InMemoryMCPClientsStorage({ db: this.#db }),
			mcpServers: new require_filesystem$1.InMemoryMCPServersStorage({ db: this.#db }),
			workspaces: new require_filesystem$5.InMemoryWorkspacesStorage({ db: this.#db }),
			skills: new require_filesystem$4.InMemorySkillsStorage({ db: this.#db }),
			favorites: new require_inmemory.InMemoryFavoritesStorage({ db: this.#db }),
			...
			threadState: new require_filesystem_versioned.InMemoryThreadStateStorage()
		};
	}
	/**
...
var FilesystemDB = class {
	...
	*/
	ensureDir() {
		if (!(0, fs.existsSync)(this.dir)) (0, fs.mkdirSync)(this.dir, { recursive: true });
		const skillsDir = (0, path.join)(this.dir, "skills");
	...
	*/
	readDomain(filename) {
		...
		const filePath = (0, path.join)(this.dir, filename);
		let data = {};
		if ((0, fs.existsSync)(filePath)) try {
	...
	*/
	writeDomain(filename, data) {
		...
		const parentDir = (0, path.dirname)(filePath);
		if (!(0, fs.existsSync)(parentDir)) (0, fs.mkdirSync)(parentDir, { recursive: true });
		(0, fs.writeFileSync)(tmpPath, JSON.stringify(data, null, 2), "utf-8");
		(0, fs.renameSync)(tmpPath, filePath);
	}
	/**
	...
	*/
	...
	listDomainFiles(directory, extension = ".json") {
		const baseDir = (0, path.resolve)(this.dir, directory);
		const rootDir = (0, path.resolve)(this.dir);
		if (!baseDir.startsWith(rootDir + path.sep) && baseDir !== rootDir) throw new Error(`Path traversal detected: directory "${directory}" escapes storage directory`);
		if (!(0, fs.existsSync)(baseDir)) return [];
		if (!(0, fs.statSync)(baseDir).isDirectory()) throw new Error(`Configured domain path "${directory}" is a file, expected a directory`);
		return (0, fs.readdirSync)(baseDir).filter((file) => (0, path.extname)(file) === extension && (0, fs.statSync)((0, path.join)(baseDir, file)).isFile()).map((file) => `${directory}/${file}`);
	}
	/**
	* Check whether a domain file currently exists on disk.
	*/
	domainFileExists(filename) {
		const filePath = (0, path.resolve)(this.dir, filename);
		const rootDir = (0, path.resolve)(this.dir);
		if (!filePath.startsWith(rootDir + path.sep) && filePath !== rootDir) throw new Error(`Path traversal detected: file "${filename}" escapes storage directory`);
		return (0, fs.existsSync)(filePath);
	}
	...
	*/
	skillDir(skillName) {
		const skillsBase = (0, path.join)(this.dir, "skills");
		const dir = (0, path.resolve)(skillsBase, skillName);
		if (!dir.startsWith(skillsBase + path.sep) && dir !== skillsBase) throw new Error(`Path traversal detected: skill name "${skillName}" escapes skills directory`);
		return dir;
	}
	/**
	...
	*/
	safeSkillPath(skillName, relativePath) {
		...
		const resolved = (0, path.resolve)(base, relativePath);
		if (!resolved.startsWith(base + path.sep) && resolved !== base) throw new Error(`Path traversal detected: "${relativePath}" escapes skill directory`);
		return resolved;
	}
	/**
	...
	*/
	listSkillFiles(skillName) {
		...
		return walkDir(dir).map((abs) => (0, path.relative)(dir, abs).split(path.sep).join("/"));
	}
	/**
	...
	*/
	readSkillFile(skillName, relativePath) {
		...
		if (!(0, fs.existsSync)(filePath)) return null;
		try {
			return (0, fs.readFileSync)(filePath);
		} catch {
			return null;
		}
	}
	...
	*/
	deleteSkillDir(skillName) {
		const dir = this.skillDir(skillName);
		if ((0, fs.existsSync)(dir)) (0, fs.rmSync)(dir, {
			recursive: true,
			force: true
		});
	}
...
function walkDir(dir) {
	const results = [];
	for (const entry of (0, fs.readdirSync)(dir)) {
		const fullPath = (0, path.join)(dir, entry);
		if ((0, fs.statSync)(fullPath).isDirectory()) results.push(...walkDir(fullPath));
...
* ```
*/
var FilesystemStore = class extends require_filesystem_versioned.MastraCompositeStore {
	#db;
	#dir;
	constructor(config = {}) {
		const dir = (0, path.resolve)(config.dir ?? ".mastra-storage");
		super({
			id: "filesystem",
		...
		this.stores = {
			agents: new require_source.FilesystemAgentsStorage({ db: this.#db }),
			promptBlocks: new require_filesystem$2.FilesystemPromptBlocksStorage({ db: this.#db }),
			scorerDefinitions: new require_filesystem$3.FilesystemScorerDefinitionsStorage({ db: this.#db }),
			mcpClients: new require_filesystem.FilesystemMCPClientsStorage({ db: this.#db }),
			mcpServers: new require_filesystem$1.FilesystemMCPServersStorage({ db: this.#db }),
			workspaces: new require_filesystem$5.FilesystemWorkspacesStorage({ db: this.#db }),
			skills: new require_filesystem$4.FilesystemSkillsStorage({ db: this.#db })
		};
	}
	/**
...
var StoreOperations = class extends require_base.MastraBase {
	constructor() {
		super({
			component: "STORAGE",
			name: "OPERATIONS"
...
Object.defineProperty(exports, "BackgroundTasksInMemory", {
	enumerable: true,
	get: function() {
		return BackgroundTasksInMemory;
	}
});
Object.defineProperty(exports, "BackgroundTasksStorage", {
	enumerable: true,
	get: function() {
		return BackgroundTasksStorage;
	}
});
...
Object.defineProperty(exports, "DatasetItemIdentityConflictError", {
	enumerable: true,
	get: function() {
		return DatasetItemIdentityConflictError;
	}
});
Object.defineProperty(exports, "DatasetsInMemory", {
	enumerable: true,
	get: function() {
		return DatasetsInMemory;
	}
});
...
Object.defineProperty(exports, "ExperimentsInMemory", {
	enumerable: true,
	get: function() {
		return ExperimentsInMemory;
	}
});
Object.defineProperty(exports, "ExperimentsStorage", {
	enumerable: true,
	get: function() {
		return ExperimentsStorage;
	}
});
Object.defineProperty(exports, "FactoryStorage", {
	enumerable: true,
	get: function() {
		return FactoryStorage;
	}
});
Object.defineProperty(exports, "FactoryStorageDomain", {
	enumerable: true,
	get: function() {
		return FactoryStorageDomain;
	}
});
...
Object.defineProperty(exports, "InMemoryChannelsStorage", {
	enumerable: true,
	get: function() {
		return InMemoryChannelsStorage;
	}
});
...
Object.defineProperty(exports, "InMemorySchedulesStorage", {
	enumerable: true,
	get: function() {
		return InMemorySchedulesStorage;
	}
});
...
Object.defineProperty(exports, "InMemoryToolProviderConnectionsStorage", {
	enumerable: true,
	get: function() {
		return InMemoryToolProviderConnectionsStorage;
	}
});
Object.defineProperty(exports, "InMemoryWorkflowDefinitionsStorage", {
	enumerable: true,
	get: function() {
		return InMemoryWorkflowDefinitionsStorage;
	}
});
...
Object.defineProperty(exports, "StoreOperationsInMemory", {
	enumerable: true,
	get: function() {
		return StoreOperationsInMemory;
	}
});
Object.defineProperty(exports, "ToolProviderConnectionsStorage", {
	enumerable: true,
	get: function() {
		return ToolProviderConnectionsStorage;
	}
});
Object.defineProperty(exports, "UniqueViolationError", {
	enumerable: true,
	get: function() {
		return UniqueViolationError;
	}
});
Object.defineProperty(exports, "WorkflowDefinitionsStorage", {
	enumerable: true,
	get: function() {
		return WorkflowDefinitionsStorage;
	}
});
Object.defineProperty(exports, "WorkflowsInMemory", {
	enumerable: true,
	get: function() {
		return WorkflowsInMemory;
	}
});
Object.defineProperty(exports, "WorkflowsStorage", {
	enumerable: true,
	get: function() {
		return WorkflowsStorage;
	}
});
Object.defineProperty(exports, "buildCreateSpanRecord", {
	enumerable: true,
	get: function() {
		return buildCreateSpanRecord;
	}
});
...
Object.defineProperty(exports, "createGitHubSourceControlProviderFromEnv", {
	enumerable: true,
	get: function() {
		return createGitHubSourceControlProviderFromEnv;
	}
});
Object.defineProperty(exports, "createStorageErrorId", {
	enumerable: true,
	get: function() {
		return createStorageErrorId;
	}
});
Object.defineProperty(exports, "createStoreErrorId", {
	enumerable: true,
	get: function() {
		return createStoreErrorId;
	}
});
...
Object.defineProperty(exports, "hasErrorCode", {
	enumerable: true,
	get: function() {
		return hasErrorCode;
	}
...
Object.defineProperty(exports, "mergeWorkflowStepResult", {
	enumerable: true,
	get: function() {
		return mergeWorkflowStepResult;
	}
});
Object.defineProperty(exports, "normalizeScheduleTarget", {
	enumerable: true,
	get: function() {
		return normalizeScheduleTarget;
	}
});
...
Object.defineProperty(exports, "require_fast_deep_equal", {
	enumerable: true,
	get: function() {
		return require_fast_deep_equal;
	}
});
...
Object.defineProperty(exports, "serializeSpanAttributes", {
	enumerable: true,
	get: function() {
		return serializeSpanAttributes;
	}
});
Object.defineProperty(exports, "storageMessageMatchesMetadataFilter", {
	enumerable: true,
	get: function() {
		return storageMessageMatchesMetadataFilter;
	}
});
...
Object.defineProperty(exports, "transformScoreRow", {
	enumerable: true,
	get: function() {
		return transformScoreRow;
	}
});
Object.defineProperty(exports, "validateStorageMetadataFilter", {
	enumerable: true,
	get: function() {
		return validateStorageMetadataFilter;
	}
});

//# sourceMappingURL=storage-DBpVHkrA.cjs.map