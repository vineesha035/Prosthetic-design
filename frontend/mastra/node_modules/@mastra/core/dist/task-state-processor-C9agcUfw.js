...
//#region src/agent/goal/objective.ts
...
const DEFAULT_GOAL_MAX_RUNS = 50;
/**
...
*/
const GOAL_SCORER_ID = "goal-scorer";
/**
...
*/
const DEFAULT_GOAL_JUDGE_PROMPT = `You are the goal judge. Your decision directly controls whether the assistant continues working toward the goal.
...
*/
function resolveEffectiveGoalSettings(record, agentDefaults) {
	return {
		judgeModelId: record?.judgeModelId ?? agentDefaults?.judgeModelId,
		maxRuns: record?.maxRuns ?? agentDefaults?.maxRuns ?? 50,
		prompt: record?.prompt ?? agentDefaults?.prompt ?? DEFAULT_GOAL_JUDGE_PROMPT,
		maxSteps: agentDefaults?.maxSteps
	};
}
function isThreadStateStore$2(value) {
	return !!value && typeof value.getState === "function" && typeof value.setState === "function" && typeof value.deleteState === "function";
}
/** Resolve the thread-scoped state store from a Mastra instance, if available. */
async function resolveGoalStore(mastra) {
	const store = await mastra?.getStorage?.()?.getStore("threadState");
	return isThreadStateStore$2(store) ? store : void 0;
}
/** Read the current objective record for a thread from the store. */
async function readObjective(store, threadId) {
	if (!store || !threadId) return void 0;
...
*/
async function writeObjective(store, threadId, record, requestContext) {
...
async function clearObjective(store, threadId, requestContext) {
	if (!store || !threadId) return;
	await store.deleteState({
		threadId,
		type: GOAL_STATE_TYPE
	});
	requestContext?.set(GOAL_REQUEST_CONTEXT_KEY, void 0);
}
function isGoalObjectiveRecord(value) {
	return !!value && typeof value.objective === "string" && typeof value.status === "string";
}
/**
...
*/
function getObjectiveFromRequestContext(requestContext) {
	if (!requestContext?.has?.("mastra:goal")) return void 0;
	const carried = requestContext.get(GOAL_REQUEST_CONTEXT_KEY);
	if (carried === void 0) return null;
	return isGoalObjectiveRecord(carried) ? carried : void 0;
}
//#endregion
//#region src/agent/goal/activity-cache.ts
const GOAL_OBJECTIVE_CACHE_KEY = "__mastra_goal_objective_cache";
function clearCachedGoalObjective(requestContext) {
	requestContext?.delete(GOAL_OBJECTIVE_CACHE_KEY);
}
function cacheGoalObjective(requestContext, threadId, objective) {
	requestContext?.set(GOAL_OBJECTIVE_CACHE_KEY, {
		threadId,
		objective: objective ?? null
	});
}
function takeCachedGoalObjective(requestContext, threadId) {
	const cached = requestContext?.get(GOAL_OBJECTIVE_CACHE_KEY);
	clearCachedGoalObjective(requestContext);
	return cached?.threadId === threadId ? cached : void 0;
}
//#endregion
//#region src/agent/goal/state-processor.ts
function renderObjective(record) {
	return `\n  ${record.objective}\n`;
}
function lp$1(value) {
	return `${value.length}:${value}`;
}
function stableObjectiveCacheKey(record, maxRuns) {
	return `goal:${lp$1(record.objective)}${lp$1(record.status)}${lp$1(String(record.runsUsed))}${lp$1(String(maxRuns))}`;
}
/**
...
*/
var GoalStateProcessor = class {
	id = "goal-state";
	stateId = GOAL_STATE_ID;
	mastra;
	__registerMastra(mastra) {
		this.mastra = mastra;
	}
	async resolveStore() {
		return resolveGoalStore(this.mastra);
	}
	getPriorObjective(args) {
		return (args.lastSnapshot?.metadata?.value)?.objective;
	}
	async computeStateSignal(args) {
		...
		let current;
		if (carried === null) current = void 0;
		else if (carried !== void 0) current = carried;
		else if (cached) current = cached.objective ?? void 0;
		else {
			const store = await this.resolveStore();
			current = store ? await store.getState({
		...
		if (!current || current.status !== "active") {
			...
			return {
				...
				contents: "\n",
				value: { objective: void 0 },
				attributes: { status: "none" },
				metadata: { value: { objective: void 0 } }
			};
		}
		const maxRuns = current.maxRuns ?? prior?.maxRuns ?? 0;
		const cacheKey = stableObjectiveCacheKey(current, maxRuns);
		const priorCacheKey = prior ? stableObjectiveCacheKey(prior, prior.maxRuns ?? 0) : void 0;
...
//#endregion
//#region src/tools/builtin/task-tools.ts
/** RequestContext key under which the current working task list is surfaced within a turn. */
const TASKS_REQUEST_CONTEXT_KEY = "mastra:tasks";
/** State-signal lane id used for the task list. */
const TASKS_STATE_ID = "tasks";
/** `threadState` storage `type` namespace under which the task list is stored. */
const TASK_STATE_TYPE = "task";
const NO_MEMORY_MESSAGE = "Task tools require agent memory (a memory-backed thread). No task was recorded. Configure the agent with Memory to use the task list.";
const taskIdSchema = z.string().min(1).describe("Stable task identifier (for example, 'task_investigate_tests'). Keep this unchanged across updates.");
...
const taskItemSchema = taskItemInputSchema.extend({ id: taskIdSchema });
...
function slugifyTaskContent(content) {
	...
	for (const char of content.toLowerCase()) {
		...
		if (code >= 97 && code <= 122 || code >= 48 && code <= 57) {
			if (pendingSeparator && slug.length > 0 && slug.length < TASK_ID_SLUG_MAX_LENGTH) slug += "_";
...
function assignTaskIds(tasks, previousTasks = []) {
	...
	for (const task of tasks) if (!task.id) omittedContentCounts.set(task.content, (omittedContentCounts.get(task.content) ?? 0) + 1);
	tasks.forEach((task, index) => {
		if (task.id || omittedContentCounts.get(task.content) !== 1) return;
		const previousMatches = previousTasks.filter((previous) => previous.content === task.content && !explicitTaskIds.has(previous.id));
		if (previousMatches.length === 1) reusablePreviousIds.set(index, previousMatches[0].id);
	});
	...
	return tasks.map((task, index) => {
		...
		const id = (task.id && !usedIds.has(task.id) ? task.id : void 0) ?? (reusablePreviousId && !usedIds.has(reusablePreviousId) ? reusablePreviousId : makeUniqueTaskId(fallbackId, usedIds, reservedIds));
		usedIds.add(id);
		return {
			id,
...
function formatTaskListResult(tasks) {
	...
	let summary = `Tasks updated: [${completed}/${tasks.length} completed]`;
...
function formatTaskCheckResult(taskCheck) {
	...
	if (!summary.hasTasks) return "No tasks found. Consider using task_write to create a task list for complex work.";
...
function demoteExtraInProgress(tasks, preferredIndex) {
	...
	return tasks.map((t, i) => t.status === "in_progress" && i !== keepIndex ? {
		...t,
		status: "pending"
	} : t);
}
...
function isMemoryBacked(agent) {
	return Boolean(agent?.threadId && agent?.resourceId);
}
function isThreadStateStore$1(value) {
	return !!value && typeof value.getState === "function" && typeof value.setState === "function";
}
/** Resolve the thread-scoped state store from the agent's Mastra storage, if available. */
async function resolveTaskStore(context) {
	const store = await context.mastra?.getStorage?.()?.getStore("threadState");
	return isThreadStateStore$1(store) ? store : void 0;
}
function emitTaskDisplayUpdate(requestContext, tasks) {
...
async function readTaskStore(context) {
...
*/
async function applyTaskMutation(context, mutation) {
	const store = await resolveTaskStore(context);
	const threadId = context.agent?.threadId;
	if (!store || !threadId) return noMemoryResult();
	const result = mutation(await readTaskStore(context));
	if (!result.isError) {
		await store.setState({
			threadId,
			type: TASK_STATE_TYPE,
...
*/
const taskWriteTool = createTool({
	id: "task_write",
...
- completed: Finished successfully`,
	inputSchema: z.object({ tasks: z.array(taskItemInputSchema).describe("The complete updated task list") }),
	...
	execute: async ({ tasks }, context) => {
		try {
			...
			return applyTaskMutation(context, (currentTasks) => {
				...
				if (hasMultipleInProgress(normalizedTasks)) return multipleInProgressError(currentTasks);
				return {
			...
			return {
				content: `Failed to update tasks: ${error instanceof Error ? error.message : "Unknown error"}`,
				tasks: [],
				isError: true
			};
		}
...
*/
const taskUpdateTool = createTool({
	id: "task_update",
...
- If the ID is unknown, the tool returns an error with available task IDs`,
	...
	}).refine((input) => input.content !== void 0 || input.status !== void 0 || input.activeForm !== void 0, { message: "Provide at least one field to update." }),
	outputSchema: taskToolResultSchema,
	execute: async ({ id, content, status, activeForm }, context) => {
		try {
			...
			return applyTaskMutation(context, (tasks) => {
				...
				const updatedTasks = demoteExtraInProgress(tasks.map((task, index) => index === taskIndex ? {
					...task,
...
*/
const taskCompleteTool = createTool({
	id: "task_complete",
	...
	execute: async ({ id }, context) => {
		try {
			...
			return applyTaskMutation(context, (tasks) => {
				...
				const updatedTasks = tasks.map((task, index) => index === taskIndex ? {
					...task,
					status: "completed"
				} : task);
				return {
			...
			return {
				content: `Failed to complete task: ${error instanceof Error ? error.message : "Unknown error"}`,
				tasks: [],
				isError: true
			};
		}
...
*/
const taskCheckTool = createTool({
	id: "task_check",
...
function isTaskItemArray(value) {
	return Array.isArray(value) && value.every((item) => item && typeof item === "object" && typeof item.id === "string" && typeof item.content === "string" && typeof item.status === "string" && typeof item.activeForm === "string");
}
/**
...
*/
function getTasksFromRequestContext(requestContext) {
	const carried = requestContext?.get(TASKS_REQUEST_CONTEXT_KEY);
	return isTaskItemArray(carried) ? carried : void 0;
}
//#endregion
//#region src/tools/builtin/task-state-processor.ts
function isThreadStateStore(value) {
	return !!value && typeof value.getState === "function";
}
function renderTaskList(tasks) {
	if (tasks.length === 0) return "";
	return `\n${tasks.map((task) => {
		return `  ${task.status === "completed" ? "✓" : task.status === "in_progress" ? "▸" : "○"} [${task.status}] {id: ${task.id}} ${task.content}`;
	}).join("\n")}\n`;
}
const DELTA_SNAPSHOT_CAP = 10;
function getTasksFromSnapshot(snapshot) {
	const tasks = (snapshot?.metadata?.value)?.tasks;
	if (Array.isArray(tasks)) return tasks;
	return [];
}
function getOpsFromDelta(signal) {
	const ops = (signal?.metadata?.delta)?.ops;
...
function effectivePriorTasks(args) {
	let tasks = getTasksFromSnapshot(args.lastSnapshot);
	for (const delta of args.deltasSinceSnapshot ?? []) tasks = applyOps(tasks, getOpsFromDelta(delta));
	return tasks;
}
function lp(value) {
	return `${value.length}:${value}`;
...
function stableTasksCacheKey(tasks) {
	return `tasks:${tasks.map(taskFingerprint).join("|")}`;
}
function diffTasks(prior, current) {
	const ops = [];
	...
	for (const task of current) {
		...
		else if (taskFingerprint(before) !== taskFingerprint(task)) ops.push({
			op: "update",
			task
		});
	}
	for (const task of prior) if (!currentIds.has(task.id)) ops.push({
		op: "remove",
		id: task.id
	});
	return ops;
...
function renderDelta(ops) {
	return `\n${ops.map((op) => {
		...
		return `  ${op.op === "add" ? "+" : icon} {id: ${task.id}} [${task.status}] ${task.content}`;
	}).join("\n")}\n`;
}
/**
...
*/
var TaskStateProcessor = class {
	id = "task-state";
	stateId = TASKS_STATE_ID;
	/**
	...
	*/
	...
	async computeStateSignal(args) {
		...
		if (!hasBase || deltaCount >= DELTA_SNAPSHOT_CAP) return {
			id: TASKS_STATE_ID,
...
//#endregion
export { GOAL_STATE_TYPE as C, resolveEffectiveGoalSettings as D, readObjective as E, resolveGoalStore as O, GOAL_STATE_ID as S, getObjectiveFromRequestContext as T, clearCachedGoalObjective as _, assignTaskIds as a, GOAL_SCORER_ID as b, getTasksFromRequestContext as c, taskCheckTool as d, taskCompleteTool as f, cacheGoalObjective as g, GoalStateProcessor as h, TASK_STATE_TYPE as i, writeObjective as k, hasMultipleInProgress as l, taskWriteTool as m, TASKS_REQUEST_CONTEXT_KEY as n, demoteExtraInProgress as o, taskUpdateTool as p, TASKS_STATE_ID as r, formatTaskListResult as s, TaskStateProcessor as t, summarizeTaskCheck as u, DEFAULT_GOAL_JUDGE_PROMPT as v, clearObjective as w, GOAL_SCORE_WAITING as x, DEFAULT_GOAL_MAX_RUNS as y };

//# sourceMappingURL=task-state-processor-C9agcUfw.js.map