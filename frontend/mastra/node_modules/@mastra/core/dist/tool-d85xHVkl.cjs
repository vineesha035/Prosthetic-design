...
function safeValidate(schema, data) {
	try {
		...
		if (result instanceof Promise) throw new Error("Your schema is async, which is not supported. Please use a sync schema.");
		if ("issues" in result && Array.isArray(result.issues) && result.issues.length > 0) return { issues: result.issues };
		return result;
	} catch (err) {
		if (err instanceof TypeError && err.message.includes("Cannot read properties of undefined")) throw new Error(`Schema validation failed due to an invalid schema definition. This often happens when a union schema (z.union or z.or) has undefined options. Please check that all schema options are properly defined. Original error: ${err.message}`);
		throw err;
	}
}
...
function buildFormattedErrors(issues) {
	...
	for (const issue of issues) if (!issue.path || issue.path.length === 0) result.errors.push(issue.message);
	else {
		let current = result;
		for (let i = 0; i < issue.path.length; i++) {
			const key = getPathKey(issue.path[i]);
			if (i === issue.path.length - 1) {
				if (!current.fields[key]) current.fields[key] = createEmptyErrors();
...
function validateToolSuspendData(schema, suspendData, toolId) {
	if (!schema || !("~standard" in schema)) return { data: suspendData };
	const validation = safeValidate(schema, suspendData);
	if ("value" in validation) return { data: validation.value };
	const errorMessages = validation.issues.map((e) => `- ${e.path?.map((p) => getPathKey(p)).join(".") || "root"}: ${e.message}`).join("\n");
	return { error: {
		error: true,
...
function normalizeNullishInput(schema, input) {
	...
	const jsonSchema = (0, _mastra_schema_compat_schema.standardSchemaToJSONSchema)(schema, { io: "input" });
...
function convertUndefinedToNull(input) {
	...
	if (Array.isArray(input)) return input.map(convertUndefinedToNull);
	if (!isPlainObject(input)) return input;
	const result = {};
	for (const [key, value] of Object.entries(input)) result[key] = convertUndefinedToNull(value);
	return result;
}
/**
...
function stripNullishValues(input) {
	...
	if (Array.isArray(input)) return input.map((item) => item === null ? null : stripNullishValues(item));
	if (!isPlainObject(input)) return input;
	const result = {};
...
function stripNullishValuesAtPaths(input, paths, currentPath = "") {
	if (input === null || input === void 0) return paths.has(currentPath) ? void 0 : input;
	if (typeof input !== "object") return input;
	if (Array.isArray(input)) return input.map((item, i) => stripNullishValuesAtPaths(item, paths, currentPath ? `${currentPath}.${i}` : String(i)));
	if (!isPlainObject(input)) return input;
	const result = {};
	for (const [key, value] of Object.entries(input)) {
		const fieldPath = currentPath ? `${currentPath}.${key}` : key;
		if ((value === null || value === void 0) && paths.has(fieldPath)) continue;
...
function getValueAtPath(obj, pathSegments) {
	...
	for (const segment of pathSegments) {
		if (current === null || current === void 0 || typeof current !== "object") return PATH_NOT_FOUND;
...
function coerceStringifiedJsonValues(schema, input) {
	if (!isPlainObject(input)) return input;
	const unwrapped = require_zod_utils.unwrapZodType(schema);
	if (!require_zod_utils.isZodObject(unwrapped)) return input;
	...
	for (const [key, value] of Object.entries(input)) {
		...
		const baseFieldSchema = require_zod_utils.unwrapZodType(fieldSchema);
		if (require_zod_utils.getZodTypeName(baseFieldSchema) === "ZodString") continue;
		const trimmed = value.trim();
		if (require_zod_utils.isZodArray(baseFieldSchema) && trimmed.startsWith("[") || require_zod_utils.isZodObject(baseFieldSchema) && trimmed.startsWith("{")) try {
			const parsed = JSON.parse(value);
			if (require_zod_utils.isZodArray(baseFieldSchema) && Array.isArray(parsed) || require_zod_utils.isZodObject(baseFieldSchema) && isPlainObject(parsed)) {
				result[key] = parsed;
				changed = true;
			}
		} catch {}
...
function validateToolInput(schema, input, toolId) {
	if (!schema || !("~standard" in schema)) return { data: input };
	schema = (0, _mastra_schema_compat_schema.toStandardSchema)(schema);
	...
	const failingNullPaths = new Set(validation.issues.filter((issue) => {
	...
	const promptJsonSchema = (0, _mastra_schema_compat_schema.standardSchemaToJSONSchema)(schema, { io: "input" });
	if (promptJsonSchema.type === "object" && promptJsonSchema.properties != null && "prompt" in promptJsonSchema.properties && normalizedInput != null && typeof normalizedInput === "object" && !Array.isArray(normalizedInput)) {
		...
		if (obj.prompt == null) {
			...
			if (alias !== void 0) {
				const coercedPromptInput = {
					...obj,
					prompt: alias
				};
				const coercedPromptValidation = safeValidate(schema, coercedPromptInput);
				if ("value" in coercedPromptValidation) return { data: coercedPromptValidation.value };
			}
		}
	}
	const errorMessages = validation.issues.map((e) => `- ${e.path?.map((p) => getPathKey(p)).join(".") || "root"}: ${e.message}`).join("\n");
...
function validateToolOutput(schema, output, toolId, suspendCalled) {
	if (!schema || !("~standard" in schema) || suspendCalled) return { data: output };
...
function redactSensitiveKeys(obj) {
	...
	const result = {};
	for (const [key, value] of Object.entries(obj)) if (SENSITIVE_KEYS.some((sensitive) => key.toLowerCase().includes(sensitive.toLowerCase()))) result[key] = "[REDACTED]";
	else if (typeof value === "object" && value !== null) result[key] = redactSensitiveKeys(value);
...
function validateRequestContext(schema, requestContext, identifier) {
	if (!schema) return { data: requestContext?.all ?? {} };
	const contextValues = requestContext?.all ?? {};
	const validation = (0, _mastra_schema_compat_schema.toStandardSchema)(schema)["~standard"].validate(contextValues);
	if (validation instanceof Promise) throw new Error("Your schema is async, which is not supported. Please use a sync schema.");
	if ("value" in validation) return { data: validation.value };
	const errorMessages = validation.issues.map((e) => `- ${e.path?.map((p) => getPathKey(p)).join(".") || "root"}: ${e.message}`).join("\n");
...
var Tool = class {
	...
	*/
	constructor(opts) {
		...
		this.inputSchema = opts.inputSchema ? (0, _mastra_schema_compat_schema.toStandardSchema)(opts.inputSchema) : void 0;
		this.outputSchema = opts.outputSchema ? (0, _mastra_schema_compat_schema.toStandardSchema)(opts.outputSchema) : void 0;
		this.suspendSchema = opts.suspendSchema ? (0, _mastra_schema_compat_schema.toStandardSchema)(opts.suspendSchema) : void 0;
		this.resumeSchema = opts.resumeSchema ? (0, _mastra_schema_compat_schema.toStandardSchema)(opts.resumeSchema) : void 0;
		...
		if (opts.execute) {
			...
			this.execute = async (inputData, context) => {
				...
				const { error: requestContextError } = validateRequestContext(this.requestContextSchema, context?.requestContext, this.id);
				if (requestContextError) return requestContextError;
				let suspendData = null;
				const baseContext = context ? {
					...context,
					...context.suspend ? { suspend: (args, suspendOptions) => {
						suspendData = args;
						return context.suspend?.(args, suspendOptions);
					} } : {}
				} : {};
				let organizedContext = baseContext;
				if (!context) organizedContext = {
					...
					if (isAgentExecution && !baseContext.agent) {
						const { agentId, toolCallId, messages, suspend, resumeData, threadId, resourceId, writableStream, ...rest } = baseContext;
						organizedContext = {
							...rest,
							agent: {
								agentId: agentId || "",
							...
							requestContext: rest.requestContext || new require_request_context.RequestContext()
						};
						...
						requestContext: baseContext.requestContext || new require_request_context.RequestContext()
					};
				}
				...
				if (resumeData) {
					...
					if (resumeValidation.error) return resumeValidation.error;
				}
				...
				if (suspendData) {
					...
					if (suspendValidation.error) return suspendValidation.error;
				}
				...
				if (outputValidation.error) return outputValidation.error;
...
//#endregion
Object.defineProperty(exports, "MASTRA_TOOL_MARKER", {
	enumerable: true,
	get: function() {
		return MASTRA_TOOL_MARKER;
	}
});
...
Object.defineProperty(exports, "validateToolOutput", {
	enumerable: true,
	get: function() {
		return validateToolOutput;
	}
});
Object.defineProperty(exports, "validateToolSuspendData", {
	enumerable: true,
	get: function() {
		return validateToolSuspendData;
	}
});

//# sourceMappingURL=tool-d85xHVkl.cjs.map