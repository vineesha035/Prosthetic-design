...
var ProcessorRunner = class ProcessorRunner {
			...
			try {
				...
				const processMethodArgs = {
					...
					sendStateSignal: async (stateSignal) => {
						...
						if (!resolvedMemory || !resolvedThreadId || !resolvedResourceId) throw new Error(`[Processor:${processor.id}] sendStateSignal requires Mastra memory with an active resourceId and threadId`);
						const loadedThread = await resolvedMemory.getThreadById({ threadId: resolvedThreadId }) ?? memoryContext?.thread;
						if (!loadedThread) throw new Error(`[Processor:${processor.id}] sendStateSignal could not load thread ${resolvedThreadId}`);
				...
				const result = processMethod ? await ProcessorRunner.validateAndFormatProcessInputStepResult(await processMethod(processMethodArgs), {
					messageList,
					processor,
					stepNumber
				}) : {};
				const { messages, systemMessages, ...rest } = result;
				if (messages) ProcessorRunner.applyMessagesToMessageList(messages, messageList, idsBeforeProcessing, check);
				if (systemMessages) messageList.replaceAllSystemMessages(systemMessages);
		...
		for (const processorOrWorkflow of this.inputProcessors) {
			...
			try {
				...
				if (result && typeof result === "object") {
					if (Object.prototype.hasOwnProperty.call(result, "prompt")) currentPrompt = result.prompt;
					if (result.response && !cachedResponse) cachedResponse = result.response;
				}
			} catch (error) {
				if (error instanceof TripWire) await invokeOnViolation(processor, error);
				throw error;
			}
		}
		return {
	...
	*/
	async runProcessOutputStep(args) {
		const { steps, messageList, stepNumber, finishReason, providerMetadata, toolCalls, text, usage, requestContext, retryCount = 0, writer } = args;
		const observabilityContext = resolveObservabilityContext(args);
		for (const [index, processorOrWorkflow] of this.outputProcessors.entries()) {
			const processableMessages = messageList.get.all.db();
			const idsBeforeProcessing = processableMessages.map((m) => m.id);
				...
				if (result instanceof MessageList) {
					...
					for (const message of result) {
						...
						if (message.role === "system") {
							...
							messageList.addSystem(systemText);
						} else messageList.add(message, check.getSource(message) || "response", { merge: false });
					}
				}
				processorSpan?.end({
					output: {
	...
	*/
	async runProcessToolResult(args) {
		const { steps, messageList, stepNumber, toolName, toolCallId, toolArgs, result, providerExecuted, requestContext, retryCount = 0, writer, abortSignal } = args;
		const observabilityContext = resolveObservabilityContext(args);
		for (const [index, processorOrWorkflow] of this.outputProcessors.entries()) {
			const processableMessages = messageList.get.all.db();
			const idsBeforeProcessing = processableMessages.map((m) => m.id);
			...
			if (isProcessorWorkflow(processorOrWorkflow)) {
				...
				await this.executeWorkflowAsProcessor(processorOrWorkflow, {
					...
					steps,
					retryCount
				}, observabilityContext, requestContext, writer, abortSignal);
				continue;
			}
			const processor = processorOrWorkflow;
			...
			try {
				...
				if (processorResult instanceof MessageList) {
					if (processorResult !== messageList) throw new MastraError({
						category: "USER",
						domain: "AGENT",
						id: "PROCESSOR_RETURNED_EXTERNAL_MESSAGE_LIST",
						text: `Processor ${processor.id} returned a MessageList instance other than the one that was passed in as an argument. New external message list instances are not supported. Use the messageList argument instead.`
					});
				} else if (processorResult) ProcessorRunner.applyMessagesToMessageList(processorResult, messageList, idsBeforeProcessing, check, "response");
				processorSpan?.end({
					output: {
						...!areProcessorMessageArraysEqual(processableMessages, messageList.get.all.db()) ? { messages: messageList.get.all.db() } : {},
						...!areProcessorMessageArraysEqual(currentSystemMessages, messageList.getAllSystemMessages()) ? { systemMessages: messageList.getAllSystemMessages() } : {}
					},
					attributes: mutations.length > 0 ? { messageListMutations: mutations } : void 0
				});
			} catch (error) {
	...
	async runProcessAPIError(args) {
		...
		for (const [index, processorOrWorkflow] of allProcessors.entries()) {
			...
			try {
				...
				const output = { retry: result?.retry ?? false };
				if (!areProcessorMessageArraysEqual(processableMessages, messagesAfter)) output.messages = messagesAfter;
				if (!areProcessorMessageArraysEqual(systemMessagesBefore, systemMessagesAfter)) output.systemMessages = systemMessagesAfter;
				if (messageIdAfter !== messageIdBefore) output.messageId = messageIdAfter;
				processorSpan?.end({
					output,
					attributes: mutations.length > 0 ? { messageListMutations: mutations } : void 0
				});
				...
				this.logger.error(`[Agent:${this.agentName}] - Request error processor ${processor.id} failed:`, processorError);
			}
		}
		return { retry: false };
	}
	static applyMessagesToMessageList(messages, messageList, idsBeforeProcessing, check, defaultSource = "input") {
		...
		for (const message of messages) {
			...
			if (message.role === "system") {
				const systemText = message.content.content ?? message.content.parts?.map((p) => p.type === "text" ? p.text : "").join("\n") ?? "";
				messageList.addSystem(systemText);
			} else messageList.add(message, check.getSource(message) || defaultSource, { merge: false });
		}
	}
	static async validateAndFormatProcessInputStepResult(result, { messageList, processor, stepNumber }) {
		if (result instanceof MessageList) {
			if (result !== messageList) throw new MastraError({
				category: "USER",
				domain: "AGENT",
		...
		} else if (Array.isArray(result)) return { messages: result };
		else if (result) {
			if (result.messageList && result.messageList !== messageList) throw new MastraError({
				category: "USER",
				domain: "AGENT",
				id: "PROCESSOR_RETURNED_EXTERNAL_MESSAGE_LIST",
				text: `Processor ${processor.id} returned a MessageList instance other than the one that was passed in as an argument. New external message list instances are not supported. Use the messageList argument instead.`
			});
			if (result.messages && result.messageList) throw new MastraError({
				category: "USER",
				domain: "AGENT",
			...
			if (result.model) {
				...
				if (!isSupportedLanguageModel(resolvedModel)) throw new MastraError({
					category: "USER",
					domain: "AGENT",
...
//#endregion
//#region src/stream/base/input.ts
/**
...
var MastraModelInput = class extends MastraBase {
	initialize({ runId, createStream, onResult }) {
		const self = this;
		let outputStream;
		outputStream = new ReadableStream({ async start(controller) {
			try {
				...
				if (initialChunks) for (const chunk of Array.isArray(initialChunks) ? initialChunks : [initialChunks]) controller.enqueue(chunk);
...
//#endregion
//#region src/stream/base/schema.ts
function asJsonSchema(schema) {
	if (!schema) return;
	if (isStandardSchemaWithJSON(schema)) return standardSchemaToJSONSchema(schema, {
		io: "input",
		target: "draft-07"
	});
	return schema;
...
function getTransformedSchema(schema, options) {
	...
	if (itemSchema.enum && Array.isArray(itemSchema.enum)) return {
		jsonSchema: {
			$schema,
			type: "object",
			properties: { result: {
...
function getResponseFormat(schema, options) {
	if (schema) return {
		type: "json",
		schema: getTransformedSchema(schema, options)?.jsonSchema
	};
	return { type: "text" };
}
//#endregion
//#region src/stream/base/output-format-handlers.ts
/**
...
function extractBalancedJsonObject(text) {
	...
	for (let i = 0; i < text.length; i++) {
		...
		if (char === "}" && depth > 0) {
			depth--;
			if (depth === 0 && start !== -1) lastBalanced = text.slice(start, i + 1);
		}
	}
	return lastBalanced;
}
...
var BaseFormatHandler = class {
	...
	*/
	async validateValue(value) {
		...
		if (this.isZodSchema(this.schema)) try {
			const ssResult = await this.schema["~standard"].validate(value);
			if (!ssResult.issues) return {
				success: true,
			...
			return {
				...
				error: new MastraError({
					...
					details: { value: typeof value === "object" ? JSON.stringify(value) : String(value) }
				}, zodError)
			};
		} catch (error) {
			return {
	...
	*/
	preprocessText(accumulatedText) {
		...
		if (/^```json\b/.test(trimmedStart)) {
			...
			if (match && match[1]) processedText = match[1].trim();
			else processedText = trimmedStart.replace(/^```json\s*\n?/, "");
		}
		const extractedObject = extractBalancedJsonObject(processedText);
		if (extractedObject) processedText = extractedObject;
...
var ObjectFormatHandler = class extends BaseFormatHandler {
	...
	async processPartialChunk({ accumulatedText, previousObject }) {
		const { value: currentObjectJson, state } = await parsePartialJson(this.preprocessText(accumulatedText));
		if (this.validatePartialChunks && this.partialSchema) {
			const result = this.partialSchema?.safeParse(currentObjectJson);
			if (result.success && result.data && result.data !== void 0 && !isDeepEqualData(previousObject, result.data)) return {
				shouldEmit: true,
		...
		if (currentObjectJson !== void 0 && currentObjectJson !== null && typeof currentObjectJson === "object" && !isDeepEqualData(previousObject, currentObjectJson)) return {
	...
	async validateAndTransformFinal(finalRawValue) {
		if (!finalRawValue) return {
			success: false,
			error: /* @__PURE__ */ new Error("No object generated: could not parse the response.")
		};
...
var ArrayFormatHandler = class extends BaseFormatHandler {
	...
	async processPartialChunk({ accumulatedText, previousObject }) {
		const { value: currentObjectJson, state: parseState } = await parsePartialJson(this.preprocessText(accumulatedText));
		if (currentObjectJson !== void 0 && !isDeepEqualData(previousObject, currentObjectJson)) {
			...
			for (let i = 0; i < rawElements.length; i++) {
				...
				if (i === rawElements.length - 1 && parseState !== "successful-parse") {
					if (element && typeof element === "object" && Object.keys(element).length > 0) filteredElements.push(element);
				} else if (element && typeof element === "object" && Object.keys(element).length > 0) filteredElements.push(element);
			}
...
var EnumFormatHandler = class extends BaseFormatHandler {
	...
	*/
	findBestEnumMatch(partialResult) {
		if (!this.schema) return;
		const enumValues = standardSchemaToJSONSchema(this.schema)?.enum;
		...
		if (possibleEnumValues.length === 0) return;
		const firstMatch = possibleEnumValues[0];
		return possibleEnumValues.length === 1 && firstMatch !== void 0 ? firstMatch : partialResult;
	}
	async processPartialChunk({ accumulatedText, previousObject }) {
		const { value: currentObjectJson } = await parsePartialJson(this.preprocessText(accumulatedText));
	...
	async validateAndTransformFinal(rawFinalValue) {
		const { value } = await parsePartialJson(this.preprocessText(rawFinalValue));
		if (!(typeof value === "object" && value !== null && "result" in value)) return {
			success: false,
			error: /* @__PURE__ */ new Error("Invalid enum format: expected object with result property")
		};
		const finalValue = value;
		if (!finalValue || typeof finalValue !== "object" || typeof finalValue.result !== "string") return {
			success: false,
			error: /* @__PURE__ */ new Error("Invalid enum format: expected object with result property")
		};
...
function createOutputHandler({ schema }) {
	...
	switch (getTransformedSchema(normalizedSchema)?.outputFormat) {
		case "array": return new ArrayFormatHandler(normalizedSchema);
		case "enum": return new EnumFormatHandler(normalizedSchema);
		default: return new ObjectFormatHandler(normalizedSchema);
	}
}
/**
...
function createObjectStreamTransformer({ structuredOutput, logger }) {
	...
	return new TransformStream({
		async transform(chunk, controller) {
			...
			if (chunk.type === "text-end") {
				...
				if (accumulatedText?.trim() && !finalResult) {
					...
					if (finalResult.success) controller.enqueue({
						from: "AGENT",
						runId: currentRunId ?? "",
						type: "object-result",
		...
		async flush(controller) {
			if (finalResult && !finalResult.success) handleValidationError(finalResult.error, controller);
			if (accumulatedText?.trim() && !finalResult) {
				finalResult = await handler.validateAndTransformFinal(accumulatedText);
				if (finalResult.success) controller.enqueue({
					from: "AGENT",
				...
				else handleValidationError(finalResult.error, controller);
			}
		}
	});
	/**
	...
	*/
	function handleValidationError(error, controller) {
		if (structuredOutput?.errorStrategy === "warn") logger?.warn(error.message);
		else if (structuredOutput?.errorStrategy === "fallback") controller.enqueue({
			from: "AGENT",
			runId: currentRunId ?? "",
			type: "object-result",
...
function createJsonTextStreamTransformer(schema) {
	...
	return new TransformStream({
		transform(chunk, controller) {
			if (chunk.type !== "object" || !chunk.object) return;
			if (outputSchema?.outputFormat === "array" && Array.isArray(chunk.object)) {
				...
				for (let i = previousArrayLength; i < chunk.object.length; i++) {
					...
					if (i > 0) controller.enqueue("," + elementJson);
					else controller.enqueue(elementJson);
				}
				previousArrayLength = chunk.object.length;
			} else controller.enqueue(JSON.stringify(chunk.object));
		},
		flush(controller) {
			if (hasStartedArray && outputSchema?.outputFormat === "array" && chunkCount > 1) controller.enqueue("]");
		}
	});
}
//#endregion
//#region src/stream/base/output.ts
...
function createDestructurableOutput(output) {
	return new Proxy(output, { get(target, prop, _receiver) {
		...
		if (typeof originalValue === "function") return originalValue.bind(target);
		return originalValue;
	} });
}
function persistProcessorDataChunk(messageList, messageId, chunk) {
...
*/
function resolveOutputTextSkippingCompletionChecks(messageList) {
	const responseDbMessages = messageList.get.response.db();
	if (responseDbMessages.some((m) => m.content?.metadata?.completionResult)) {
...
var MastraModelOutput = class extends MastraBase {
	...
	*/
	...
	constructor({ model: _model, stream, messageList, options, messageId, initialState }) {
		...
		if (options.structuredOutput?.schema) this.#structuredOutputMode = options.structuredOutput.model ? "processor" : "direct";
		if (options.outputProcessors?.length) this.processorRunner = new ProcessorRunner({
			inputProcessors: [],
		...
		if (processorRunner && options.isLLMExecutionStep) {
			...
			processedStream = stream.pipeThrough(new TransformStream({ async transform(chunk, controller) {
				if (chunk.type === "finish" && chunk.payload?.stepResult?.reason === "tool-calls") {
					...
					*/
					if (!processorStates.has(STRUCTURED_OUTPUT_PROCESSOR_NAME)) {
						const processorIndex = processorRunner.outputProcessors.findIndex((p) => p.id === STRUCTURED_OUTPUT_PROCESSOR_NAME);
						if (processorIndex !== -1) {
						...
						if (structuredOutputProcessorState) structuredOutputProcessorState.customState.controller = controller;
					}
					const streamWriter = { custom: async (data, writerOptions) => {
						persistProcessorDataChunk(self.messageList, writerOptions?.messageId ?? self.messageId, data);
						controller.enqueue(data);
					} };
					const { part: processed, blocked, reason, tripwireOptions, processorId } = await processorRunner.processPart(chunk, processorStates, resolveObservabilityContext(options), options.requestContext, self.messageList, 0, streamWriter);
					const enqueueTripwire = (r, opts, pid) => {
						controller.enqueue({
							type: "tripwire",
							payload: {
					...
					if (processed) controller.enqueue(processed);
					const reprocessed = await processorRunner.drainReprocessParts(processorStates, resolveObservabilityContext(options), options.requestContext, self.messageList, 0, streamWriter);
					for (const r of reprocessed) {
						if (r.blocked) {
							enqueueTripwire(r.reason, r.tripwireOptions, r.processorId);
							return;
		...
		if (self.#structuredOutputMode === "direct" && self.#options.isLLMExecutionStep) processedStream = processedStream.pipeThrough(createObjectStreamTransformer({
			structuredOutput: self.#options.structuredOutput,
			logger: self.logger
		}));
		this.#baseStream = processedStream.pipeThrough(new TransformStream({
			transform: async (chunk, controller) => {
				switch (chunk.type) {
					...
					case "abort":
						...
						break;
					case "raw":
						if (!self.#options.includeRawChunks) return;
						break;
					case "object-result":
						self.#bufferedObject = chunk.object;
						if (self.#delayedPromises.object.status.type === "pending") self.#delayedPromises.object.resolve(chunk.object);
						break;
					case "source":
					...
					case "tool-call-delta":
						if (!self.#toolCallArgsDeltas[chunk.payload.toolCallId]) self.#toolCallArgsDeltas[chunk.payload.toolCallId] = [];
					...
					case "reasoning-delta": {
						...
						if (bufferedReasoning) {
							...
							if (chunk.payload.providerMetadata) bufferedReasoning.payload.providerMetadata = chunk.payload.providerMetadata;
						}
						break;
					}
					case "reasoning-end": {
						const bufferedReasoning = self.#bufferedReasoningDetails[chunk.payload.id];
						if (chunk.payload.providerMetadata && bufferedReasoning) bufferedReasoning.payload.providerMetadata = chunk.payload.providerMetadata;
						break;
					}
					case "tool-call": {
						const existingSynthetic = self.#toolCalls.find((tc) => tc.payload.toolCallId === chunk.payload.toolCallId);
						if (existingSynthetic) {
							if (chunk.payload.args && Object.keys(existingSynthetic.payload.args || {}).length === 0) existingSynthetic.payload.args = chunk.payload.args;
							if (chunk.payload.providerMetadata && !existingSynthetic.payload.providerMetadata) existingSynthetic.payload.providerMetadata = chunk.payload.providerMetadata;
							if (chunk.payload.dynamic != null && existingSynthetic.payload.dynamic == null) existingSynthetic.payload.dynamic = chunk.payload.dynamic;
							if (chunk.payload.observability && !existingSynthetic.payload.observability) existingSynthetic.payload.observability = chunk.payload.observability;
							return;
						}
						...
						if (toolCallPayload?.output?.from === "AGENT" && toolCallPayload?.output?.type === "finish") {
							...
							if (finishPayload?.usage) self.updateUsageCount(finishPayload.usage);
						}
						break;
					}
					case "tool-result":
					...
					case "step-finish": {
						...
						if (chunk.payload.metadata.request) self.#request = chunk.payload.metadata.request;
						const { providerMetadata, request, ...otherMetadata } = chunk.payload.metadata;
						const payloadSteps = chunk.payload.output?.steps || [];
						const stepTripwire = payloadSteps[payloadSteps.length - 1]?.tripwire;
						...
						const stepResult = {
							...
							get staticToolCalls() {
								return self.#bufferedByStep.toolCalls.filter((part) => part.type === "tool-call" && part.payload?.dynamic === false);
							},
							get dynamicToolCalls() {
								return self.#bufferedByStep.toolCalls.filter((part) => part.type === "tool-call" && part.payload?.dynamic === true);
							},
							get staticToolResults() {
								return self.#bufferedByStep.toolResults.filter((part) => part.type === "tool-result" && part.payload?.dynamic === false);
							},
							get dynamicToolResults() {
								return self.#bufferedByStep.toolResults.filter((part) => part.type === "tool-result" && part.payload?.dynamic === true);
							},
					...
					case "tripwire":
						...
						self.resolvePromises({
							...
							warnings: self.#warnings,
							providerMetadata: void 0,
							response: { dbMessages: self.messageList.get.response.db() },
							request: {},
							reasoning: [],
						...
						return;
					case "finish":
						if (self.#status !== "failed" && self.#status !== "canceled") self.#status = "success";
						if (chunk.payload.stepResult.reason) self.#finishReason = chunk.payload.stepResult.reason;
						const finalProviderMetadata = chunk.payload.metadata?.providerMetadata ?? chunk.payload.providerMetadata;
						if (finalProviderMetadata) self.#stepProviderMetadata = finalProviderMetadata;
						if (chunk.payload.stepResult.reason === "tripwire") {
							const outputSteps = chunk.payload.output?.steps;
							const stepTripwire = (outputSteps?.[outputSteps?.length - 1])?.tripwire;
							self.#tripwire = {
						...
						if (self.#bufferedObject !== void 0) {
							const lastAssistantMessage = [...messageList.get.response.db()].reverse().find((m) => m.role === "assistant");
							if (lastAssistantMessage) {
								if (!lastAssistantMessage.content.metadata) lastAssistantMessage.content.metadata = {};
						...
						chunk.payload.output.usage = {
							...
							...self.#usageCount.reasoningTokens !== void 0 && { reasoningTokens: self.#usageCount.reasoningTokens },
							...self.#usageCount.cachedInputTokens !== void 0 && { cachedInputTokens: self.#usageCount.cachedInputTokens },
							...self.#usageCount.cacheCreationInputTokens !== void 0 && { cacheCreationInputTokens: self.#usageCount.cacheCreationInputTokens },
							...self.#usageCount.raw !== void 0 && { raw: self.#usageCount.raw }
						};
						try {
							if (self.processorRunner && !self.#options.isLLMExecutionStep) {
								...
								const outputResultWriter = { custom: async (data, writerOptions) => {
									persistProcessorDataChunk(self.messageList, writerOptions?.messageId ?? self.messageId, data);
									self.#emitChunk(data);
									controller.enqueue(data);
								} };
								...
								if (self.#status !== "failed" && self.#status !== "canceled") self.messageList = await self.processorRunner.runOutputProcessors(self.messageList, resolveObservabilityContext(options), self.#options.requestContext, 0, outputResultWriter, outputResult);
								const outputText = resolveOutputTextSkippingCompletionChecks(self.messageList);
								if (lastStep && outputText && outputText !== originalText) lastStep.text = outputText;
								...
								const lastStep = self.#bufferedSteps[self.#bufferedSteps.length - 1];
								const hasToolStep = self.#bufferedSteps.some((step) => step.toolCalls.length > 0 || step.toolResults.length > 0);
								this.resolvePromises({
							...
							if (error instanceof TripWire) {
								...
								self.#error = getErrorFromUnknown(error, { fallbackMessage: "Unknown error in stream" });
								self.resolvePromises({
									finishReason: "error",
									text: ""
								});
							}
							if (self.#delayedPromises.object.status.type !== "resolved") self.#delayedPromises.object.resolve(void 0);
						}
						...
						if (baseFinishStep && baseFinishStep.providerMetadata === void 0 && finalProviderMetadata !== void 0) baseFinishStep.providerMetadata = finalProviderMetadata;
						this.resolvePromises({
							usage: self.#usageCount,
							warnings: self.#warnings,
						...
						if (baseFinishStep) {
							const onFinishPayload = {
								...
								staticToolResults: (await self.toolResults).filter((toolResult) => toolResult?.payload?.dynamic === false),
								dynamicToolCalls: (await self.toolCalls).filter((toolCall) => toolCall?.payload?.dynamic === true),
								dynamicToolResults: (await self.toolResults).filter((toolResult) => toolResult?.payload?.dynamic === true),
								...self.#model.modelId && self.#model.provider && self.#model.version ? { model: self.#model } : {},
					...
					case "goal":
						if (chunk.payload.shouldContinue === true) {
							...
							if (byStep.text.length > 0 || byStep.toolCalls.length > 0 || byStep.reasoning.length > 0 || byStep.sources.length > 0 || byStep.files.length > 0) self.#truncateAtNextStepFinish = true;
						}
						break;
					case "error":
						const error = getErrorFromUnknown(chunk.payload.error, { fallbackMessage: "Unknown error chunk in stream" });
						self.#error = error;
						self.#status = "failed";
						self.#streamFinished = true;
						Object.values(self.#delayedPromises).forEach((promise) => {
							if (promise.status.type === "pending") promise.reject(self.#error);
						});
						self.#closeTransportIfNeeded();
						break;
				}
			...
			flush: () => {
				if (self.#delayedPromises.object.status.type === "pending") self.#delayedPromises.object.resolve(void 0);
		...
		if (initialState) this.deserializeState(initialState);
	}
	resolvePromise(key, value) {
		if (!(key in this.#delayedPromises)) throw new MastraError({
	...
	#closeTransportIfNeeded() {
		...
		if (!transport || !transport.closeOnFinish || this.#transportClosed) return;
	...
	*/
	get fullStream() {
		...
		if (!configuredTransforms) return this.#createEventedStream();
	...
	*/
	...
	updateUsageCount(usage) {
		if (!usage) return;
		if (usage.inputTokens !== void 0) this.#usageCount.inputTokens = (this.#usageCount.inputTokens ?? 0) + usage.inputTokens;
		if (usage.outputTokens !== void 0) this.#usageCount.outputTokens = (this.#usageCount.outputTokens ?? 0) + usage.outputTokens;
		if (usage.totalTokens !== void 0) this.#usageCount.totalTokens = (this.#usageCount.totalTokens ?? 0) + usage.totalTokens;
		if (usage.reasoningTokens !== void 0) this.#usageCount.reasoningTokens = (this.#usageCount.reasoningTokens ?? 0) + usage.reasoningTokens;
		if (usage.cachedInputTokens !== void 0) this.#usageCount.cachedInputTokens = (this.#usageCount.cachedInputTokens ?? 0) + usage.cachedInputTokens;
		if (usage.cacheCreationInputTokens !== void 0) this.#usageCount.cacheCreationInputTokens = (this.#usageCount.cacheCreationInputTokens ?? 0) + usage.cacheCreationInputTokens;
		if (usage.raw !== void 0) this.#usageCount.raw = usage.raw;
	}
	populateUsageCount(usage) {
		if (!usage) return;
		if (usage.inputTokens !== void 0 && this.#usageCount.inputTokens === void 0) this.#usageCount.inputTokens = usage.inputTokens;
		if (usage.outputTokens !== void 0 && this.#usageCount.outputTokens === void 0) this.#usageCount.outputTokens = usage.outputTokens;
		if (usage.totalTokens !== void 0 && this.#usageCount.totalTokens === void 0) this.#usageCount.totalTokens = usage.totalTokens;
		if (usage.reasoningTokens !== void 0 && this.#usageCount.reasoningTokens === void 0) this.#usageCount.reasoningTokens = usage.reasoningTokens;
		if (usage.cachedInputTokens !== void 0 && this.#usageCount.cachedInputTokens === void 0) this.#usageCount.cachedInputTokens = usage.cachedInputTokens;
		if (usage.cacheCreationInputTokens !== void 0 && this.#usageCount.cacheCreationInputTokens === void 0) this.#usageCount.cacheCreationInputTokens = usage.cacheCreationInputTokens;
		if (usage.raw !== void 0 && this.#usageCount.raw === void 0) this.#usageCount.raw = usage.raw;
	}
	async consumeStream(options) {
		...
		if (this.#consumeStreamErrored) options?.onError?.(this.#consumeStreamError);
	}
	/**
	* Returns complete output including text, usage, tool calls, and all metadata.
	*/
	async getFullOutput() {
		await this.consumeStream({ onError: (error) => {
	...
	*/
	get objectStream() {
		return this.#createEventedStream().pipeThrough(new TransformStream({ transform(chunk, controller) {
			if (chunk.type === "object") controller.enqueue(chunk.object);
		} }));
	}
	/**
	...
	get elementStream() {
		...
		return this.#createEventedStream().pipeThrough(new TransformStream({ transform(chunk, controller) {
			if (chunk.type === "object") {
				if (Array.isArray(chunk.object)) for (; publishedElements < chunk.object.length; publishedElements++) controller.enqueue(chunk.object[publishedElements]);
			}
		} }));
	}
	/**
	...
	*/
	get textStream() {
		if (this.#structuredOutputMode === "direct") {
			if (getTransformedSchema(this.#options.structuredOutput?.schema)?.outputFormat === "array") return this.#createEventedStream().pipeThrough(createJsonTextStreamTransformer(this.#options.structuredOutput?.schema));
		}
		return this.#createEventedStream().pipeThrough(new TransformStream({ transform(chunk, controller) {
			if (chunk.type === "text-delta") controller.enqueue(chunk.payload.text);
		} }));
	}
	/**
	...
	*/
	get object() {
		if (!this.processorRunner && !this.#options.structuredOutput?.schema && this.#delayedPromises.object.status.type === "pending") this.#delayedPromises.object.resolve(void 0);
	...
	/** @internal */
	_waitUntilFinished() {
		if (this.#streamFinished) return Promise.resolve();
...
const getModelOutputForTripwire = async ({ tripwire, runId, options, model, messageList, ...rest }) => {
...
//#endregion
export { tryGenerateWithJsonFallback as C, isProcessorWorkflow as E, supportedLanguageModelSpecifications as S, DelayedPromise as T, summarizeProcessorToolsForSpan as _, escapeUnescapedControlCharsInJsonStrings as a, isSupportedLanguageModel as b, safeClose as c, ProcessorState as d, runner_exports as f, summarizeProcessorResultForSpan as g, summarizeProcessorModelForSpan as h, createDestructurableOutput as i, safeEnqueue as l, summarizeActiveToolsForSpan as m, getModelOutputForTripwire as n, getResponseFormat as o, REPROCESS_PART_KEY as p, MastraModelOutput as r, MastraModelInput as s, TripWire as t, ProcessorRunner as u, summarizeToolChoiceForSpan as v, tryStreamWithJsonFallback as w, resolveThreadIdFromArgs as x, createProcessorSendSignal as y };

//# sourceMappingURL=trip-wire-csEv7lz7.js.map