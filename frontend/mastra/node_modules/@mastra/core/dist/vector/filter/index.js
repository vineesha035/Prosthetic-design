//#region src/vector/filter/base.ts
var BaseFilterTranslator = class BaseFilterTranslator {
	/**
	...
	*/
	...
	isLogicalOperator(key) {
		return BaseFilterTranslator.DEFAULT_OPERATORS.logical.includes(key);
	}
	isBasicOperator(key) {
		return BaseFilterTranslator.DEFAULT_OPERATORS.basic.includes(key);
	}
	isNumericOperator(key) {
		return BaseFilterTranslator.DEFAULT_OPERATORS.numeric.includes(key);
	}
	isArrayOperator(key) {
		return BaseFilterTranslator.DEFAULT_OPERATORS.array.includes(key);
	}
	isElementOperator(key) {
		return BaseFilterTranslator.DEFAULT_OPERATORS.element.includes(key);
	}
	isRegexOperator(key) {
		return BaseFilterTranslator.DEFAULT_OPERATORS.regex.includes(key);
	}
	isFieldOperator(key) {
		return this.isOperator(key) && !this.isLogicalOperator(key);
	}
	isCustomOperator(key) {
		return this.getSupportedOperators().custom?.includes(key) ?? false;
	}
	...
	isValidOperator(key) {
		...
		return Object.values(support).flat().includes(key);
	}
	/**
	...
	*/
	simulateAllOperator(field, values) {
		return { $and: values.map((value) => ({ [field]: { $in: [this.normalizeComparisonValue(value)] } })) };
	}
	/**
	...
	*/
	...
	validateFilter(filter) {
		...
		if (!validation.supported) throw new Error(validation.messages.join(", "));
	}
	/**
	...
	*/
	validateFilterSupport(node, path = "") {
		const messages = [];
		if (this.isPrimitive(node) || this.isEmpty(node)) return {
			supported: true,
			messages: []
		};
			...
			if (this.isOperator(key)) {
				...
				if (this.isLogicalOperator(key)) {
					...
					if (Array.isArray(value)) {
						if (value.some((item) => typeof item === "object" && Object.keys(item).length === 1 && this.isFieldOperator(Object.keys(item)[0]))) {
							isSupported = false;
