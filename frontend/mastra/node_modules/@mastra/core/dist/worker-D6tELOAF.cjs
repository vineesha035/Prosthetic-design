...
//#region src/schedules/worker.ts
/** PubSub topic on which the scheduler publishes `agent-schedule.fire` events. */
const TOPIC_AGENT_SCHEDULES = "agent-schedules";
...
*/
var AgentScheduleWorker = class extends require_pull_transport.MastraWorker {
	name = "agent-schedule";
	#config;
	#transport;
	#pushCb;
	...
	async init(deps) {
		...
		if (!deps.mastra) throw new Error("AgentScheduleWorker requires Mastra instance");
	}
	async start() {
		if (this.#running) return;
		if (!this.deps) throw new Error("AgentScheduleWorker: call init() before start()");
		if (!(this.deps.pubsub.supportedModes ?? ["pull"]).includes("pull")) {
			const cb = (event, ack, nack) => {
				this.#handleEvent(event, ack, nack);
			};
			this.#pushCb = cb;
		...
		await this.#transport.start({ route: (event, ack, nack) => this.#handleEvent(event, ack, nack) });
		this.#running = true;
	}
	async stop() {
		if (!this.#running) return;
...
async function selfClean(mastra, scheduleId) {
	try {
		...
		mastra.getLogger?.()?.debug?.("agent-schedule self-clean failed", {
			scheduleId,
			error
		});
	}
...
async function executeAgentSchedule(mastra, scheduleId, target, ctx = {}) {
	...
	if (hooks?.prepare) try {
		...
		await safeHookCall(log, () => hooks.onError?.({
			mastra,
			agentId,
			schedule: scheduleRef,
			trigger,
	...
	if (prepared === null) {
		await safeHookCall(log, () => hooks?.onFinish?.({
			mastra,
			agentId,
			schedule: scheduleRef,
			trigger,
	...
	if (effective.threadId) {
		if (!effective.resourceId) {
			...
			await safeHookCall(log, () => hooks?.onError?.({
				...
				trigger,
				phase: "run",
				error: /* @__PURE__ */ new Error(reason),
				effective
			}));
			return {
				status: "invalid-input",
		...
		if (memory) {
			if (!await memory.getThreadById({ threadId: effective.threadId })) {
				await selfClean(mastra, scheduleId);
	...
	try {
		const result = await agent.generate(effective.prompt, { providerOptions: mergeProviderOptions(effective.providerOptions, scheduleRunMeta) });
		...
		if (isAbortError(error)) {
			await safeHookCall(log, () => hooks?.onAbort?.({
				mastra,
				agentId,
				schedule: scheduleRef,
				trigger,
...
function buildIfIdleOptions(ifIdle) {
	...
	return {
		...
		...requestContext ? { streamOptions: { requestContext: new require_request_context.RequestContext(Object.entries(requestContext)) } } : {}
	};
}
...
async function loadScheduleRef(mastra, scheduleId, target, logger) {
	try {
		...
		if (schedule && schedule.agentId !== void 0) return {
			...schedule,
			agentId: schedule.agentId
		};
	} catch (err) {
		logger?.debug?.("AgentScheduleWorker: failed to load schedule row for hook context", {
			scheduleId,
			error: err
		});
	}
...
//#endregion
exports.AgentScheduleWorker = AgentScheduleWorker;

//# sourceMappingURL=worker-D6tELOAF.cjs.map