...
var LSPManager = class {
	...
	*/
	async acquireFileLock(filePath) {
		while (this.fileLocks.has(filePath)) await this.fileLocks.get(filePath);
		let release;
	...
	*/
	async initClient(serverDef, projectRoot, key) {
		...
		initPromise.catch(() => {});
		try {
			await Promise.race([initPromise, new Promise((_, reject) => setTimeout(() => reject(/* @__PURE__ */ new Error("LSP client initialization timed out")), initTimeout + 1e3))]);
			return this.clients.get(key) || null;
		} catch (err) {
			timedOut = true;
			...
			const results = await Promise.allSettled(servers.map(async (serverDef) => {
				...
				if (this.clients.has(key)) {
					...
					if (!existing.isAlive) {
						this.clients.delete(key);
						existing.shutdown().catch(() => {});
					} else return this.collectDiagnostics(existing, filePath, content, languageId);
				}
...
var ProcessHandle = class {
	...
	*/
	...
	get writer() {
		if (!this._writer) this._writer = new Writable({ write: (chunk, _encoding, cb) => {
	...
	constructor(options) {
		...
		if (options.processes) {
			...
			if (!this.executeCommand) this.executeCommand = async (command, args, opts) => {
				...
				const result = await (await pm.spawn(fullCommand, {
					...opts,
...
var LocalSandbox = class LocalSandbox extends MastraSandbox {
	...
	async start() {
		...
		if (this.isolation === "seatbelt") {
			...
			if (userProvidedPath) {
				...
				try {
					this._seatbeltProfile = await fs$2.readFile(userProvidedPath, "utf-8");
				} catch (err) {
					...
					await fs$2.writeFile(userProvidedPath, this._seatbeltProfile, "utf-8");
				}
			} else {
				this._seatbeltProfile = generateSeatbeltProfile(this.workingDirectory, this._nativeSandboxConfig);
				...
				await fs$2.writeFile(this._seatbeltProfilePath, this._seatbeltProfile, "utf-8");
			}
		}
	...
	*/
	async destroy() {
		...
		this.mounts.clear();
		if (this._seatbeltProfilePath && !this._userProvidedProfilePath) try {
	...
	*/
	addMountPathToIsolation(mountPath, hostPath) {
		...
		if (this.isolation === "seatbelt") this._seatbeltProfile = generateSeatbeltProfile(this.workingDirectory, this._nativeSandboxConfig);
	}
	/**
	...
	*/
	async indexMany(docs, options) {
		...
		await pMap(docs, (doc) => this.index(doc), {
			stopOnError,
			concurrency: options?.concurrency ?? DEFAULT_INDEX_MANY_CONCURRENCY
		});
	}
	/**
	* Remove a document from the index
	...
	*/
	async #embedAll(texts) {
		...
		if (isBatchEmbedder(embedder)) {
			...
			if (max === void 0 || texts.length <= max) return embedder(texts);
			const chunks = [];
			for (let i = 0; i < texts.length; i += max) chunks.push(texts.slice(i, i + max));
			return (await pMap(chunks, (chunk) => embedder(chunk), { concurrency: DEFAULT_INDEX_MANY_CONCURRENCY })).flat();
		}
...
//#endregion
//#region src/workspace/sandbox/sandbox.ts
/**
...
const executeCommandInputSchema = z.object({
	...
	timeout: z.preprocess((value) => {
		if (typeof value !== "string") return value;
		const trimmed = value.trim();
		return NUMERIC_TIMEOUT_STRING_REGEX.test(trimmed) ? Number(trimmed) : value;
	}, z.number()).nullish().describe("Maximum execution time in seconds. Example: 60 for 1 minute."),
	cwd: z.string().nullish().describe("Working directory for the command"),
	tail: z.number().nullish().describe(`For foreground commands: limit output to the last N lines, similar to tail -n. Defaults to 200. Use 0 for no limit.`)
});
...
async function executeCommand(input, context) {
	...
	try {
		...
		if (!result.success) {
			const parts = [await truncateOutput(result.stdout, tail, tokenLimit, tokenFrom), await truncateOutput(result.stderr, tail, tokenLimit, tokenFrom)].filter(Boolean);
		...
		const parts = [await truncateOutput(stdout, tail, tokenLimit, tokenFrom), await truncateOutput(stderr, tail, tokenLimit, tokenFrom)].filter(Boolean);
...
Use this after starting a background command with execute_command (background: true) to check if the process is still running and read its output.`,
	...
	inputSchema: z.object({
		...
		tail: z.number().optional().describe(`Number of lines to return, similar to tail -n. Positive or negative returns last N lines from end. Defaults to 200. Use 0 for no limit.`),
	...
	execute: async ({ pattern, path: inputPath = ".", contextLines = 0, maxCount, caseSensitive = true, includeHidden = false }, context) => {
		...
		try {
			...
			if (normalizedSearch === ".git" || normalizedSearch.endsWith("/.git")) filePaths = [];
				...
				if ((await filesystem.stat(searchPath)).type === "file") filePaths = isTextFile(searchPath) ? [searchPath] : [];
				else {
					const collectFiles = async (dir) => {
						...
						for (const entry of entries) {
							...
							if (entry.type === "file") {
								...
								files.push(fullPath);
							} else if (entry.type === "directory" && !entry.isSymlink) files.push(...await collectFiles(fullPath));
						}
						return files;
					};
				...
				if (normalizedContextLines > 0) {
					...
					for (const hunk of hunks) {
						...
						for (let i = hunk.start; i <= hunk.end; i++) {
							...
							} else outputLines.push(`${filePath}:${i + 1}- ${lines[i]}`);
						}
					}
...
async function formatAsTree(fs, path, options) {
	...
	*/
	async function buildTree(currentPath, depth) {
		...
		if (exclude) {
			const patterns = Array.isArray(exclude) ? exclude : exclude.split("|").map((p) => p.trim()).filter(Boolean);
		...
		for (let i = 0; i < filtered.length; i++) {
			...
			if (entry.type === "directory") {
				dirCount++;
				if (!entry.isSymlink) await buildTree(joinPath(currentPath, entry.name), depth + 1);
			} else fileCount++;
		}
	}
...
//#endregion
export { detectIsolation as A, MountNotSupportedError as B, formatSkillActivation as C, VersionedSkillSource as D, CompositeVersionedSkillSource as E, SandboxProcessManager as F, LocalFilesystem as G, SandboxExecutionError as H, ProcessHandle as I, callLifecycle as J, MastraFilesystem as K, FilesystemNotMountableError as L, isIsolationAvailable as M, MastraSandbox as N, BM25Index as O, MountManager as P, IsolationUnavailableError as R, createSkillTools as S, publishSkillFromSource as T, SandboxNotReadyError as U, SandboxError as V, SandboxTimeoutError as W, requireWorkspace as _, readFileTool as a, supportsNetworking as b, indexContentTool as c, BrowserCliHandler as d, browserCliHandler as f, requireSandbox as g, requireFilesystem as h, searchTool as i, getRecommendedIsolation as j, LocalSandbox as k, fileStatTool as l, deleteFileTool as m, resolveToolConfig as n, mkdirTool as o, editFileTool as p, CompositeFilesystem as q, writeFileTool as r, listFilesTool as s, createWorkspaceTools as t, executeCommandTool as u, WORKSPACE_TOOLS as v, collectSkillForPublish as w, Workspace as x, WORKSPACE_TOOLS_PREFIX as y, MountError as z };

//# sourceMappingURL=workspace-CvZ9jmZN.js.map