...
picomatch = require_rolldown_runtime.__toESM(picomatch, 1);
...
async function fsExists(absolutePath) {
	try {
		await fs_promises.access(absolutePath);
		return true;
	} catch {
		return false;
	}
...
async function fsStat(absolutePath, userPath) {
	try {
		const stats = await fs_promises.stat(absolutePath);
		return {
		...
		if (isEnoentError(error)) throw new require_errors.FileNotFoundError(userPath);
		throw error;
	}
}
//#endregion
...
function createGlobMatcher(patterns, options) {
	const matcher = (0, picomatch.default)((Array.isArray(patterns) ? patterns : [patterns]).map(normalizeForMatch), {
		posix: true,
...
var LocalSkillSource = class {
	...
	*/
	...
	async readFile(skillPath) {
		...
		const content = await fs_promises.readFile(resolved);
		if (isTextFile(skillPath)) return content.toString("utf-8");
		return content;
	}
	async readdir(skillPath) {
		const resolved = this.#resolvePath(skillPath);
		const entries = await fs_promises.readdir(resolved, { withFileTypes: true });
		return Promise.all(entries.map(async (entry) => {
			const entryPath = path.join(resolved, entry.name);
			const isSymlink = entry.isSymbolicLink();
			let type = entry.isDirectory() ? "directory" : "file";
			if (isSymlink) try {
				type = (await fs_promises.stat(entryPath)).isDirectory() ? "directory" : "file";
			} catch {
				type = "file";
			}
			return {
	...
	async realpath(skillPath) {
		return fs_promises.realpath(this.#resolvePath(skillPath));
	}
};
//#endregion
...
var WorkspaceSkillsImpl = class WorkspaceSkillsImpl {
	...
	*/
	...
	async refresh() {
		for (const candidates of this.#skills.values()) for (const skill of candidates) await this.#removeSkillFromIndex(skill);
		this.#skills.clear();
	...
	*/
	...
	async search(query, options = {}) {
		...
		const totalIndexedDocuments = [...this.#skills.values()].reduce((count, candidates) => count + candidates.reduce((skillCount, skill) => skillCount + 1 + skill.references.length, 0), 0);
		...
		for (const rawSkillsPath of this.#resolvedPaths) {
			...
			if (isGlobPattern(skillsPath)) {
				...
				const results = await Promise.allSettled(resolved.map(async (entry) => {
					if (entry.type === "file") await this.#discoverDirectSkill(entry.path, source);
					else if (!await this.#discoverDirectSkill(entry.path, source)) await this.#discoverSkillsInPath(entry.path, source);
				}));
			...
			} else if (!await this.#discoverDirectSkill(skillsPath, source)) await this.#discoverSkillsInPath(skillsPath, source);
		}
		this.#lastDiscoveryTime = Date.now();
	}
	/**
	...
	*/
	async #parseSkillFile(filePath, dirName, source) {
		...
		const parsed = (0, gray_matter.default)(typeof rawContent === "string" ? rawContent : rawContent.toString("utf-8"));
...
Object.defineProperty(exports, "WorkspaceSkillsImpl", {
	enumerable: true,
	get: function() {
...
Object.defineProperty(exports, "fsStat", {
	enumerable: true,
	get: function() {
		return fsStat;
	}
});
Object.defineProperty(exports, "isEexistError", {
	enumerable: true,
	get: function() {
		return isEexistError;
	}
});
Object.defineProperty(exports, "isEnoentError", {
	enumerable: true,
	get: function() {
		return isEnoentError;
	}
});
Object.defineProperty(exports, "isGlobPattern", {
	enumerable: true,
	get: function() {
		return isGlobPattern;
	}
});
Object.defineProperty(exports, "isTextFile", {
	enumerable: true,
	get: function() {
		return isTextFile;
	}
...
Object.defineProperty(exports, "resolvePathPattern", {
	enumerable: true,
	get: function() {
		return resolvePathPattern;
	}
});
Object.defineProperty(exports, "resolveToBasePath", {
	enumerable: true,
	get: function() {
		return resolveToBasePath;
	}
});
Object.defineProperty(exports, "validateSkillMetadata", {
	enumerable: true,
	get: function() {
...
//# sourceMappingURL=workspace-skills-CjnfyQuP.cjs.map