...
function getZodTypeName(schema) {
	...
	if (schemaAny._def?.typeName) return schemaAny._def.typeName;
	const zod4Type = schemaAny._def?.type;
	if (typeof zod4Type === "string" && zod4Type) return "Zod" + zod4Type.charAt(0).toUpperCase() + zod4Type.slice(1);
}
/**
...
*/
function safeExtendZodObject(schema, shape) {
	return (schema.safeExtend ?? schema.extend).call(schema, shape);
}
/**
...
function getZodInnerType(schema, typeName) {
	const schemaAny = schema;
	if (typeName === "ZodNullable" || typeName === "ZodOptional" || typeName === "ZodDefault") return schemaAny._zod?.def?.innerType ?? schemaAny._def?.innerType;
	if (typeName === "ZodEffects") return schemaAny._zod?.def?.schema ?? schemaAny._def?.schema;
	if (typeName === "ZodBranded") return schemaAny._zod?.def?.type ?? schemaAny._def?.type;
}
/**
...
Object.defineProperty(exports, "getZodInnerType", {
	enumerable: true,
	get: function() {
		return getZodInnerType;
	}
});
Object.defineProperty(exports, "getZodTypeName", {
	enumerable: true,
	get: function() {
		return getZodTypeName;
	}
});
...
Object.defineProperty(exports, "safeExtendZodObject", {
	enumerable: true,
	get: function() {
		return safeExtendZodObject;
	}
});
Object.defineProperty(exports, "unwrapZodType", {
	enumerable: true,
	get: function() {
...
//# sourceMappingURL=zod-utils-BAGXGqPm.cjs.map