...
let zod_from_json_schema_v3 = require("zod-from-json-schema-v3");
...
function parseArrayDef(def, refs) {
	...
	if (def.type?._def && def.type?._def?.typeName !== zod_v3.ZodFirstPartyTypeKind.ZodAny) res.items = parseDef(def.type._def, {
		...refs,
...
const selectParser = (def, typeName, refs) => {
	switch (typeName) {
		...
		case zod_v3.ZodFirstPartyTypeKind.ZodNumber: return parseNumberDef(def, refs);
		case zod_v3.ZodFirstPartyTypeKind.ZodObject: return parseObjectDef(def, refs);
		case zod_v3.ZodFirstPartyTypeKind.ZodBigInt: return parseBigintDef(def, refs);
		...
		case zod_v3.ZodFirstPartyTypeKind.ZodUndefined: return parseUndefinedDef(refs);
		case zod_v3.ZodFirstPartyTypeKind.ZodNull: return parseNullDef(refs);
		...
		case zod_v3.ZodFirstPartyTypeKind.ZodLiteral: return parseLiteralDef(def, refs);
		...
		case zod_v3.ZodFirstPartyTypeKind.ZodNever: return parseNeverDef(refs);
		case zod_v3.ZodFirstPartyTypeKind.ZodEffects: return parseEffectsDef(def, refs);
		case zod_v3.ZodFirstPartyTypeKind.ZodAny: return parseAnyDef(refs);
		case zod_v3.ZodFirstPartyTypeKind.ZodUnknown: return parseUnknownDef(refs);
...
var providerMetadataSchema = zod.z.record(zod.z.string(), zod.z.record(zod.z.string(), jsonValueSchema));
var toolResultContentSchema = zod.z.array(zod.z.union([zod.z.object({
...
var coreUserMessageSchema = zod.z.object({
	...
	content: zod.z.union([zod.z.string(), zod.z.array(zod.z.union([
		textPartSchema,
		imagePartSchema,
		filePartSchema
	]))]),
...
var BaseParamsSchema = zod.z.object({ _meta: zod.z.optional(zod.z.object({}).passthrough()) }).passthrough();
...
var ServerCapabilitiesSchema = zod.z.object({
	...
	prompts: zod.z.optional(zod.z.object({ listChanged: zod.z.optional(zod.z.boolean()) }).passthrough()),
	resources: zod.z.optional(zod.z.object({
		subscribe: zod.z.optional(zod.z.boolean()),
		listChanged: zod.z.optional(zod.z.boolean())
	}).passthrough()),
	tools: zod.z.optional(zod.z.object({ listChanged: zod.z.optional(zod.z.boolean()) }).passthrough())
}).passthrough();
ResultSchema.extend({
...
var PaginatedResultSchema = ResultSchema.extend({ nextCursor: zod.z.optional(zod.z.string()) });
...
PaginatedResultSchema.extend({ tools: zod.z.array(ToolSchema) });
...
var TextResourceContentsSchema = ResourceContentsSchema.extend({ text: zod.z.string() });
var BlobResourceContentsSchema = ResourceContentsSchema.extend({ blob: zod.z.string().base64() });
...
ResultSchema.extend({
	content: zod.z.array(zod.z.union([
		TextContentSchema,
		ImageContentSchema,
		EmbeddedResourceSchema
	])),
	isError: zod.z.boolean().default(false).optional()
}).or(ResultSchema.extend({ toolResult: zod.z.unknown() }));
...
var JSONRPCNotificationSchema = zod.z.object({ jsonrpc: zod.z.literal(JSONRPC_VERSION) }).merge(zod.z.object({
	method: zod.z.string(),
	params: zod.z.optional(BaseParamsSchema)
})).strict();
zod.z.union([
...
function convertZodSchemaToAISDKSchema(zodSchema, target = "jsonSchema7") {
	return jsonSchema(require_zod_to_json.zodToJsonSchema(zodSchema, target), { validate: (value) => {
		const result = zodSchema.safeParse(value);
		return result.success ? {
			success: true,
			value: result.data
...
function convertSchemaToZod(schema) {
	if (isZodType(schema)) return schema;
		...
		try {
			if ("toJSONSchema" in zod.z) return (0, zod_from_json_schema.convertJsonSchemaToZod)(jsonSchemaToConvert);
			else return (0, zod_from_json_schema_v3.convertJsonSchemaToZod)(jsonSchemaToConvert);
		} catch (e) {
...
function applyCompatLayer({ schema, compatLayers, mode }) {
	if (mode === "jsonSchema") {
		const standardSchema = require_schema.toStandardSchema(schema);
		for (const compat of compatLayers) if (compat.shouldApply()) return require_schema.standardSchemaToJSONSchema(compat.processToCompatSchema(standardSchema));
		return require_schema.standardSchemaToJSONSchema(standardSchema);
	} else {
		let zodSchema;
		if (isZodType(schema)) zodSchema = schema;
		else {
			if (require_schema.isStandardSchemaWithJSON(schema)) throw new Error("StandardSchemaWithJSON is not supported for applyCompatLayer and aiSdkSchema mode");
...
const isOptional = (v) => v instanceof zod_v3.ZodOptional;
const isObj = (v) => v instanceof zod_v3.ZodObject;
const isArr = (v) => v instanceof zod_v3.ZodArray;
const isUnion = (v) => v instanceof zod_v3.ZodUnion;
const isString = (v) => v instanceof zod_v3.ZodString;
const isNumber = (v) => v instanceof zod_v3.ZodNumber;
/**
...
var SchemaCompatLayer$1 = class {
	...
	*/
	isOptional(v) {
		return v instanceof zod_v3.ZodOptional;
	}
	/**
	* Type guard for object Zod types
	*/
	isObj(v) {
		return v instanceof zod_v3.ZodObject;
	}
	/**
	* Type guard for null Zod types
	*/
	isNull(v) {
		return v instanceof zod_v3.ZodNull;
	}
	/**
	* Type guard for nullable Zod types
	*/
	isNullable(v) {
		return v instanceof zod_v3.ZodNullable;
	}
	/**
	* Type guard for array Zod types
	*/
	isArr(v) {
		return v instanceof zod_v3.ZodArray;
	}
	/**
	* Type guard for union Zod types
	*/
	isUnion(v) {
		return v instanceof zod_v3.ZodUnion;
	}
	/**
	* Type guard for string Zod types
	*/
	isString(v) {
		return v instanceof zod_v3.ZodString;
	}
	/**
	* Type guard for number Zod types
	*/
	isNumber(v) {
		return v instanceof zod_v3.ZodNumber;
	}
	/**
	* Type guard for date Zod types
	*/
	isDate(v) {
		return v instanceof zod_v3.ZodDate;
	}
	/**
	* Type guard for default Zod types
	*/
	isDefault(v) {
		return v instanceof zod_v3.ZodDefault;
	}
	/**
	* Type guard for intersection Zod types
	*/
	isIntersection(v) {
		return v instanceof zod_v3.ZodIntersection;
	}
	/**
	...
	*/
	defaultZodObjectHandler(value, options = { passthrough: true }) {
		...
		let result = zod_v3.z.object(processedShape);
		if (value._def.unknownKeys === "strict") result = result.strict();
		if (value._def.catchall && !(value._def.catchall instanceof zod_v3.z.ZodNever)) result = result.catchall(value._def.catchall);
	...
	*/
	defaultZodArrayHandler(value, handleChecks = ALL_ARRAY_CHECKS) {
		...
		let result = zod_v3.z.array(processedType);
		const constraints = [];
	...
	*/
	defaultZodUnionHandler(value) {
		...
		let result = zod_v3.z.union(processedOptions);
		if (value.description) result = result.describe(value.description);
		return result;
	}
	/**
	...
	*/
	defaultZodDateHandler(value) {
		...
		let result = zod_v3.z.string().describe("date-time");
		const description = this.mergeParameterDescription(value.description, constraints);
		if (description) result = result.describe(description);
		return result;
	}
	...
	*/
	collectIntersectionLeaves(value) {
		if (value instanceof zod_v3.ZodIntersection) return [...this.collectIntersectionLeaves(value._def.left), ...this.collectIntersectionLeaves(value._def.right)];
		return [value];
	}
	/**
	...
	*/
	defaultZodIntersectionHandler(value) {
		...
		if (processed.every((p) => p instanceof zod_v3.ZodObject)) {
			const mergedShape = {};
			for (const obj of processed) Object.assign(mergedShape, obj.shape);
			let result = zod_v3.z.object(mergedShape);
			if (value.description) result = result.describe(value.description);
			return result;
		}
		return zod_v3.z.any().describe(value.description || "intersection type");
	}
	/**
...
var SchemaCompatLayer$2 = class {
	...
	*/
	isOptional(v) {
		return v instanceof zod_v4.ZodOptional;
	}
	/**
	* Type guard for object Zod types
	*/
	isObj(v) {
		return v instanceof zod_v4.ZodObject;
	}
	/**
	* Type guard for null Zod types
	*/
	isNull(v) {
		return v instanceof zod_v4.ZodNull;
	}
	/**
	* Type guard for nullable Zod types
	*/
	isNullable(v) {
		return v instanceof zod_v4.ZodNullable;
	}
	/**
	* Type guard for array Zod types
	*/
	isArr(v) {
		return v instanceof zod_v4.ZodArray;
	}
	/**
	* Type guard for union Zod types
	*/
	isUnion(v) {
		return v instanceof zod_v4.ZodUnion;
	}
	/**
	* Type guard for string Zod types
	*/
	isString(v) {
		return v instanceof zod_v4.ZodString;
	}
	/**
	* Type guard for number Zod types
	*/
	isNumber(v) {
		return v instanceof zod_v4.ZodNumber;
	}
	/**
	* Type guard for date Zod types
	*/
	isDate(v) {
		return v instanceof zod_v4.ZodDate;
	}
	/**
	* Type guard for default Zod types
	*/
	isDefault(v) {
		return v instanceof zod_v4.ZodDefault;
	}
	/**
	* Type guard for intersection Zod types
	*/
	isIntersection(v) {
		return v instanceof zod_v4.ZodIntersection;
	}
	/**
	* Determines whether this compatibility layer should be applied for the current model.
	*
	...
	*/
	defaultZodObjectHandler(value, options = { passthrough: true }) {
		...
		let result = zod_v4.z.object(processedShape);
		if (value._zod.def.catchall instanceof zod_v4.z.ZodNever) result = zod_v4.z.strictObject(processedShape);
		else if (value._zod.def.catchall instanceof zod_v4.z.ZodUnknown) {
			if (options.passthrough) result = zod_v4.z.looseObject(processedShape);
	...
	*/
	defaultZodArrayHandler(value, handleChecks = ALL_ARRAY_CHECKS$1) {
		...
		let result = zod_v4.z.array(processedType);
		const constraints = [];
	...
	*/
	defaultZodUnionHandler(value) {
		...
		if (processedOptions.length < 2) throw new Error("Union must have at least 2 options");
		let result = zod_v4.z.union(processedOptions);
		if (value.description) result = result.describe(value.description);
		return result;
	}
	/**
	...
	*/
	defaultZodDateHandler(value) {
		...
		constraints.push(`Date format is date-time`);
		let result = zod_v4.z.string().describe("date-time");
		const description = this.mergeParameterDescription(value.description, constraints);
		if (description) result = result.describe(description);
		return result;
	}
	...
	*/
	collectIntersectionLeaves(value) {
		if (value instanceof zod_v4.ZodIntersection) return [...this.collectIntersectionLeaves(value._zod.def.left), ...this.collectIntersectionLeaves(value._zod.def.right)];
		return [value];
	}
	/**
	* Default handler for Zod intersection types.
	...
	*/
	defaultZodIntersectionHandler(value) {
		const processed = this.collectIntersectionLeaves(value).map((leaf) => this.processZodType(leaf));
		if (processed.every((p) => p instanceof zod_v4.ZodObject)) {
			const mergedShape = {};
			...
			let result = zod_v4.z.object(mergedShape);
			if (value.description) result = result.describe(value.description);
			return result;
		}
		return zod_v4.z.any().describe(value.description || "intersection type");
	}
	/**
	* Processes a Zod object schema and converts it to an AI SDK Schema.
	*
...
var SchemaCompatLayer = class {
	...
	*/
	processToJSONSchema(schema, io = "input") {
		const jsonSchema = require_schema.standardSchemaToJSONSchema(require_schema.toStandardSchema(schema), {
			target: "draft-07",
			io
		});
	...
	processToCompatSchema(schema) {
		return { "~standard": {
			version: 1,
			vendor: "mastra",
			validate: (value) => require_schema.toStandardSchema(schema)["~standard"].validate(value),
			jsonSchema: {
				input: () => {
	...
	*/
	toJSONSchema(zodSchema) {
		...
		const jsonSchema = require_schema.standardSchemaToJSONSchema(require_schema.toStandardSchema(zodSchema), {
			target,
			io: "input"
		});
		(0, import_json_schema_traverse.default)(jsonSchema, { cb: {
...
var OpenAISchemaCompatLayer = class extends SchemaCompatLayer {
	...
	processZodType(value) {
		if (isOptional$1(zod.z)(value)) {
			const innerType = "_def" in value ? value._def.innerType : value._zod?.def?.innerType;
			if (innerType) {
				if (isNullable(zod.z)(innerType)) return this.processZodType(innerType).transform((val) => val === null ? void 0 : val);
			...
			const innerType = "_def" in value ? value._def.innerType : value._zod?.def?.innerType;
			if (innerType) {
				if (isOptional$1(zod.z)(innerType)) {
			...
			return value;
		} else if (isObj$1(zod.z)(value)) return this.defaultZodObjectHandler(value);
		else if (isUnion$1(zod.z)(value)) return this.defaultZodUnionHandler(value);
		else if (isArr$1(zod.z)(value)) return this.defaultZodArrayHandler(value);
		else if (isString$1(zod.z)(value)) {
			const model = this.getModel();
			const checks = ["emoji"];
			if (model.modelId?.includes("gpt-4o-mini")) return this.defaultZodStringHandler(value, ["emoji", "regex"]);
			return this.defaultZodStringHandler(value, checks);
		}
		if (isIntersection(zod.z)(value)) return this.defaultZodIntersectionHandler(value);
		return this.defaultUnsupportedZodTypeHandler(value, [
			"ZodNever",
			"ZodUndefined",
			"ZodTuple"
	...
	*/
	processToAISDKSchema(zodSchema) {
		const transformedJsonSchema = require_schema.standardSchemaToJSONSchema(this.processToCompatSchema(zodSchema));
	...
	processToCompatSchema(schema) {
		const originalStandardSchema = require_schema.toStandardSchema(schema);
		return { "~standard": {
			version: 1,
			vendor: "mastra",
			validate: (value) => {
...
var OpenAIReasoningSchemaCompatLayer = class extends OpenAISchemaCompatLayer {
	...
	processZodType(value) {
		if (isOptional$1(zod.z)(value)) {
			...
			const innerType = "_def" in value ? value._def.innerType : value._zod?.def?.innerType;
			if (innerType && isOptional$1(zod.z)(innerType)) {
				const innerInnerType = "_def" in innerType ? innerType._def.innerType : innerType._zod?.def?.innerType;
				if (innerInnerType) return this.processZodType(innerInnerType).nullable().transform((val) => val === null ? void 0 : val);
			}
			if (innerType) return this.processZodType(innerType).nullable();
			return value;
		} else if (isObj$1(zod.z)(value)) return this.defaultZodObjectHandler(value, { passthrough: false });
		else if (isArr$1(zod.z)(value)) return this.defaultZodArrayHandler(value);
		else if (isUnion$1(zod.z)(value)) return this.defaultZodUnionHandler(value);
		else if (isDefault(zod.z)(value)) {
			...
			if (description) result = result.describe(description);
			return result;
		} else if (isNumber$1(zod.z)(value)) return this.defaultZodNumberHandler(value);
		else if (isString$1(zod.z)(value)) return this.defaultZodStringHandler(value);
		else if (isDate(zod.z)(value)) return this.defaultZodDateHandler(value);
		else if (isNull(zod.z)(value)) return zod.z.any().refine((v) => v === null, { message: "must be null" }).describe(value.description || "must be null");
		else if (value.constructor.name === "ZodAny") return zod.z.string().describe((value.description ?? "") + `\nArgument was an "any" type, but you (the LLM) do not support "any", so it was cast to a "string" type`);
		if (isIntersection(zod.z)(value)) return this.defaultZodIntersectionHandler(value);
		return this.defaultUnsupportedZodTypeHandler(value);
	}
};
...
function applyOpenAICompatTransforms(schema, modelInfo) {
	...
	return require_schema.toStandardSchema(compatLayer.processZodType(zodSchema));
}
/**
...
var AnthropicSchemaCompatLayer = class extends SchemaCompatLayer {
	...
	processZodType(value) {
		...
		else if (isIntersection(zod.z)(value)) return this.defaultZodIntersectionHandler(value);
		return this.defaultUnsupportedZodTypeHandler(value);
	}
	processToAISDKSchema(zodSchema) {
		const transformedJsonSchema = require_schema.standardSchemaToJSONSchema(this.processToCompatSchema(zodSchema));
...
var DeepSeekSchemaCompatLayer = class extends SchemaCompatLayer {
	...
	processZodType(value) {
		if (isOptional$1(zod.z)(value)) return this.defaultZodOptionalHandler(value, [
			"ZodObject",
			"ZodArray",
			"ZodUnion",
			"ZodString",
		...
		else if (isArr$1(zod.z)(value)) return this.defaultZodArrayHandler(value, ["min", "max"]);
		else if (isUnion$1(zod.z)(value)) return this.defaultZodUnionHandler(value);
		else if (isString$1(zod.z)(value)) return this.defaultZodStringHandler(value);
		else if (isIntersection(zod.z)(value)) return this.defaultZodIntersectionHandler(value);
		return value;
...
var GoogleSchemaCompatLayer = class extends SchemaCompatLayer {
	...
	processZodType(value) {
		...
		else if (isNullable(zod.z)(value)) return this.defaultZodNullableHandler(value);
		else if (isNull(zod.z)(value)) return zod.z.any().refine((v) => v === null, { message: "must be null" }).describe(value.description || "must be null");
		else if (isObj$1(zod.z)(value)) return this.defaultZodObjectHandler(value);
		else if (isArr$1(zod.z)(value)) return this.defaultZodArrayHandler(value, []);
		else if (isUnion$1(zod.z)(value)) return this.defaultZodUnionHandler(value);
	...
	processToAISDKSchema(zodSchema) {
		const fixedJsonSchema = fixAISDKNullableUnionTypes(require_schema.standardSchemaToJSONSchema(this.processToCompatSchema(zodSchema)));
...
exports.SchemaCompatLayerV3 = SchemaCompatLayer$1;
exports.SchemaCompatLayerV4 = SchemaCompatLayer$2;
...
exports.ensureAllPropertiesRequired = require_zod_to_json.ensureAllPropertiesRequired;
...
exports.prepareJsonSchemaForOpenAIStrictMode = require_zod_to_json.prepareJsonSchemaForOpenAIStrictMode;
