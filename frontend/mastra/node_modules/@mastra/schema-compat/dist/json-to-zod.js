import jsonSchemaToZodOriginal, { addJsdocs, its, parseAllOf, parseAnyOf, parseOneOf, parseSchema } from "json-schema-to-zod";
...
function parseObject(objectSchema, refs) {
	...
	if (its.an.anyOf(objectSchema)) output += `.and(${parseAnyOf({
		...objectSchema,
	...
	if (its.a.oneOf(objectSchema)) output += `.and(${parseOneOf({
		...objectSchema,
	...
	if (its.an.allOf(objectSchema)) output += `.and(${parseAllOf({
		...objectSchema,
...
const parserOverride = (schema, refs) => {
	...
	if (its.an.anyOf(schema)) {
		const allObjects = schema.anyOf.every((item) => typeof item === "object" && its.an.object(item) && item.properties !== void 0);
		if (schema.anyOf.length > 1 && allObjects) {
			...
			if (propertiesWithConst.length === schema.anyOf.length) {
				...
				if (discriminators.length > 0) {
					...
					if (discriminator) parsed = `z.discriminatedUnion("${discriminator}", [${schema.anyOf.map((schema, i) => parseSchema(schema, {
						...refs,
						path: [
							...refs.path,
							"anyOf",
...
function jsonSchemaToZod(schema, options = {}) {
	return jsonSchemaToZodOriginal(schema, {
		...options,
		parserOverride
