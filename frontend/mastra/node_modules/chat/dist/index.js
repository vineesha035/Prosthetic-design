import {
  ...
  toCardElement,
  toModalElement
} from "./chunk-5M65T3S5.js";
import {
  runInConversation,
  toAiMessages
} from "./chunk-23HOOUQ5.js";
import {
  Actions,
  BaseFormatConverter,
  Button,
...
var ThreadImpl = class _ThreadImpl {
  ...
  async handleStream(rawStream, callerOptions) {
    ...
    const options = {
      ...
      ...callerOptions,
      ...this._fallbackStreamingPlaceholderText !== void 0 ? {
  ...
  async fallbackStream(textStream, options) {
    ...
    const placeholderText = this._fallbackStreamingPlaceholderText === void 0 ? "..." : this._fallbackStreamingPlaceholderText;
...
var Chat = class {
   ...
   * Handles waitUntil registration and error catching internally.
   */
  processMessageDeleted(event, options) {
  ...
  processAppContextChanged(event, options) {
    const task = runInConversation(event.channelId, async () => {
  ...
  async dispatchSlashCommand(event, channel, options) {
    const fullEvent = {
      ...event,
      channel,
   ...
   */
  handleIncomingMessage(adapter, threadId, message) {
    return runInConversation(
      threadId,
      () => this.routeIncomingMessage(adapter, threadId, message)
    );
  }
  async routeIncomingMessage(adapter, threadId, message) {
   ...
   */
  async handleMessageUpdated(adapter, threadId, message, previousMessage) {
    setMessageAdapter(message, adapter);
  ...
  async resolveMessageIdentity(adapter, threadId, message) {
...
var DateInput2 = DateInput;
var ExternalSelect2 = ExternalSelect;
var Modal2 = Modal;
var NumberInput2 = NumberInput;
