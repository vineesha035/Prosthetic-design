...
var TypeHandler = class {
  apply(types, schema) {
    ...
    if (typeSet.has("integer") && types.number !== false) {
      const currentNumber = types.number || import_v42.z.number();
      if (currentNumber instanceof import_v42.z.ZodNumber) {
...
var ConstHandler = class {
  apply(types, schema) {
    ...
    if (typeof constValue === "string") {
      types.string = import_v43.z.literal(constValue);
    } else if (typeof constValue === "number") {
      types.number = import_v43.z.literal(constValue);
    } else if (typeof constValue === "boolean") {
      types.boolean = import_v43.z.literal(constValue);
    } else if (constValue === null) {
      types.null = import_v43.z.null();
...
var EnumHandler = class {
  apply(types, schema) {
    ...
    types.null = valuesByType.null.length > 0 ? import_v44.z.null() : false;
  ...
  createTypeSchema(values, type) {
    ...
    if (values.length === 1) {
      return import_v44.z.literal(values[0]);
    }
    if (type === "string") {
      return import_v44.z.enum(values);
    }
    if (type === "number") {
      const [first, second, ...rest] = values;
      return import_v44.z.union([import_v44.z.literal(first), import_v44.z.literal(second), ...rest.map((v) => import_v44.z.literal(v))]);
    }
    if (type === "boolean") {
      return import_v44.z.union([import_v44.z.literal(true), import_v44.z.literal(false)]);
    }
    return false;
  }
};
...
var FileHandler = class {
  apply(types, schema) {
    ...
    if (stringSchema.type === "string" && stringSchema.format === "binary" && stringSchema.contentEncoding === "binary") {
      let fileSchema = import_v45.z.file();
  ...
  apply(types, schema) {
    ...
    if (schema.type === void 0 && (stringSchema.minLength !== void 0 || stringSchema.maxLength !== void 0 || stringSchema.pattern !== void 0)) {
      if (types.string === void 0) {
        types.string = import_v46.z.string();
      }
    }
  }
};
var MinLengthHandler = class {
  apply(types, schema) {
    ...
    if (types.string !== false) {
      const currentString = types.string || import_v46.z.string();
      if (currentString instanceof import_v46.z.ZodString) {
...
var MinimumHandler = class {
  apply(types, schema) {
    ...
    if (types.number !== false) {
      const currentNumber = types.number || import_v47.z.number();
      if (currentNumber instanceof import_v47.z.ZodNumber) {
  ...
  apply(types, schema) {
    ...
    if (schema.type === void 0 && (arraySchema.minItems !== void 0 || arraySchema.maxItems !== void 0 || arraySchema.items !== void 0 || arraySchema.prefixItems !== void 0)) {
      if (types.array === void 0) {
        types.array = import_v48.z.array(import_v48.z.any());
      }
    }
  }
};
var MinItemsHandler = class {
  apply(types, schema) {
    ...
    if (types.array !== false) {
      types.array = (types.array || import_v48.z.array(import_v48.z.any())).min(arraySchema.minItems);
    }
  }
};
var MaxItemsHandler = class {
  apply(types, schema) {
    ...
    if (types.array !== false) {
      types.array = (types.array || import_v48.z.array(import_v48.z.any())).max(arraySchema.maxItems);
    }
  }
};
var ItemsHandler = class {
  apply(types, schema) {
    ...
    if (Array.isArray(arraySchema.items)) {
      types.array = types.array || import_v48.z.array(import_v48.z.any());
      ...
      let newArray = import_v48.z.array(itemSchema);
      if (types.array && types.array instanceof import_v48.z.ZodArray) {
      ...
      if (!arraySchema.prefixItems) {
        types.array = import_v48.z.array(import_v48.z.any()).max(0);
      } else {
        types.array = types.array || import_v48.z.array(import_v48.z.any());
      }
...
var TupleHandler = class {
  apply(types, schema) {
    ...
    if (itemSchemas.length === 0) {
      tuple = import_v49.z.tuple([]);
    } else {
      tuple = import_v49.z.tuple(itemSchemas);
    }
...
var PropertiesHandler = class {
  apply(types, schema) {
    ...
    let objectZod = import_v410.z.object(shape);
    ...
    if (enforcibleKeys.length > 0) {
      ...
      types.object = import_v410.z.any().refine(hasRequiredKeys, {
  ...
  apply(types, schema) {
    ...
    if (schema.type === void 0 && (objectSchema.maxProperties !== void 0 || objectSchema.minProperties !== void 0)) {
      if (types.object === void 0) {
        types.object = import_v410.z.object({}).passthrough();
      }
    }
  }
};
var MaxPropertiesHandler = class {
  apply(types, schema) {
    ...
    if (types.object !== false) {
      const baseObject = types.object || import_v410.z.object({}).passthrough();
...
var IfThenElseHandler = class {
  apply(zodSchema, schema) {
    ...
    const thenZod = schema.then !== void 0 ? convertJsonSchemaToZod(schema.then) : import_v411.z.any();
    const elseZod = schema.else !== void 0 ? convertJsonSchemaToZod(schema.else) : import_v411.z.any();
    return zodSchema.refine(
...
var AllOfHandler = class {
  apply(zodSchema, schema) {
    ...
    return allOfSchemas.reduce(
      (acc, s) => import_v412.z.intersection(acc, s),
      zodSchema
    );
  }
};
...
var AnyOfHandler = class {
  apply(zodSchema, schema) {
    ...
    const anyOfSchema = schema.anyOf.length === 1 ? convertJsonSchemaToZod(schema.anyOf[0]) : import_v413.z.union([
    ...
    return import_v413.z.intersection(zodSchema, anyOfSchema);
  }
};
...
var ObjectPropertiesHandler = class {
  apply(zodSchema, schema) {
    ...
    const baseSchema = zodSchema instanceof import_v414.z.ZodPipe ? zodSchema.out : zodSchema;
    const isDirectObject = baseSchema instanceof import_v414.z.ZodObject || baseSchema instanceof import_v414.z.ZodRecord;
...
var DependentSchemasHandler = class {
  apply(zodSchema, schema) {
    ...
    if (mayDependOnProtoKey(dependentSchemas)) {
      return import_v415.z.any().refine(check, { message }).pipe(zodSchema);
    }
...
var ProtoRequiredHandler = class {
  apply(zodSchema, schema) {
    ...
    return import_v416.z.any().refine(
...
var DependentRequiredHandler = class {
  apply(zodSchema, schema) {
    ...
    if (mayDependOnProtoKey(dependentRequired)) {
      return import_v417.z.any().refine(check, { message }).pipe(zodSchema);
    }
    return zodSchema.refine(check, { message });
  }
};
...
var RefHandler = class {
  apply(zodSchema, schema) {
    ...
    if (refs.length === 1 && !refs[0].deferred && zodSchema instanceof import_v418.z.ZodAny) {
...
function convertSchemaNode(schema) {
  if (typeof schema === "boolean") {
    return schema ? import_v419.z.any() : import_v419.z.never();
  }
  const types = {};
  ...
  if (types.string !== false) {
    allowedSchemas.push(types.string || import_v419.z.string());
  }
  if (types.number !== false) {
    allowedSchemas.push(types.number || import_v419.z.number());
  }
  if (types.boolean !== false) {
    allowedSchemas.push(types.boolean || import_v419.z.boolean());
  }
  if (types.null !== false) {
    allowedSchemas.push(types.null || import_v419.z.null());
  }
  if (types.array !== false) {
    allowedSchemas.push(types.array || import_v419.z.array(import_v419.z.any()));
  }
  ...
  if (types.object !== false) {
    if (types.object) {
      ...
      allowedSchemas.push(import_v419.z.looseObject({}));
    }
  }
  ...
  if (allowedSchemas.length === 0) {
    ...
    if (!hasConstraints) {
      zodSchema = import_v419.z.any();
    } else {
      zodSchema = import_v419.z.union(allowedSchemas);
    }
  }
